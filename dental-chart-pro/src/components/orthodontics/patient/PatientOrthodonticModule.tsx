...
import { Smile, Activity, Calendar, FileText, LayoutGrid, Users, FolderOpen } from 'lucide-react';
...
const PatientOrthodonticModule: React.FC<PatientOrthodonticModuleProps> = ({ patientId }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'timeline' | 'cases' | 'records' | 'lab'>('overview');
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'cases':
        return <OrthoCasesTab cases={cases} onOpenSnapshotEditor={handleOpenSnapshotEditor} />;
      case 'records':
        return <OrthoRecordsTab caseData={cases[0]} />;
      case 'lab':
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 w-fit shadow-sm">
        ...
        <button 
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'records' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
        >
