...
} = require("@shared/models/RefundExecutionRecord");
...
class RefundService {
  ...
  async approveRefund(regionCode, ticketId, actorId, ipAddress, correlationId = null) {
    ...
    metrics.activeRefundTransactions.inc({
      regionCode
    });
    try {
      ...
      if (!ticket || ticket.category !== "REFUND_REQUEST") {
        metrics.refundExecutionTotal.inc({
          status: "failed",
          regionCode
        });
      ...
      const providerRefund = await provider.refundPayment(invoice.paymentReference, amountToRefund, idempotencyKey);
      ...
      const updatedInvoice = await RegionalInvoice.findOneAndUpdate({
        ...
        $inc: {
          refundedAmountMinor: amountToRefund,
          version: 1
        },
        $set: {
          isRefunded: invoice.refundedAmountMinor + amountToRefund >= invoice.finalAmount
        }
      }, {
        session,
        new: true
