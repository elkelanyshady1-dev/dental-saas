...
const {
  auditLogSchema: auditSchemaForForensics
} = require("@shared/models/AuditLog"); // Alias for clarity
const {
  subscriptionMutationRecordSchema
} = require("../../billing/models/SubscriptionMutationRecord.model");
...
async function transitionStatus(regionCode, ticketId, newStatus, actorId, actorType, details = {}, session = null) {
  ...
  logger.info({
    ...
    from: currentStatus,
    to: newStatus
  }, "Ticket status transition recorded");
...
async function getForensicContext(regionCode, ticketId) {
  ...
  const Organization = getPlatformModel(OrganizationDef); // StripeEvent and RefreshToken might be regionally isolated or legacy. 
  ...
  const [org, mutations, audits] = await Promise.all([Organization.findById(ticket.organizationId).select("subscription regionCode"), RegionalMutation.find({
    organizationId: ticket.organizationId
  }).sort({
    createdAt: -1
  }).limit(10), RegionalAuditLog.find({
    organizationId: ticket.organizationId
  }).sort({
    createdAt: -1
  }).limit(10)]);
