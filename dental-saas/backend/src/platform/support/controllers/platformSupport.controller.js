...
exports.getOrgTickets = async (req, res) => {
    try {
        const tickets = await Ticket.find({ organizationId: req.user.organizationId }).sort({ createdAt: -1 });
        res.json(tickets);
    } catch (err) {
...
exports.getPlatformTickets = async (req, res) => {
    try {
        const tickets = await Ticket.find().populate("organizationId", "name").sort({ createdAt: -1 });
        res.json(tickets);
    } catch (err) {
        res.status(500).json({ message: "Failed to fetch tickets", error: err.message });
    }
...
exports.getForensicContext = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Failed to fetch forensic context", error: err.message });
    }
};
