...
const { getBillingSettings } = require("./billing/services/billingSettings.service");
const { resolveExchangeRate } = require("../finance/services/fxResolver.service");
const { logBillingEvent } = require("./billing/services/billingAuditLog.service");
...
async function createRevenueScheduleOnPayment(invoice, contract, session = null) {
    try {
        ...
        const existing = await RevenueSchedule.findOne({ invoiceId: invoice._id });
        if (existing) {
            logger.info({ invoiceId: invoice._id, scheduleId: existing._id },
                "[RevenueSchedule] Already exists — skipping creation");
            return existing;
        }

        const now = new Date();
        const startDate = invoice.billingCycleStart
            ? new Date(invoice.billingCycleStart)
            : new Date(invoice.createdAt || now);
        const endDate = invoice.billingCycleEnd
            ? new Date(invoice.billingCycleEnd)
            : _addMonths(startDate, 1);
        ...
        if (invoiceCurrency && invoiceCurrency !== baseReportingCurrency) {
            try {
                const resolution = await resolveExchangeRate(
                    invoiceCurrency,
                ...
                exchangeRate = null;
                logger.warn({ invoiceCurrency, baseReportingCurrency, invoiceId: invoice._id },
        ...
        const sched = new RevenueSchedule({
            ...
            exchangeRate: exchangeRate ?? 0,  // 0 flags missing rate for ops team
        ...
        logger.error({ err, invoiceId: invoice?._id },
...
module.exports = { createRevenueScheduleOnPayment };
