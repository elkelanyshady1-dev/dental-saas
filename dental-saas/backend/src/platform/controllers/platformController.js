...
exports.getOrganizations = async (req, res) => {
  try {
    // Filters
    const filter = req.query.includeArchived === "true" ? {} : {
      isArchived: {
        $ne: true
      }
    };
    ...
    if (newStatus === "suspended") {
      try {
        ...
        try {
          ...
          try {
            dbManager.releaseConnection(String(organization._id));
          } catch (_) {}
        }
      } catch (err) {
...
exports.globalSearch = async (req, res) => {
  try {
    ...
    }).select("_id name email role isActive").sort({
      name: 1
    }).limit(5).lean()]);
    ...
    res.json({
      ...
      data: {
        organizations,
        branches: [],
        // TODO: federated cross-org search
        users: [],
        // TODO: federated cross-org search
        platformUsers
      }
