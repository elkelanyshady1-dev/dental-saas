...
const PlatformConfig = require("../models/PlatformConfig").default;
...
exports.extendSubscription = async (req, res) => {
    try {
        ...
        const contract = await OrgContract.findOne({ organizationId: org._id, contractStatus: "active" });
        if (!contract) {
            return res.status(404).json({ message: "No active OrgContract found. Cannot extend." });
        }
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                try { dbManager.releaseConnection(String(org._id)); } catch (_) {}
            }
        } catch (err) {
...
exports.reactivateOrganization = async (req, res) => {
    try {
        ...
        res.json({ message: "Organization reactivated", status: org.status });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};
