...
const { platformCapabilityResolver } = require("../../services/platformCapabilityResolver");
...
exports.search = async (req, res) => {
    try {
        ...
        if (!q || q.length < 2) {
            return res.json({ success: true, data: { organizations: [], users: [], contracts: [], invoices: [] }, query: q });
        }
        ...
        const [orgs, users, contracts, invoices] = await Promise.all([
            ...
            canViewOrgs
                ? Organization.find({
                    $or: [{ name: regex }, { slug: regex }, { country: regex }],
                    isArchived: { $ne: true }
                })
                    .select("name slug country subscription.status isActive createdAt")
                    .limit(RESULT_LIMIT)
                    .lean()
                : [],
            ...
            canViewOrgs
                ? OrgContract.find({
                    $or: [{ planCode: regex }, { contractStatus: regex }]
                })
                    .select("planCode contractStatus currency organizationId effectiveTo createdAt")
                    .limit(RESULT_LIMIT)
                    .lean()
                : [],
            ...
            canViewOrgs
                ? PlatformInvoice.find({
                    ...
                    .select("invoiceNumber status totalAmount currency organizationId createdAt")
                    .limit(RESULT_LIMIT)
                    .lean()
                : [],
        ]);
        ...
        logger.info({
            ...
            query: q,
            results: { orgs: orgs.length, users: users.length, contracts: contracts.length, invoices: invoices.length }
        ...
        logger.error({ err, requestId: req.requestId }, "[platformSearch] search failed");
        return res.status(500).json({ success: false, error: "Search failed", requestId: req.requestId });
    }
};
