...
const QUEUES_REMOVED_PAYLOAD = {
  ...
  reason: "BullMQ communication queues + Redis client were removed in Phase 6. " + "Sync delivery goes through communication.dispatcher; async delivery " + "goes through QStash (which manages its own retries). No inspectable " + "queue depth or Redis connectivity exists at the app layer.",
  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        deliveryRate,
        retryRate,
        providerUsage: providerBreakdown.map(p => ({
    ...
    logger.error({
      err: err.message
    }, "[MonitoringCtrl] getEmailMetrics failed");
...
async function getWorkerStatus(req, res) {
  ...
  try {
    ...
    logger.warn({
      err: err.message
    }, "[MonitoringCtrl] recentSent count failed");
  ...
  const overallStatus = recentSent === null ? "unknown" : recentSent > 0 ? "healthy" : "degraded";
  return res.json({
    success: true,
    data: {
      ...status,
      overallStatus,
      note: "queue-depth + redis-ping telemetry removed in Phase 6"
    }
  });
}
module.exports = {
