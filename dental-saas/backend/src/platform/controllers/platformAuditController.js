...
const { getPlatformToken } = require("../../middleware/platformProtect"); // for token in export URL
...
function _buildFilter(query) {
    const filter = { regionCode: "GLOBAL" };
    ...
    if (query.geoLocation) filter.geoLocation = { $regex: query.geoLocation, $options: "i" };
    ...
    if (query.actorName) {
        const re = { $regex: query.actorName, $options: "i" };
        filter.$or = [
            { actorFirstName: re },
            { actorLastName: re }
        ];
...
exports.getAuditLogs = async (req, res) => {
    try {
        ...
        logger.error({ err, requestId: req.requestId }, "[platformAudit] getAuditLogs failed");
...
exports.getEntityAuditTimeline = async (req, res) => {
    try {
        ...
        logger.error({ err, requestId: req.requestId }, "[platformAudit] getEntityAuditTimeline failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
...
exports.exportAuditLogs = async (req, res) => {
    try {
        ...
        if (format === "json") {
            ...
            return res.json({ exported: logs.length, data: logs });
        }

        // ── CSV ──
        const CSV_COLS = [
            "timestamp", "action", "actorFirstName", "actorLastName", "actorRole",
            "actorType", "entityType", "entityId", "ipAddress", "geoLocation",
            "browser", "os", "device", "requestId", "success"
        ];
        ...
        for (const log of logs) {
            rows.push([
                ...
                escape(log.action),
                escape(log.actorFirstName),
                escape(log.actorLastName),
                escape(log.actorRole),
                escape(log.actorType),
                escape(log.entityType || log.entity),
                escape(log.entityId),
                escape(log.ipAddress),
                escape(log.geoLocation),
                escape(log.browser),
                escape(log.os),
                escape(log.device),
                escape(log.requestId || log.correlationId),
                escape(log.success)
            ].join(","));
        }
        ...
        logger.error({ err, requestId: req.requestId }, "[platformAudit] exportAuditLogs failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
    ...
    try {
        ...
        if (!VALID_REGIONS.includes(regionCode)) {
            return res.status(400).json({
                success: false,
                error: { code: "INVALID_REGION", message: `Invalid region '${req.query.region}'. Allowed: ${VALID_REGIONS.join(", ")}` }
            });
        }

        const entries = await AuditLog.find({ regionCode })
            .sort({ createdAt: 1 })
            .limit(limit);

        if (entries.length === 0) {
            return res.json({ valid: true, scannedEntries: 0, message: "No audit entries found." });
        }
        ...
        logger.error({ err }, "[platformAudit] verifyChain failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
