...
exports.list = async (req, res) => {
    try {
        ...
        const filter = unreadOnly ? { readBy: { $nin: [userId] } } : {};
        ...
        logger.error({ err, requestId: req.requestId }, "[platformNotification] list failed");
        return res.status(500).json({ success: false, error: "Failed to load notifications", requestId: req.requestId });
    }
};
...
exports.unreadCount = async (req, res) => {
    try {
        ...
        return res.json({ success: true, count, requestId: req.requestId });
    } catch (err) {
        logger.error({ err, requestId: req.requestId }, "[platformNotification] unreadCount failed");
        return res.status(500).json({ success: false, error: "Failed to get count", requestId: req.requestId });
    }
};
...
exports.markRead = async (req, res) => {
    try {
        ...
        logger.error({ err, requestId: req.requestId }, "[platformNotification] markRead failed");
        return res.status(500).json({ success: false, error: "Failed to mark as read", requestId: req.requestId });
    }
};
...
exports.markAllRead = async (req, res) => {
    try {
        ...
        logger.error({ err, requestId: req.requestId }, "[platformNotification] markAllRead failed");
        return res.status(500).json({ success: false, error: "Failed to mark all as read", requestId: req.requestId });
    }
};
