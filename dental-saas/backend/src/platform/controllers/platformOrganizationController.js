...
} = require("../../platform/billing/services/contractResolver.service");

// ─── Guardian Layer (v2.0) ────────────────────────────────────────────────────
const {
  wrappedProvisionOrganization
} = require("../guardian/provisioning.guardian");
...
exports.createOrganizationProvisioned = asyncHandler(async (req, res) => {
  ...
  const result = await wrappedProvisionOrganization(organizationService.provisionOrganization, data, {
...
exports.getOrganizationDetails = async (req, res) => {
  try {
    ...
    try {
      const [branchCount, userCount, activeUsers, suspendedUsers, auditCount, pendingContract] = await Promise.all([orgModels.Branch.countDocuments({
        organizationId: org._id
      }), orgModels.User.countDocuments({
        organizationId: org._id
      }), orgModels.User.countDocuments({
        organizationId: org._id,
        isActive: true
        ...
        organizationId: org._id,
        isActive: false
      }), orgModels.AuditLog.countDocuments({
        organizationId: org._id
      }),
      ...
      res.json({
        ...
        commercial: {
          ...
          appliedCoupon: commercial.appliedCoupon ? {
            code: commercial.appliedCoupon.code,
            discountType: commercial.appliedCoupon.discountType,
            discountValue: commercial.appliedCoupon.discountValue
          } : null,
...
exports.getOrganizationAnalytics = async (req, res) => {
  try {
    ...
    try {
      const [totalBranches, totalUsers, activeUsers, suspendedUsers, auditEventsCount] = await Promise.all([orgModels.Branch.countDocuments({
      ...
      if (sub.status === "trial") {
        const trialEnd = sub.trialEndsAt ? new Date(sub.trialEndsAt) : (() => {
...
exports.getOrganizationUsers = async (req, res) => {
  try {
    ...
    const orgModels = getOrgModels(orgId);
    try {
      const users = await orgModels.User.find({
        organizationId: orgId
      }).populate("roleId", "name").populate("branchAccess", "name").sort({
        createdAt: -1
      }).lean();
      res.json(users);
    } finally {
...
exports.getOrganizationUserDetails = asyncHandler(async (req, res) => {
  ...
  const orgModels = getOrgModels(orgId);
  try {
    const user = await orgModels.User.findOne({
      _id: userId,
      organizationId: orgId
    }).populate("roleId", "name permissions").populate("branchAccess", "name").lean();
    if (!user) {
    ...
    const securityActions = ["LOGIN_SUCCESS", "LOGIN_FAILURE", "PASSWORD_CHANGED", "PLATFORM_USER_PASSWORD_SET", "PLATFORM_USER_TEMP_PASSWORD_GENERATED", "PLATFORM_USER_RESET_EMAIL_SENT", "PLATFORM_USER_FORCE_LOGOUT"];
...
exports.getOrganizationBranches = async (req, res) => {
  try {
    ...
    const orgModels = getOrgModels(orgId);
    try {
      const branches = await orgModels.Branch.find({
        organizationId: orgId
      }).sort({
        createdAt: -1
      }).lean();
  ...
  try {
    ...
    try {
      ...
      logger.warn({
        err: limitErr,
        orgId
      }, "[BranchCreate] Entitlement limit check failed — allowing");
    }
    const orgModels = getOrgModels(orgId);
    try {
      const {
      ...
      try {
        await orgUsageService.incrementBranches(orgId);
...
exports.getOrganizationAuditLogs = async (req, res) => {
  try {
    ...
    try {
      ...
      const [logs, total] = await Promise.all([orgModels.AuditLog.find(filter).populate("userId", "name email").sort({
        createdAt: -1
      }).skip(skip).limit(Number(limit)).lean(), orgModels.AuditLog.countDocuments(filter)]);
      res.json({
        logs,
        total,
        page: Number(page),
...
exports.updateOrganizationModules = async (req, res) => {
  try {
    ...
    org.modules = {
      ...(org.modules.toObject?.() ?? org.modules),
      ...safeModules
    };
    await org.save();
    res.json({
...
exports.getBranchAnalytics = async (req, res) => {
  try {
    ...
    try {
      ...
      const branch = await orgModels.Branch.findOne({
        _id: branchId,
        organizationId: orgId
      }).lean();
      if (!branch) return res.status(404).json({
      ...
      const [totalUsers, activeUsers, suspendedUsers, auditEventsCount] = await Promise.all([orgModels.User.countDocuments({
        organizationId: orgId,
...
function effectiveHours(branch, org) {
  ...
  return org?.appointmentSettings?.workingHours || {
    start: "08:00",
    end: "20:00"
  };
}
...
exports.getBranchDetails = async (req, res) => {
  try {
    ...
    try {
      ...
      const [userCount, auditCount] = await Promise.all([orgModels.User.countDocuments({
        organizationId: orgId,
        branchAccess: branchId
      }), orgModels.AuditLog.countDocuments({
        organizationId: orgId
...
exports.updateBranchConfiguration = async (req, res) => {
  try {
    ...
    try {
      ...
      const [updated, org] = await Promise.all([orgModels.Branch.findByIdAndUpdate(branchId, {
        $set: $setOps
      }, {
        new: true,
        runValidators: true
...
exports.getOrganizationAnalytics = async (req, res) => {
  try {
    ...
    try {
      ...
      const [branches, totalUsers, activeUsers, totalPatients, patientsThisMonth, totalAppointments, appointmentsThisMonthData] = await Promise.all([orgModels.Branch.find({
        organizationId: orgId
      }).select("name isActive").lean(), orgModels.User.countDocuments({
        organizationId: orgId
      }), orgModels.User.countDocuments({
        organizationId: orgId,
        isActive: true
      }), orgModels.Patient.countDocuments({
        organizationId: orgId
      }), orgModels.Patient.countDocuments({
        organizationId: orgId,
        createdAt: {
      ...
      }), orgModels.Appointment.countDocuments({
        organizationId: orgId
...
exports.getBranchAnalytics = async (req, res) => {
  try {
    ...
    try {
      ...
      const now = new Date();
      const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
      const [totalUsers, activeUsers, totalPatients, totalAppointments, appointmentsThisMonth] = await Promise.all([orgModels.User.countDocuments({
        organizationId: orgId,
        branchAccess: branchId
      }), orgModels.User.countDocuments({
        organizationId: orgId,
...
exports.checkDuplicateOrganizations = asyncHandler(async (req, res) => {
  ...
  const matches = await Organization().find({
    ...
    isArchived: {
      $ne: true
    } // archived orgs are not active conflicts
  }).limit(5).select('name country subscription.status createdAt normalizedName').lean();
...
exports.getOrganizationContracts = asyncHandler(async (req, res) => {
  ...
  const currentContract = org.currentContractId ? contracts.find(c => String(c._id) === String(org.currentContractId)) || null : null;
  return res.json({
    currentContract,
    contracts,
    invoices
...
exports.addContact = asyncHandler(async (req, res) => {
  ...
  if (!CONTACT_ROLES.includes(role)) {
    return res.status(400).json({
      message: `role must be one of: ${CONTACT_ROLES.join(', ')}`
    });
  }
  if (!ownerName?.trim() && !phone?.trim()) {
    return res.status(400).json({
      message: 'Provide at least ownerName or phone.'
    });
  }
  const newContact = {
    role,
...
exports.updateContact = asyncHandler(async (req, res) => {
  ...
  if (!org) return res.status(404).json({
    message: 'Organization or contact not found.'
  });
  return res.json({
    success: true,
    contacts: org.contacts || []
...
exports.updateCrmTask = asyncHandler(async (req, res) => {
  ...
  if (!org) return res.status(404).json({
    message: 'Organization or task not found.'
  });
  return res.json({
    success: true,
    tasks: org.crm?.tasks || []
