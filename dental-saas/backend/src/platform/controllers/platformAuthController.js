...
} = require("../../controllers/platformMetadataController");
const {
...
function _auditPlatformAuthEvent(fields) {
  auditService.createAuditRecord({
    ...
    logger.error({
      ...
      error: err.message
    }, "Failed to write platform auth audit record");
  });
}
...
const platformLogin = async (req, res) => {
  ...
  try {
    ...
    if (TRACE) req.logger.debug({
      event: "AUTH_TRACE_USER_LOOKUP",
      found: !!user,
      isActive: user?.isActive,
      role: user?.role
    }, "[AUTH_TRACE] User lookup");
    if (!user) {
      if (TRACE) req.logger.debug({
        event: "AUTH_TRACE_LOGIN_FAIL",
        reason: "USER_NOT_FOUND"
      }, "[AUTH_TRACE] Login fail");
      _auditPlatformAuthEvent({
    ...
    if (TRACE) req.logger.debug({
      event: "AUTH_TRACE_BCRYPT",
      result: isMatch
    }, "[AUTH_TRACE] bcrypt.compare");
    if (!isMatch) {
      if (TRACE) req.logger.debug({
        event: "AUTH_TRACE_LOGIN_FAIL",
    ...
    if (user.twoFactorEnabled) {
      if (TRACE) req.logger.debug({
        event: "AUTH_TRACE_2FA_REQUIRED",
        userId: user._id
      }, "[AUTH_TRACE] 2FA required");
      return res.json({
        requires2FA: true,
        userId: user._id,
        email: user.email
...
const verify2FA = async (req, res) => {
  try {
    ...
    // 2. Dev Bypass Logic
    const isDevBypass = process.env.NODE_ENV === "development" && process.env.ALLOW_SUPERADMIN_DEV_BYPASS === "true" && code === "000000";
    let isValid = false;
    let isRecoveryCode = false;
    if (isDevBypass) {
      isValid = true;
      req.logger.warn({
        event: "TWO_FA_DEV_BYPASS",
        userId: user._id,
        email: user.email,
        ipAddress: req.ip
      }, "Dev bypass used for platform 2FA");
    } else {
        ...
        for (const rc of user.recoveryCodes) {
          if (!rc.used && (await bcrypt.compare(code.toUpperCase(), rc.codeHash))) {
            ...
            req.logger.warn({
              ...
              email: user.email,
              ipAddress: req.ip
            }, "Recovery code used — global session invalidation triggered");
            _auditPlatformAuthEvent({
              actorId: user._id,
    ...
    if (!isValid) {
      ...
      if (user.failed2FAAttempts >= 5 && user.lastFailed2FAAt > tenMinsAgo) {
        ...
        req.logger.warn({
          ...
          ipAddress: req.ip,
          lockedUntil: user.twoFALockedUntil
        }, "Platform user locked due to failed 2FA attempts");
        _auditPlatformAuthEvent({
          actorId: user._id,
    ...
    if (!knownIP) {
      ...
      req.logger.warn({
        ...
        ipAddress: ip,
        correlationId: req.correlationId
      }, "Platform login from new/untrusted IP");
      _auditPlatformAuthEvent({
        actorId: user._id,
        action: "NEW_IP_LOGIN",
        entity: "PlatformUser",
...
const issueTokens = async (user, req, res) => {
  ...
  if (TRACE) {
    req.logger.debug({
      event: "AUTH_TRACE_JWT_SIGN",
      userId: user._id,
      role: user.role,
      regionCode
    }, "[AUTH_TRACE] JWT signing");
    req.logger.debug({
      event: "AUTH_TRACE_JWT_SECRET",
      defined: !!process.env.JWT_SECRET,
      length: (process.env.JWT_SECRET || "").length
    }, "[AUTH_TRACE] JWT secret check");
  }
  ...
  if (TRACE) req.logger.debug({
    event: "AUTH_TRACE_TOKEN_CREATED",
  ...
  const cookieOpts = {
    ...
    path: "/api/platform",
    // platform-scoped, never /api/auth
    maxAge: REFRESH_TOKEN_EXPIRES_IN
  };
  if (TRACE) req.logger.debug({
    event: "AUTH_TRACE_COOKIE",
    cookieOpts
  }, "[AUTH_TRACE] Setting platformRefreshToken cookie");
  ...
  _auditPlatformAuthEvent({
    actorId: user._id,
    actor: user,
    // v21.0 — for name split + role snapshot
    req,
    // v21.0 — for device/geo extraction
  ...
  return res.json({
    ...
    user: {
      ...
      role: user.role,
      type: "platform",
      // required by PlatformGuard — never omit
...
const setup2FA = async (req, res) => {
  try {
    ...
    res.json({
      qrCodeUrl,
      secret,
      // provide plain secret once for manual entry
      message: "Scan QR code and verify to complete setup"
    });
  } catch (error) {
    req.logger.error({
      event: "TWO_FA_SETUP_ERROR",
      error: error.message,
      stack: error.stack
    }, "2FA setup error");
    res.status(500).json({
...
const complete2FASetup = async (req, res) => {
  try {
    ...
    const recoveryCodesPlain = Array.from({
      length: 10
    ...
    req.logger.error({
      event: "TWO_FA_COMPLETE_ERROR",
      error: error.message,
      stack: error.stack
    }, "2FA complete setup error");
    res.status(500).json({
      message: "Server error"
    });
  }
  ...
  try {
    ...
    if (user.role === "superadmin") {
      ...
      if (superadminCount <= 1) {
        return res.status(400).json({
          message: "Cannot disable 2FA for the required last active superadmin."
        });
      }
    }
    const secret = decrypt(user.twoFactorSecretEncrypted);
...
const platformLogout = async (req, res) => {
  try {
    ...
    _auditPlatformAuthEvent({
      ...
      // v21.0 — for name split
      req,
      // v21.0 — for device/geo
      action: "LOGOUT",
      entity: "PlatformUser",
...
const platformRefresh = async (req, res) => {
  ...
  try {
    if (TRACE) {
      req.logger.debug({
        event: "AUTH_TRACE_REFRESH",
        cookieKeys: Object.keys(req.cookies || {}),
        hasPlatformRefreshToken: !!req.cookies?.platformRefreshToken
      }, "[AUTH_TRACE] Refresh attempt");
    }
    ...
    if (!rawToken) {
      if (TRACE) req.logger.debug({
        ...
        reason: "COOKIE_NOT_SENT"
      }, "[AUTH_TRACE] Refresh fail");
      return res.status(401).json({
        message: "No platform refresh token"
      });
    }
    ...
    if (TRACE) req.logger.debug({
      event: "AUTH_TRACE_REFRESH_DOC",
      found: !!refreshDoc,
      revoked: refreshDoc?.revoked,
      expired: refreshDoc ? refreshDoc.expiresAt < Date.now() : "N/A"
    }, "[AUTH_TRACE] Refresh doc lookup");
    ...
    if (!user || !user.isActive) {
      ...
      return res.status(401).json({
        message: "Platform user inactive"
      });
    }
    ...
    if (process.env.AUTH_TRACE === "true") req.logger.debug({
      event: "AUTH_TRACE_REFRESH_EXCEPTION",
      error: err.message
    }, "[AUTH_TRACE] Refresh exception");
    req.logger.error({
