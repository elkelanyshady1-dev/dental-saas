...
function getOrgModels(organizationId) {
  ...
  if (conn.name === "saasdental") {
    throw new Error(`[platformUserController] RLS VIOLATION: getOrgModels resolved on platform DB ` + `for org=${organizationId}. Expected dental_org_${organizationId}.`);
  }
  return {
...
exports.createPlatformUser = async (req, res) => {
  try {
    const {
      ...
      email,
      role,
      onboardingMethod = "temp_password"
    } = req.body;

    // Support both firstName+lastName and legacy single `name` field
    const resolvedName = firstName && lastName ? `${firstName.trim()} ${lastName.trim()}` : legacyName?.trim() || null;
    ...
    if (onboardingMethod === "invite") {
      emailService.sendInviteEmail ? emailService.sendInviteEmail(newUser, inviteToken).catch(() => {}) : null;
    }
...
exports.getPlatformUserAuditHistory = async (req, res) => {
  try {
    ...
    const logs = await AuditLog_Platform().find({
      ...
      createdAt: -1
    }).select("action entity method statusCode success createdAt").limit(100).lean();
    res.json(logs);
  } catch (error) {
    res.status(500).json({
      message: error.message
...
exports.manageCredentials = async (req, res) => {
  try {
    ...
    const {
      mode,
      password
    } = manageCredentialsSchema.parse(req.body);
    const actor = req.platformUser;
    ...
    switch (mode) {
      ...
      case "set_password":
        if (!isSuperAdmin) return res.status(403).json({
          message: "Only superadmin can set password manually"
        });
        if (!password) return res.status(400).json({
          message: "Password is required for set_password mode"
        });
      ...
      case "temporary_password":
        if (!isSuperAdmin) return res.status(403).json({
          message: "Only superadmin can generate temporary password"
        });
...
exports.getPlatformUserDetail = async (req, res) => {
  try {
    ...
    const seen = new Set();
    const auditLogs = [...asActor, ...asTarget].filter(l => {
...
exports.resetPlatformUser2FA = async (req, res) => {
  try {
    ...
    res.json({
      message: "2FA reset and user logged out successfully"
    });
  } catch (error) {
    res.status(500).json({
      message: error.message
...
exports.updateOrgUserProfile = async (req, res) => {
  try {
    ...
    const {
      User
    } = getOrgModels(organizationId);
    logger.debug({
      db: User.db.name,
      organizationId
    }, "[updateOrgUserProfile] DB resolved");
...
exports.getOrganizationRolesGovernance = async (req, res) => {
  try {
    ...
    logger.debug({
      db: conn.name,
      organizationId
    }, "[getOrgRoles] DB resolved");

    // Per-org DB: no organizationId filter needed — DB isolates tenant
    let roles = await Role.find().select("name isSystemRole _id").sort({
      name: 1
    }).lean();
    ...
    if (roles.length === 0) {
      logger.warn({
        organizationId
      }, "[getOrgRoles] No roles found — auto-seeding");
      const initializeRoles = require("@utils/roleInitializer");
      await initializeRoles(organizationId, {
        connection: conn
      });
      roles = await Role.find().select("name isSystemRole _id").sort({
        name: 1
      }).lean();
    }
    res.json({
...
exports.getOrganizationUserDetailGovernance = async (req, res) => {
  try {
    ...
    const [asActor, asTarget] = await Promise.all([AuditLog.find({
      actorId: userId,
      actorType: "tenant_user"
    }).sort({
      createdAt: -1
    }).limit(50).lean(), AuditLog.find({
      entityId: userId
    }).sort({
      createdAt: -1
    }).limit(50).lean()]);
...
exports.createOrganizationUserGovernance = async (req, res) => {
  try {
    ...
    const resolvedName = firstName && lastName ? `${firstName.trim()} ${lastName.trim()}` : legacyName?.trim() || firstName?.trim() || null;
    ...
    if (roleCount === 0) {
      logger.warn({
        organizationId
      }, "[createOrgUser] No roles found — auto-seeding from SSOT");
      const initializeRoles = require("@utils/roleInitializer");
      await initializeRoles(organizationId, {
        connection: conn
      });
    ...
    if (!roleDoc && roleNameField) {
      ...
      roleDoc = await Role.findOne({
        name: {
          $regex: new RegExp(`^${roleNameField.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i")
        }
      });
    }
    if (!roleDoc) {
...
exports.updateOrganizationUserStatusGovernance = async (req, res) => {
  try {
    ...
    logger.debug({
      db: conn.name,
      organizationId
    }, "[updateOrgUserStatus] DB resolved");
...
exports.updateOrganizationUserRoleGovernance = async (req, res) => {
  try {
    ...
    if (!newRole) return res.status(400).json({
    ...
    res.json({
      message: "User role overridden successfully",
      user
    });
  } catch (error) {
    res.status(500).json({
...
exports.forceLogoutOrganizationUser = async (req, res) => {
  try {
    ...
    logger.debug({
      db: conn.name,
      organizationId
    }, "[forceLogoutOrgUser] DB resolved");
    ...
    res.json({
      message: "Organization user forced to logout successfully"
    });
  } catch (error) {
    res.status(500).json({
      message: error.message
...
exports.resetOrganizationUserPassword = async (req, res) => {
  try {
    ...
    logger.debug({
      ...
      organizationId,
      userId
    }, "[resetOrgUserPassword] DB resolved");
    ...
    if (!user) {
      logger.warn({
        ...
        organizationId,
        db: conn.name
      }, "[resetOrgUserPassword] User not found in org DB");
      return res.status(404).json({
        success: false,
        message: "User not found"
      });
    ...
    logger.info({
      ...
      action: auditAction,
      db: conn.name
    }, "[resetOrgUserPassword] Complete");

    // ✅ Guaranteed response — always returns JSON, never hangs
    return res.json({
      success: true,
      message: responseMessage,
      ...(mode === "temp_password" && temporaryPassword ? {
        temporaryPassword
      } : {})
    });
  } catch (error) {
