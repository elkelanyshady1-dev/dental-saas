...
communicationRetryLogSchema.index({ channel: 1, createdAt: -1 });
communicationRetryLogSchema.index({ isDLQ: 1, createdAt: -1 });
communicationRetryLogSchema.index({ channel: 1, isDLQ: 1, createdAt: -1 });

// TTL: auto-delete retry logs older than 90 days
communicationRetryLogSchema.index({ createdAt: 1 }, { expireAfterSeconds: 90 * 24 * 60 * 60 });
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, communicationRetryLogSchema),
};
