...
const {
  resolveAccessType
} = require("../utils/accessTypeResolver");

// ── v22.3: Reconciliation engine ─────────────────────────────────────────────
const {
  reconcileBilling
} = require("../services/billingReconciliation.service");
...
const SOURCE_MAP = {
  ...
  // platform admin placing a contract = sales-managed
...
const BillingOrchestrator = {
  ...
  async markUncollectible(invoiceId, opts = {}) {
    logger.info({
      invoiceId
    }, "[Orchestrator] markUncollectible");
  ...
  async resolveProcessing(invoiceId, outcome, opts = {}) {
    logger.info({
      invoiceId,
      outcome
    }, "[Orchestrator] resolveProcessing");
  ...
  async recordManualPayment(data) {
    ...
    }, "[Orchestrator] recordManualPayment");
  ...
  async previewUpgradePrice({
    ...
    const pricing = await computePrice({
      ...
      ...(customPriceOverride !== undefined ? {
        overridePrice: customPriceOverride
      } : {})
    });
  ...
  async upgradeSubscription({
    ...
    const log = (msg, extra = {}) => logger.info({
      organizationId,
      planVersionId,
      actorId,
      requestId,
      ...extra
    }, `[Orchestrator] upgrade.${msg}`);
    log("start");
    ...
    if (org.isArchived) {
      const e = new Error(`Organization ${organizationId} is archived`);
      e.status = 409;
      throw e;
    }
    if (!pv) {
      const e = new Error(`PlanVersion ${planVersionId} not found`);
      e.status = 404;
      throw e;
    }
    if (pv.status === "draft") {
      const e = new Error(`PlanVersion ${planVersionId} is a draft — publish it first`);
      e.status = 400;
      throw e;
    }
    if (pv.visibility === "internal") {
      const e = new Error(`PlanVersion ${planVersionId} has visibility=internal — not available for upgrades`);
      e.status = 400;
      throw e;
    }
    if (!["monthly", "yearly", "biennial"].includes(billingInterval)) {
      const e = new Error(`Invalid billingInterval: "${billingInterval}"`);
      e.status = 400;
      throw e;
    }
    ...
    if (existingPendingPayment) {
      const e = new Error(`Organization already has a pending_payment contract (${existingPendingPayment._id}). ` + `Complete or cancel it before creating a new subscription.`);
      e.status = 409;
      e.code = "CONTRACT_ALREADY_PENDING_PAYMENT";
      throw e;
    }
    ...
    const accessType = payloadAccessType ?? resolveAccessType({
    ...
    if (accessType !== "paid") {
      ...
      const contract = await this.atomicContractSwitch({
        ...
        contractPayload: {
          ...
          initialStatus: "draft",
          metadata: new Map([["initiatedFrom", "platform_ui"], ["initiatedByRole", "superadmin"], ["requestId", requestId || null], ["accessType", accessType], ["entitlementOverrides", entitlementOverrides ? JSON.stringify(entitlementOverrides) : null]])
        },
        actorId,
        requestId
      });
        ...
        setImmediate(async () => {
          try {
            await led().writeLedgerEntry({
              ...
              // valid LEDGER_SOURCES enum value
            ...
            logger.error({
              ...
              organizationId,
              requestId
            }, "[Orchestrator] nonBillable — ledger write failed");
          }
        });
      } else {
        logger.info({
    ...
    try {
      ...
      contract = await sub().createContract({
        ...
        trialDays: resolvedTrialDays,
        accessType,
        // v23.0: always "paid" in this branch
        pricingSnapshot: pricing.snapshot,
        source: normalizeSource("platform_admin"),
        paymentProvider: paymentMethod || "manual",
      ...
      invoice = await inv().issueInvoice(String(invoice._id), {
        actorId,
        session
      });
      log("invoice.issued", {
  ...
  async atomicContractSwitch({
    ...
    if (idempotencyKey) {
      ...
      if (existing) {
        log("idempotent.return", {
          contractId: String(existing._id),
          status: existing.contractStatus
        });
        return existing;
      }
    }
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        if (activeCount > 1) {
          throw new Error(`INVARIANT_VIOLATION: Organization ${organizationId} already has ${activeCount} ` + `active contracts. Run scripts/repairContractPointers.js --commit before switching.`);
        }
        ...
        contract = await sub().createContract(contractPayload, actorId, {
          session
        });
      ...
      setImmediate(async () => {
        try {
          ...
          if (!ptrContract || ptrContract.contractStatus !== "active") {
            logger.error({
              organizationId,
              currentContractId: orgCheck?.currentContractId
  ...
  async renewSubscription({
    ...
    logger.info({
      ...
      currency,
      requestId
    }, "[Orchestrator] renewSubscription — start");
    const {
      invoice,
      isNew
    } = await inv().generateInvoice(contractId);
    let payment;
    try {
      ({
        payment
      } = await pay().applyPayment({
        invoiceId: String(invoice._id),
        amount,
        method,
        provider,
      ...
      logger.error({
        ...
        contractId,
        requestId
      }, "[Orchestrator] renewSubscription — payment failed");
      throw payErr;
    }
    ...
    await led().writeLedgerEntry({
      ...
      logger.error({
        ...
        contractId,
        requestId
      }, "[Orchestrator] renewSubscription — ledger write failed (non-fatal)");
    });
    return {
      invoice,
      payment,
  ...
  async safeUpgradeSubscription(params) {
    try {
      ...
      const isIntegrity = err instanceof UpgradeIntegrityError || err.code === "LEDGER_WRITE_CRITICAL_FAILURE";
      logger.error({
        err,
