...
const BillingEventLog = require("@shared/models/BillingEventLog").default;
const { computePayloadHash } = require("@shared/models/BillingEventLog");
const { writeLedgerEntry } = require("../models/BillingLedger.model");
const { updatePaymentStatusByProviderPaymentId } = require("./paymentStatusService");
...
async function handleCanonicalEvent(event, options = {}) {
    const { regionCode = "GLOBAL", correlationId } = options;

    // ── Guard 1: Provider validation ──────────────────────────────────────────
    if (!SUPPORTED_PROVIDERS.includes(event.provider)) {
        throw new Error(
            `[canonicalEventProcessor] Unsupported provider: "${event.provider}". ` +
    ...
    if (!CANONICAL_TYPE_HANDLERS[event.type]) {
        ...
        return { processed: false, reason: "unrecognized_canonical_type" };
    }
    ...
    await CANONICAL_TYPE_HANDLERS[event.type](event, { regionCode, correlationId });
...
async function handlePaymentSucceeded(event, { regionCode, correlationId }) {
    const { amount, currency, metadata } = event;
    ...
    if (platformInvoiceId) {
        ...
        if (invoice) {
            logger.info(
                { platformInvoiceId, paymentIntentId, correlationId },
    ...
    if (!invoice && paymentIntentId) {
        invoice = await PlatformInvoice.findOne({ providerPaymentId: paymentIntentId });
        if (invoice) {
            logger.info(
                { paymentIntentId, invoiceId: invoice._id, correlationId },
    ...
    if (invoice.status === "paid") {
        logger.info(
            { invoiceId: invoice._id, correlationId },
    ...
    if (invoice.providerPaymentId) {
        await updatePaymentStatusByProviderPaymentId({
            ...
            actorType: "system",
            regionCode,
            metadata: { source: "canonical_event", type: "payment.succeeded", correlationId }
        });
    }
    ...
    logger.info(
        { invoiceId: invoice._id, organizationId: invoice.organizationId, amount, currency },
    ...
    setImmediate(async () => {
        try {
            ...
            logger.error({ err, invoiceId: invoice._id }, "[canonicalEventProcessor] reactivateOnPayment failed (non-fatal)");
        }
    });
    ...
    await _writeAudit({
        ...
        details: { paymentIntentId, platformInvoiceId, amount, currency, correlationId }
    });
    ...
    await writeLedgerEntry({
        ...
        metadata: { paymentIntentId, platformInvoiceId, correlationId }
    });
    ...
    await _persistPaymentAttempt({
        ...
        metadata: { source: "payment.succeeded", paymentIntentId, platformInvoiceId, correlationId }
    });
    ...
    setImmediate(async () => {
        await emitBillingTimelineEvent({
            ...
            providerEventId: event.externalId,
            source: "system",
            payload: { provider: event.provider, amount, currency, paymentIntentId, correlationId }
        });
    });
}
 ...
 */
async function handlePaymentFailed(event, { regionCode, correlationId }) {
    const { externalId: providerPaymentId, metadata } = event;

    const invoice = await PlatformInvoice.findOne({ providerPaymentId });

    if (!invoice) {
        logger.warn({ providerPaymentId, correlationId }, "[canonicalEventProcessor] payment.failed: no invoice found");
        return;
    }
    ...
    await updatePaymentStatusByProviderPaymentId({
        ...
        actorType: "system",
        regionCode,
        metadata: { source: "canonical_event", type: "payment.failed", correlationId }
    });
    ...
    await _writeAudit({
        ...
        entityId: invoice._id,
        details: { providerPaymentId, reason: invoice.failureReason, correlationId }
    });
    ...
    await writeLedgerEntry({
        ...
        currency: invoice.currency,
        source: "canonicalEventProcessor",
        metadata: { providerPaymentId, reason: invoice.failureReason, correlationId }
    });
    ...
    await _persistPaymentAttempt({
        ...
        requestId: correlationId ?? null,
        metadata: { source: "payment.failed", providerPaymentId, correlationId }
    });
    ...
    setImmediate(async () => {
        await emitBillingTimelineEvent({
            ...
            providerEventId: event.externalId,
            source: "system",
            payload: { provider: event.provider, reason: invoice.failureReason, correlationId }
        });
    });
}
 ...
 */
async function handleRefundCompleted(event, { regionCode, correlationId }) {
    const { externalId: providerPaymentId, amount, metadata } = event;

    const invoice = await PlatformInvoice.findOne({ providerPaymentId });

    if (!invoice) {
        logger.warn({ providerPaymentId, correlationId }, "[canonicalEventProcessor] refund.completed: no invoice found");
        return;
    }
    ...
    await updatePaymentStatusByProviderPaymentId({
        ...
        actorType: "system",
        regionCode,
        metadata: { source: "canonical_event", type: "refund.completed", correlationId }
    });
    ...
    await _writeAudit({
        ...
        entityId: invoice._id,
        details: { providerPaymentId, amount, isFullRefund, correlationId }
    });
    ...
    await writeLedgerEntry({
        ...
        currency: invoice.currency,
        source: "canonicalEventProcessor",
        metadata: { providerPaymentId, isFullRefund, correlationId }
    });
    ...
    await _persistPaymentAttempt({
        ...
        status: "refunded",
        requestId: correlationId ?? null,
        metadata: { source: "refund.completed", isFullRefund, correlationId }
    });
}
 ...
 */
async function handleDisputeCreated(event, { regionCode, correlationId }) {
    const { externalId: providerPaymentId, metadata } = event;

    const invoice = await PlatformInvoice.findOne({ providerPaymentId });

    if (!invoice) {
        logger.warn({ providerPaymentId, correlationId }, "[canonicalEventProcessor] dispute.created: no invoice found");
        return;
    }

    // Idempotency check — avoid duplicate dispute tickets
    const existing = await Ticket.findOne({ providerDisputeId: event.externalId });
    if (existing) {
        logger.info({ providerDisputeId: event.externalId, correlationId }, "[canonicalEventProcessor] Dispute ticket already exists");
        return;
    }
    ...
    await updatePaymentStatusByProviderPaymentId({
        ...
        actorType: "system",
        regionCode,
        metadata: { source: "canonical_event", type: "dispute.created", correlationId }
    });
    ...
    await Ticket.create({
        ...
        createdBy: "000000000000000000000000", // system actor
        category: "dispute",
        priority: "HIGH",
        status: "OPEN",
    ...
    await _writeAudit({
        ...
        entity: "PLATFORM_INVOICE",
        entityId: invoice._id,
        details: { providerDisputeId: event.externalId, amount: event.amount, correlationId }
    });
    ...
    await _persistPaymentAttempt({
        ...
        status: "disputed",
        requestId: correlationId ?? null,
        metadata: { source: "dispute.created", disputeId: event.externalId, correlationId }
    });
}
 ...
 */
async function handleSubscriptionCreated(event, { regionCode, correlationId }) {
    logger.info({ event, correlationId }, "[canonicalEventProcessor] subscription.created — informational only");
    await writeLedgerEntry({
        ...
        source: "canonicalEventProcessor",
        metadata: { correlationId, raw: event.metadata }
    });
}
 ...
 */
async function handleSubscriptionCanceled(event, { regionCode, correlationId }) {
    logger.info({ event, correlationId }, "[canonicalEventProcessor] subscription.canceled — informational only");
    await writeLedgerEntry({
 ...
 */
async function handleSubscriptionUpdated(event, { regionCode, correlationId }) {
    logger.info({ event, correlationId }, "[canonicalEventProcessor] subscription.updated — informational only");
}
...
async function _writeAudit({ organizationId, action, entity, entityId, details }) {
    try {
        ...
        logger.error({ err, action, entityId }, "[canonicalEventProcessor] Audit write failed — non-fatal");
    }
}
 ...
 */
async function _persistPaymentAttempt({ invoiceId, contractId, organizationId, provider, providerPaymentId, providerEventId, amount, currency, status, errorCode, errorMessage, requestId, metadata }) {
    try {
        const existingCount = await PaymentAttempt.countDocuments({ invoiceId });
        ...
        logger.error(
            { err, invoiceId, status, provider },
            "[canonicalEventProcessor] _persistPaymentAttempt failed — non-fatal"
        );
    }
}

module.exports = { handleCanonicalEvent };
