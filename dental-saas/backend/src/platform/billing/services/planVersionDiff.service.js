...
function diffPricing(pricingA = {}, pricingB = {}) {
    ...
    const regionsA = Object.fromEntries(
        (pricingA.regions || []).map(r => [r.regionCode, r])
    );
    const regionsB = Object.fromEntries(
        (pricingB.regions || []).map(r => [r.regionCode, r])
    );
    ...
    if (normalize(bcA) !== normalize(bcB)) {
        changes.push({ regionCode: "BASE", currency: "", field: "baseCurrency", from: bcA, to: bcB });
    }
...
function diffLimits(limitsA = {}, limitsB = {}) {
    return diffObjects(limitsA, limitsB, (field, from, to) => ({ field, from, to }));
}
...
function diffModules(modulesA = {}, modulesB = {}) {
    return diffObjects(modulesA, modulesB, (module, from, to) => ({ module, from, to }));
}
...
async function comparePlanVersions(versionAId, versionBId) {
    ...
    const [vA, vB] = await Promise.all([
        PlanVersion.findById(toOid(versionAId))
            .select("versionTag label templateCode pricing limits modules trialDays inflationPolicy")
            .lean(),
        PlanVersion.findById(toOid(versionBId))
            .select("versionTag label templateCode pricing limits modules trialDays inflationPolicy")
            .lean()
    ]);
    ...
    const trialChange = (vA.trialDays ?? null) !== (vB.trialDays ?? null)
        ? { from: vA.trialDays ?? null, to: vB.trialDays ?? null }
        : null;
    ...
    const inflationChange = ipA !== ipB
        ? { from: vA.inflationPolicy ?? null, to: vB.inflationPolicy ?? null }
        : null;
...
module.exports = { comparePlanVersions };
