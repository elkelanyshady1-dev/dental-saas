...
const RefundExecutionRecord = require("@shared/models/RefundExecutionRecord").default;
...
const { assertValidRefundTransition } = require("../domain/refundStateMachine");
const { validateRefundEligibility } = require("./refundPolicy.service");
const { logBillingEvent } = require("./billingAuditLog.service");
 ...
 */
async function requestRefund({ invoiceId, contractId, amount, reasonCode, requestedBy, idempotencyKey }) {
    // ── Idempotency check ───────────────────────────────────────────────────
    if (idempotencyKey) {
        const existing = await RefundExecutionRecord.findOne({ idempotencyKey }).lean();
        if (existing) {
            logger.info({ idempotencyKey, recordId: existing._id }, "[RefundProcessor] Idempotent skip on requestRefund");
            return { record: existing, requiresApproval: false, fraudFlag: false, idempotent: true };
        }
    }
    ...
    if (!contract) {
        ...
        err.status = 404; err.code = "CONTRACT_NOT_FOUND"; throw err;
    }
    ...
    if (!decision.allowed) {
        ...
        err.status = 422; err.code = "REFUND_POLICY_DENIED"; throw err;
    }
    ...
    if (requestedMinor > remainingRefundable) {
        const err = new Error(
            `Refund amount (${requestedMinor} minor) exceeds remaining refundable amount (${remainingRefundable} minor).`
        );
        err.status = 422; err.code = "REFUND_EXCEEDS_INVOICE"; throw err;
    }
    ...
    if (decision.fraudFlag) {
        logger.warn({ recordId: record._id, organizationId: invoice.organizationId },
            "[RefundProcessor] FRAUD FLAG: velocity limit reached on refund request");
    }

    logger.info({ recordId: record._id, invoiceId, amount, requiresApproval: decision.requiresApproval },
        "[RefundProcessor] Refund requested");

    return { record, requiresApproval: decision.requiresApproval, fraudFlag: decision.fraudFlag };
}
...
async function approveRefund(refundId, approvedBy) {
    ...
    if (!record) {
        ...
        err.status = 404; err.code = "REFUND_NOT_FOUND"; throw err;
    }
    ...
    logger.info({ refundId, approvedBy }, "[RefundProcessor] Refund approved");
    return record;
}
...
async function rejectRefund(refundId, rejectedBy, reason = "manual_rejection") {
    ...
    logger.info({ refundId, rejectedBy, reason }, "[RefundProcessor] Refund rejected");
    return record;
}
...
async function processRefund(refundId, processedBy) {
    ...
    if (record.status === "refund_completed") {
        logger.info({ refundId }, "[RefundProcessor] Idempotent skip — already completed");
    ...
    if (record.providerRefundId) {
        ...
        if (byProvider && byProvider._id.toString() !== refundId) {
            logger.warn({ refundId, providerRefundId: record.providerRefundId },
    ...
    await logBillingEvent({
        ...
        metadata: { refundId: record._id.toString(), amountMinor: record.amountMinor }
    });
    ...
    try {
        if (contract?.paymentProvider && invoice.providerPaymentId) {
            ...
            providerRefund = { status: "succeeded", id: `manual:${Date.now()}` };
        }
    } catch (providerErr) {
        logger.error({ providerErr, refundId }, "[RefundProcessor] Provider refund call failed");
        ...
        await logBillingEvent({
            ...
            performedBy: processedBy,
            metadata: { refundId: record._id.toString(), reason: providerErr.message }
        });
        throw providerErr;
    }
    ...
    if (!providerRefund || providerRefund.status === "failed") {
        ...
        err.code = "PROVIDER_REFUND_REJECTED"; err.status = 502; throw err;
    }
    ...
    try {
        ...
        const schedule = await RevenueSchedule.findOne({ invoiceId: invoice._id }).session(session);
        if (schedule && schedule.totalAmount > 0) {
            ...
            if (Math.abs(integritySum - integrityTarget) > epsilon) {
                ...
                const intErr = new Error(
                    `[RefundProcessor] Revenue integrity violation: ` +
                    `recognized(${schedule.recognizedAmount}) + deferred(${schedule.deferredAmount}) ` +
                    `!= total(${schedule.totalAmount})`
                );
                intErr.code = "REVENUE_INTEGRITY_VIOLATION"; intErr.status = 500;
                throw intErr;
            }
        ...
        logger.info({ refundId, providerRefundId: providerRefund.id, amountMinor: record.amountMinor },
            "[RefundProcessor] Refund completed");

        return { record, providerRefundId: providerRefund.id };
...
async function _failRefund(record, reason) {
    try {
        ...
        logger.warn({ smErr: smErr.message, refundId: record._id }, "[RefundProcessor] State machine rejected fail transition (may already be terminal)");
    }
}
