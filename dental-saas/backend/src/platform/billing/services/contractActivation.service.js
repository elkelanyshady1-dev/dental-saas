...
async function applyModuleEntitlements(org, planVersion) {
  ...
  const mappings = {
    ...
    finance: "accounting",
    // PlanVersion uses "finance", Org uses "accounting"
    inventory: "inventory",
    lab: "labs",
  ...
  try {
    ...
    if (!skipInvoiceCheck) {
      ...
      if (invoice.status !== "paid") {
        throw new ContractActivationError(`Cannot activate contract: Invoice ${invoiceId} is not paid (status: "${invoice.status}"). ` + `Contract activation requires invoice.status = "paid".`, "INVOICE_NOT_PAID");
      }
    }
    ...
    if (contract.contractStatus !== "pending_payment" && contract.effectiveFrom && contract.effectiveFrom > new Date()) {
      throw new ContractActivationError(`Cannot activate contract ${contractId} before its effective date (${contract.effectiveFrom.toISOString()})`, "CANNOT_ACTIVATE_BEFORE_EFFECTIVE_DATE");
    }
    ...
    if (invoice && String(invoice.organizationId) !== String(contract.organizationId)) {
      throw new ContractActivationError(`Invoice ${invoiceId} belongs to org ${invoice.organizationId}, not ${contract.organizationId}`, "ORG_MISMATCH");
    }
    ...
    if (previousContract) {
      ...
      logger.info({
        previousContractId: previousContract._id,
