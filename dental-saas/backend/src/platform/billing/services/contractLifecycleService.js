...
async function suspendContract(contractId, opts = {}) {
    const { reason = "Non-payment", actorId = null, actorType = "system", requestId = null } = opts;
    ...
    if (!allowedSources.includes(contract.contractStatus)) {
        const err = new Error(
            `Cannot suspend contract in status "${contract.contractStatus}". ` +
            `Only active/grace contracts can be suspended.`
        );
        err.statusCode = 409;
    ...
    logger.info(
        { contractId, from: prevStatus, to: "suspended", actorId, requestId },
    ...
    setImmediate(async () => {
        try {
            await writeLedgerEntry({
                eventType: "contract.suspended",     // v21.0: explicit event type
            ...
            await emitBillingTimelineEvent({
                ...
                source: actorType,
                payload: { previousStatus: prevStatus, reason, actorId }
            });
        } catch (e) {
            logger.error({ err: e }, "[contractLifecycleService] suspendContract ledger/timeline write failed (non-fatal)");
        }
    });
...
async function voidContract(contractId, opts = {}) {
    const { reason = "", actorId = null, requestId = null } = opts;

    const [contract, paidInvoiceCount] = await Promise.all([
        OrgContract.findById(contractId),
        PlatformInvoice.countDocuments({ contractId, status: "paid" })
    ]);
    ...
    if (terminalStates.includes(contract.contractStatus)) {
        const err = new Error(
            `Contract is in terminal status "${contract.contractStatus}" and cannot be voided.`
        );
        err.statusCode = 409;
    ...
    if (paidInvoiceCount > 0) {
        const err = new Error(
            `Cannot void contract: ${paidInvoiceCount} paid invoice(s) exist. ` +
            `Refund all payments before voiding. (CONTRACT_HAS_PAID_INVOICES)`
        );
        err.statusCode = 409;
    ...
    logger.info(
        { contractId, from: prevStatus, to: "void", actorId, requestId },
    ...
    setImmediate(async () => {
        try {
            await writeLedgerEntry({
                eventType: "contract.voided",         // v21.0: explicit event type
                organizationId: contract.organizationId,
                contractId: contract._id,
                invoiceId: null,
                provider: "internal",
            ...
            logger.error({ err: e }, "[contractLifecycleService] voidContract ledger/timeline write failed (non-fatal)");
        }
    });
...
async function graceContract(contractId, opts = {}) {
    const { failureReason = "Payment failed", gracePeriodDays = 7 } = opts;
    ...
    logger.info(
        { contractId, from: prevStatus, to: "grace", graceEndsAt, failureReason },
    ...
    setImmediate(async () => {
        try {
            await writeLedgerEntry({
                ...
                source: "paymentApplication",
                actorType: "system",
                metadata: { action: "CONTRACT_GRACE", failureReason, graceEndsAt }
            });
        } catch (e) {
            logger.error({ err: e }, "[contractLifecycleService] graceContract ledger write failed (non-fatal)");
        }
    });
...
module.exports = { suspendContract, voidContract, graceContract };
