...
async function streamInvoicesCsv(filters, res) {
    ...
    const header = rowToCsv([
        "Invoice Number", "Organization ID", "Total Amount", "Currency",
        "Status", "Payment Status", "Created At", "Paid At", "Due Date"
    ]);
    res.write(header);

    try {
        const cursor = PlatformInvoice
            .find(filters)
            .select("invoiceNumber organizationId totalAmount currency status paymentStatus createdAt paidAt dueDate")
            .sort({ createdAt: -1 })
            .limit(5000)
            .lean()
            .cursor();
        ...
        logger.error({ err }, "[financeExport] streamInvoicesCsv failed");
...
async function streamLedgerCsv(filters, res) {
    ...
    const header = rowToCsv([
        "Timestamp", "Event Type", "Amount", "Currency",
        "Organization ID", "Contract ID", "Invoice ID",
        "Provider", "Source", "Actor Type"
    ]);
    res.write(header);

    try {
        const cursor = BillingLedger
            .find(filters)
            .select("createdAt eventType amount currency organizationId contractId invoiceId provider source actorType")
            .sort({ createdAt: -1 })
            .limit(5000)
            .lean()
            .cursor();
        ...
        logger.error({ err }, "[financeExport] streamLedgerCsv failed");
        res.write(rowToCsv(["ERROR", err.message]));
    } finally {
        res.end();
    }
...
async function streamPaymentsCsv(filters, res) {
    ...
    const header = rowToCsv([
        "Timestamp", "Organization ID", "Invoice ID",
        "Provider", "Provider Payment ID", "Amount", "Currency",
        "Status", "Attempt #", "Error Code", "Error Message"
    ]);
    res.write(header);

    try {
        const cursor = PaymentAttempt
            .find(filters)
            .select("createdAt organizationId invoiceId provider providerPaymentId amount currency status attemptNumber errorCode errorMessage")
            .sort({ createdAt: -1 })
            .limit(5000)
            .lean()
            .cursor();
        ...
        logger.error({ err }, "[financeExport] streamPaymentsCsv failed");
        res.write(rowToCsv(["ERROR", err.message]));
    } finally {
        res.end();
    }
