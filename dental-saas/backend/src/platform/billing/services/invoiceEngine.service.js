...
const InvoiceSequence = require("../models/InvoiceSequence.model").default;
...
async function generatePlatformInvoice(contractId, options = {}) {
    ...
    if (!["draft", "active", "ready", "pending_payment"].includes(contract.contractStatus)) {
        const err = new Error(
            `Cannot generate invoice for contract in status "${contract.contractStatus}". ` +
            `Allowed: draft, ready, pending_payment, active.`
        );
        err.status = 400;
    ...
    const existingQuery = PlatformInvoice.findOne({ idempotencyKey });
    if (session) existingQuery.session(session);
    const existing = await existingQuery;
    if (existing) {
        logger.info(
            { invoiceId: existing._id, contractId, idempotencyKey },
    ...
    if (creditApplied > 0) {
        const freshContract = await OrgContract.findById(contract._id)
            .select("creditBalance")
            .session(session || null)
            .lean();
        if (!freshContract || freshContract.creditBalance < creditApplied) {
            const err = new Error(
                `[InvoiceEngine] Credit race condition detected: ` +
                `creditBalance=${freshContract?.creditBalance ?? 0} < creditApplied=${creditApplied} ` +
                `for contract ${contract._id}. Aborting to prevent double-deduction.`
            );
    ...
    const resolvedType = invoiceType ||
        (contract.contractStatus === "draft" ? "initial" : "subscription");
    ...
    if (creditApplied > 0) {
        const updateOpts = { ...createOpts, new: false };
