...
} = require("@shared/models/DomainEventOutbox");
const {
  ticketSchema
...
exports.cancelSubscription = async ({
  ...
  if (lockToken) {
    try {
      ...
      let mutation = await RegionalMutation.findOne({
        idempotencyKey
      });
      if (mutation && mutation.status === "COMPLETED") return {
        success: true,
        idempotent: true
      };
      if (!mutation) {
      ...
      const providerResult = await provider.cancelSubscription(org.subscription.providerSubscriptionId, {
        cancelAtPeriodEnd,
        idempotencyKey
      });
      ...
      try {
        ...
        metrics.mutationPubSubNotificationsTotal?.inc({
          status: "COMPLETED",
          regionCode
        });
        return {
      ...
      logger.error({
        ...
        orgId,
        idempotencyKey
      }, "cancelSubscription mutation failed");
      throw error;
    } finally {
...
exports.adjustCredits = async ({
  ...
  if (lockToken) {
    try {
      ...
      const providerResult = await provider.applyCustomerCredit(org.subscription.providerCustomerId, amountMinor,
      // Sprint 4: billingCurrency removed from subscription subdoc — use org.billingCurrency
      (org.billingCurrency || "usd").toLowerCase(), idempotencyKey);
      const orgLockToken = await acquireOrgLockDistributed(orgId);
      const session = await mongooseConnection.startSession();
      try {
        const transactionLogic = async sess => {
          ...
          const activeContract = await require("../../billing/models/OrgContract.model").findOne({
            organizationId: orgId,
            contractStatus: "active"
          }).select("creditBalance").session(sess).lean();
...
exports.executeRefund = async ({
  ...
  if (lockToken) {
    try {
      ...
      const providerResult = await provider.refundPayment(invoice.paymentReference, amountMinor, idempotencyKey);
      const orgLockToken = await acquireOrgLockDistributed(orgId);
      const session = await mongooseConnection.startSession();
      try {
        const transactionLogic = async sess => {
          await RegionalTicket.updateOne({
            ...
            $inc: {
              financialImpactMinor: amountMinor
            }
          }, {
            session: sess
          });
