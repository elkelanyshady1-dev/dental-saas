...
const {
  activateContract: _activateContract,
  ContractActivationError
} = require("./contractActivation.service");
...
const IMMUTABLE_ACTIVE_FIELDS = new Set(["lockedPrice", "currency", "planVersionId", "planCode", "planVersionTag", "effectiveFrom", "trialDays"]);
function assertNoImmutableChange(updates) {
  const violations = Object.keys(updates).filter(k => IMMUTABLE_ACTIVE_FIELDS.has(k));
  if (violations.length > 0) {
    throw new ContractEngineError(`Cannot modify immutable fields on an active contract: ${violations.join(", ")}`, "IMMUTABLE_FIELD_VIOLATION");
  }
}
...
async function createContract(data, actorId, options = {}) {
  ...
  const {
    ...
    metadata = undefined,
    // optional Map or plain object for audit traceability
  ...
  // Phase 9 hardening — Section 4: CONTRACT_INVALID_STATE invariant.
  //
  // Every persisted contract MUST have a resolvable amount + currency.
  // For PAID contracts (lockedPrice > 0) the paymentProvider must also be
  // set; trial / provisioning contracts (lockedPrice === 0) are allowed
  // to defer the provider until activation. This guard catches caller
  // bugs (missing fields after a refactor) before they create a contract
  // with NULL financials that downstream invoice/billing readers cannot
  // make sense of.
  if (typeof lockedPrice !== "number" || lockedPrice < 0) {
    throw Object.assign(new Error("CONTRACT_INVALID_STATE: lockedPrice must be a non-negative number."), {
  ...
  if (!currency) {
    throw Object.assign(new Error("CONTRACT_INVALID_STATE: currency is required."), {
      code: "CONTRACT_INVALID_STATE",
      status: 500,
      lockedPrice,
      currency,
  ...
  if (lockedPrice > 0 && !paymentProvider) {
    throw Object.assign(new Error("CONTRACT_INVALID_STATE: paid contracts require paymentProvider."), {
      code: "CONTRACT_INVALID_STATE",
      status: 500,
      lockedPrice,
      currency,
  ...
  if (!initialStatus) {
    ...
    if (existingDraft) {
      throw new ContractEngineError(`Organization already has a draft contract (${existingDraft._id}). Activate or terminate it first.`, "DUPLICATE_DRAFT");
    }
  }
  ...
  if (initialStatus) {
    ...
    if (isPaidContract) {
      ...
      if (activeTrial) {
        ...
        if (existingPending) {
          throw new ContractEngineError(`Organization already has a pending_activation contract (${existingPending._id}). Cancel it first.`, "DUPLICATE_PENDING");
        }
  ...
  if (planVersionId) {
    ...
    // ─────────────────────────────────────────────────────────────────────
    if (pv.status === "draft") {
      throw new ContractEngineError(`PlanVersion ${planVersionId} is still in draft status and cannot be used for contracts. ` + `Publish the version first.`, "PLAN_VERSION_NOT_PUBLISHED");
    }
    if (pv.visibility === "internal") {
      throw new ContractEngineError(`PlanVersion ${planVersionId} has visibility="internal" and is not available for org contracts. ` + `Only public and sales-visibility plans may be attached to contracts.`, "PLAN_NOT_AVAILABLE_FOR_CONTRACTS");
    }
    if (pv.status === "deprecated" && pv.visibility !== "sales") {
      throw new ContractEngineError(`PlanVersion ${planVersionId} is deprecated and visibility="${pv.visibility}". ` + `Deprecated plans are only allowed for sales-managed contracts (visibility=sales). ` + `Use an active plan version or change the plan's visibility to "sales" first.`, "DEPRECATED_PUBLIC_PLAN_NOT_ALLOWED");
    }
  ...
  const contractData = {
    ...
    lockedPrice: accessType === "promo" ? 0 : lockedPrice,
    // hard-enforce 0 for promo
    currency: currency.toUpperCase(),
    effectiveFrom: resolvedEffectiveFrom,
    effectiveTo: effectiveTo || null,
    trialDays,
    trialStartDate: trialStartDate || (trialDays > 0 ? new Date() : null),
    trialEndDate: dataTrialEndDate || (trialDays > 0 ? (() => {
      const d = new Date();
  ...
  if (!initialStatus && resolvedEffectiveFrom) {
    ...
    const overlappingContract = await OrgContract().findOne({
      ...
      effectiveTo: {
        $ne: null,
        $gt: resolvedEffectiveFrom
      }
    }).select("_id contractStatus effectiveFrom effectiveTo").session(session || null);
    if (overlappingContract) {
      throw new ContractEngineError(`CONTRACT_TIMELINE_OVERLAP: New contract effectiveFrom (${resolvedEffectiveFrom.toISOString()}) ` + `falls within existing contract ${overlappingContract._id} ` + `(status: ${overlappingContract.contractStatus}, ` + `effectiveTo: ${overlappingContract.effectiveTo.toISOString()}). ` + `Supersede or terminate the existing contract first.`, "CONTRACT_TIMELINE_OVERLAP");
    }
  }
  const opts = session ? {
    session
  ...
  logger.info({
    ...
    accessType: contract.accessType,
    // ← MUST match what was passed in
  ...
  if (accessType && contract.accessType !== accessType) {
    throw new Error(`ACCESS_TYPE_PERSISTENCE_FAILED: expected="${accessType}" got="${contract.accessType}" ` + `contractId=${contract._id}`);
  }
  return contract;
}
...
async function replaceContract(sourceContractId, updates, actorId, options = {}) {
  ...
  if (source.contractStatus !== "active") {
    throw new ContractEngineError(`Only active contracts can be replaced (current status: ${source.contractStatus})`, "INVALID_SOURCE_STATUS");
  }
  ...
  if (existingDraft) {
    throw new ContractEngineError(`A pending draft replacement already exists (${existingDraft._id})`, "DUPLICATE_DRAFT");
  }
  const newContractData = {
    ...
    creditBalance: 0,
    // fresh contract starts with no credit
...
async function activateContract(contractId, invoiceId, actorId, options = {}) {
  try {
    return await _activateContract(contractId, invoiceId, {
      ...
      skipInvoiceCheck: options.skipInvoiceCheck,
      // ← v22.2: pass through for upgrade flow
