    ...
    try {
        ...
        for (const regionCode of activeRegionCodes) {
            try {
                ...
                logger.error({ regionCode, err: err.message }, "Mutation reconciliation failed for region");
            }
        }
    } catch (err) {
        logger.error({ err }, "Failed to fetch active regions for Mutation Reconciliation");
    }
}
...
async function processRegionMutationReconciliation(regionCode) {
    ...
    for (const mutation of staleMutations) {
        try {
            ...
            logger.error({ err, mutationId: mutation._id, regionCode }, "Failed to reconcile regional mutation");
        }
    }
}

async function reconcileOne(regionCode, mutation, mongooseConnection) {
    const { organizationId, type } = mutation;
    ...
    if (type === "CANCEL") {
        ...
        const transactions = await provider.listCustomerBalanceTransactions(
