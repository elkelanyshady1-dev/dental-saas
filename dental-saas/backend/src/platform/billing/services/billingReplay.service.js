...
const REPLAYABLE_TYPES = new Set(["payment.succeeded", "payment.failed", "invoice.refunded", "subscription.created", "subscription.canceled"]);
...
function buildCanonicalEvent(entry) {
  return {
    ...
    // canonical: "payment.succeeded" etc.
    externalId: entry.providerEventId,
    // canonical: provider's event ID
    isReplay: true,
    // signal to processor that this is a replay
 ...
 * }>}
 */
async function replayBillingEvent(ledgerEntryId, {
  replayedBy
} = {}) {
  // ── 1. Load ledger entry ─────────────────────────────────────────────────
  ...
  if (!REPLAYABLE_TYPES.has(entry.eventType)) {
    throw new Error(`[BillingReplay] Event type "${entry.eventType}" is not replayable. ` + `Replayable types: ${[...REPLAYABLE_TYPES].join(", ")}.`);
  }
  logger.info({
  ...
  let processResult;
  try {
    processResult = await handleCanonicalEvent(canonicalEvent, {
      correlationId: `replay:${entry._id}`
    });
  } catch (processorErr) {
    logger.error({
      err: processorErr,
    ...
    throw new Error(`[BillingReplay] Processor error during replay: ${processorErr.message}`);
  }
 ...
 */
async function listReplayableEvents({
  page = 1,
  limit = 50,
  provider,
  eventType
