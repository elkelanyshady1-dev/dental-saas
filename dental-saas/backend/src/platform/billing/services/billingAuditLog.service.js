 ...
 */
async function getOrgBillingHistory(organizationId, { page = 1, limit = 50, eventType, contractId } = {}) {
    ...
    const data = await BillingAuditLog.find(filter)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean();
...
module.exports = { logBillingEvent, getOrgBillingHistory };
