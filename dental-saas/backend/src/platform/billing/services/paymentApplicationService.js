...
const { roundCurrency, toMinorUnits, assertPrecision } = require("@utils/money");
...
async function applyPayment({
    ...
    actorId = null,
    requestId = null,
    idempotencyKey = null,   // v21.1: client-supplied dedup key (Idempotency-Key header)
    ...
    if (idempotencyKey) {
        ...
        if (existing) {
            ...
            logger.info(
                { idempotencyKey, paymentId: existing._id, invoiceId: existing.invoiceId, requestId },
    ...
    try {
        ...
        if (!PAYMENT_ACCEPTING_STATUSES.has(invoice.status)) {
            throw Object.assign(
                new Error(`Invoice in status "${invoice.status}" cannot receive payments. Only open/partial/overdue invoices accept payments.`),
        ...
        if (safeAmount > amountRemaining + 0.001) {  // 0.001 tolerance for borderline floats
            throw Object.assign(
                new Error(`Payment amount ${safeAmount} exceeds remaining balance ${amountRemaining.toFixed(2)} — PAYMENT_NOT_GREATER_THAN_INVOICE`),
                { statusCode: 422, code: "PAYMENT_EXCEEDS_BALANCE" }
            );
        }
        ...
        const [payment] = await PaymentAttempt.create([{
            ...
            amount: safeAmount,  // Section 8: always use the rounded amount
            ...
            attemptNumber: 1,
            requestId,
            metadata: { actorId, transactionRef }
        }], { session });
        ...
        logger.info({
            ...
            amount: safeAmount,  // Section 8: log the safe amount
            amountMinor: toMinorUnits(safeAmount, invoice.currency),  // Section 9
            newStatus,
            prevStatus,
            actorId,
            requestId
        ...
        setImmediate(async () => {
            try {
                ...
                logger.error({ err: e }, "[paymentApplicationService] Ledger write failed (non-fatal)");
            }
        });
        ...
        setImmediate(async () => {
            try {
                await emitBillingTimelineEvent({
                    ...
                    source: "user",
                    payload: { amount, method, transactionRef, amountPaid: invoice.amountPaid, totalAmount: invoice.totalAmount }
                });
            } catch (e) {
                logger.error({ err: e }, "[paymentApplicationService] Timeline emit failed (non-fatal)");
            }
        });
        ...
        if (newStatus === "paid") {
            setImmediate(() => {
                try {
                    ...
                    logger.error({ err: e }, "[paymentApplicationService] EventBus PLATFORM_INVOICE_PAID emit failed (non-fatal)");
                }
            });
        }
                ...
                try {
                    ...
                    if (contract.contractStatus === "pending_payment") {
                        ...
                        await activateContract(
                            String(contract._id),
                            String(invoice._id),   // invoice is now paid — INVOICE_NOT_PAID guard will pass
                            {
                        ...
                        logger.info(
                            { contractId: String(contract._id), invoiceId: String(invoice._id), actorId },
                        ...
                        logger.info(
                            { contractId: String(contract._id), invoiceId: String(invoice._id) },
                    ...
                    logger.error(
                        { err: e, contractId: invoice.contractId, invoiceId: invoice._id },
        ...
        if (ownsSession) {
            try { await session.abortTransaction(); } catch { /* ignore abort error */ }
        }
        throw err;
    } finally {
 ...
 * @returns {Promise<{invoice: object, payment: object}>}
 */
async function refundPayment({ paymentId, reason = "", actorId = null, requestId = null }) {
    if (!paymentId) throw Object.assign(new Error("paymentId is required"), { statusCode: 400 });
    ...
    try {
        ...
        if (payment.status === "refunded") {
            throw Object.assign(new Error("Payment has already been refunded"), { statusCode: 409, code: "ALREADY_REFUNDED" });
        }

        if (payment.status !== "captured") {
            throw Object.assign(
                new Error(`Payment in status "${payment.status}" cannot be refunded. Only captured payments can be refunded.`),
        ...
        payment.metadata = { ...payment.metadata, refundReason: reason, refundedBy: actorId, refundedAt: new Date() };
        await payment.save({ session });

        // Update invoice amountPaid
        const invoice = await PlatformInvoice.findById(payment.invoiceId).session(session);
        if (!invoice) {
            throw Object.assign(new Error("Associated invoice not found"), { statusCode: 404 });
        }
        ...
        setImmediate(async () => {
            try {
                await writeLedgerEntry({
                    eventType: "payment.refunded",   // v21.0: explicit event type
                    organizationId: invoice.organizationId,
                    contractId: invoice.contractId,
                    invoiceId: invoice._id,
                    paymentAttemptId: payment._id,    // v21.0: traceability
                    provider: payment.provider || "manual",
                    amount: payment.amount,
                    currency: invoice.currency,
                    source: "paymentApplication",     // v21.0: correct source
                    actorType: actorId ? "user" : "system",
                    metadata: {
                        refundReason: reason,
                        actorId,
                ...
                logger.error({ err: e }, "[paymentApplicationService] Refund ledger write failed (non-fatal)");
            }
        });
...
module.exports = { applyPayment, refundPayment };
