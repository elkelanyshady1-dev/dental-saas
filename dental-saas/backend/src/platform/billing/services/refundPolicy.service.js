...
const POLICY_DEFAULTS = {
  ...
  // decimal units (e.g. $1000)
  largeRefundRatioPct: 50,
  // 50% of contract lockedPrice
  maxRefundsPerOrg: 3,
...
async function validateRefundEligibility(contract, invoice, amount, organizationId) {
  ...
  if (new Date() > windowCutoff) {
    return _deny(`Refund window of ${policy.refundWindowDays} days has expired. Invoice was paid on ${paidAt.toISOString()}.`, policy);
  }
  ...
  if (recognitionStarted && !policy.allowAfterRecognition) {
    return _deny(`Revenue recognition has already begun (${recognizedAmount} recognized). ` + `Platform policy prohibits refunds after recognition starts.`, policy);
  }
  ...
  if (amount > policy.largeRefundThreshold) {
    ...
    logger.info({
      amount,
      threshold: policy.largeRefundThreshold,
      organizationId
  ...
  if (contractValue > 0 && amount / contractValue * 100 > policy.largeRefundRatioPct) {
    requiresApproval = true;
    logger.info({
      pct: (amount / contractValue * 100).toFixed(1),
      organizationId
