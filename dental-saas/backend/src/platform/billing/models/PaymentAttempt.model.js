...
const paymentAttemptSchema = new mongoose.Schema(
    ...
    {
        timestamps: { createdAt: true, updatedAt: false }, // append-style — no updates expected
        collection: "paymentattempts"
    }
);

// ─── Indexes ──────────────────────────────────────────────────────────────────
paymentAttemptSchema.index({ invoiceId: 1, createdAt: -1 });          // Per-invoice history
paymentAttemptSchema.index({ organizationId: 1, createdAt: -1 });     // Per-org history
paymentAttemptSchema.index({ providerPaymentId: 1 }, { sparse: true }); // Provider lookup
paymentAttemptSchema.index({ requestId: 1 }, { sparse: true });       // End-to-end tracing
paymentAttemptSchema.index({ status: 1, createdAt: -1 });              // Status-based scans
paymentAttemptSchema.index({ provider: 1, createdAt: -1 });           // Provider analytics
paymentAttemptSchema.index({ method: 1, createdAt: -1 });             // v21.0: method analytics
paymentAttemptSchema.index(                                           // v21.1: idempotency
    { idempotencyKey: 1 },
    { unique: true, sparse: true, name: "payment_idempotency_key" }
);
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, paymentAttemptSchema),
};
