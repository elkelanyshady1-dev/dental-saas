...
const pricingSnapshotSchema = new mongoose.Schema({
  ...
  snapshotType: {
    type: String,
    enum: ["computed", "backfilled"],
    default: "computed"
  },
  _backfilledAt: {
    type: Date,
    default: null
  } // set only when snapshotType = "backfilled"
}, {
  _id: false
});
...
orgContractSchema.index({
  "dunning.gracePeriodEndsAt": 1
});
orgContractSchema.index({
  "appliedCoupon.code": 1
});
orgContractSchema.index({
  salesOwnerId: 1
});
