...
const {
  computeLedgerHash
} = require("../utils/ledgerHash");

// ─── Supported event types ─────────────────────────────────────────────────────
const LEDGER_EVENT_TYPES = Object.freeze(["invoice.created", "invoice.voided",
// v22.0: invoice void action (amountPaid === 0)
"payment.succeeded", "payment.failed", "payment.partial",
// v21.0: partial payment applied
"payment.refunded",
// v21.0: explicit refund event
"invoice.refunded",
// legacy: kept for backward compat with existing entries
"subscription.created", "subscription.canceled", "contract.activated", "contract.suspended",
// v21.0: suspension event
"contract.voided",
// v21.0: void event
"contract.terminated",
// v22.0: explicit termination event
// Sprint 1: renewal ledger event — written by contractRenewal.service on success
"renewal.completed"]);

// ─── Source systems that may write ledger entries ──────────────────────────────
const LEDGER_SOURCES = Object.freeze(["canonicalEventProcessor", "contractActivation", "invoiceEngine", "contractRenewal",
// Sprint 1: renewal ledger events
"paymentApplication",
// v21.0: manual payment application service
"refundEngine",
// v22.0: refundProcessor.service
...
billingLedgerSchema.pre(["updateOne", "findOneAndUpdate", "replaceOne", "updateMany"], function () {
  throw new Error("[BillingLedger] Immutability violation: BillingLedger entries are append-only and cannot be updated. " + "Create a correcting entry (type: manualAdjustment) instead.");
});
billingLedgerSchema.pre(["deleteOne", "findOneAndDelete", "deleteMany"], function () {
  throw new Error("[BillingLedger] Immutability violation: BillingLedger entries are append-only and cannot be deleted. " + "The ledger is a permanent financial record.");
});
...
billingLedgerSchema.index({
  ...
  sparse: true,
  name: "ledger_hash_lookup"
});
billingLedgerSchema.index({
  previousHash: 1
}, {
  sparse: true,
  name: "ledger_prev_hash_lookup"
});
