...
const versionPricingRegionSchema = new mongoose.Schema({
    regionCode: { type: String, required: true, uppercase: true },
...
const pricingV3OverrideSchema = new mongoose.Schema({
    ...
    stripePriceId_monthly: { type: String, default: "" },
    stripePriceId_yearly: { type: String, default: "" },
    paymobPriceId_monthly: { type: String, default: "" },
    paymobPriceId_yearly: { type: String, default: "" }
}, { _id: false });
...
const pricingV3RegionSchema = new mongoose.Schema({
    ...
    excludedCountries: [{ type: String, uppercase: true }],
...
const versionQuotasSchema = new mongoose.Schema({
    storageMB: { type: Number, default: 0 },  // Total allowed storage (MB)
    imagesMB:  { type: Number, default: 0 },  // Optional sub-quota for images (MB)
}, { _id: false });
...
planVersionSchema.index({ templateId: 1, status: 1 });
planVersionSchema.index({ templateId: 1, versionTag: 1 }, { unique: true });
planVersionSchema.index({ templateCode: 1, status: 1 });
planVersionSchema.index({ "pricing.regions.countries": 1 });
planVersionSchema.index({ activatedAt: -1 });
...
planVersionSchema.pre("save", async function () {
    ...
    if (this.status === "deprecated" && this.visibility === "public") {
        throw new Error(
            ...
            `A deprecated PlanVersion cannot have visibility="public". ` +
            `Set visibility to "sales" or "internal" before deprecating.`
        );
    }
    ...
    if (!this.isNew && this.status === "active") {
        ...
        const immutableFields = [
            "limits", "modules", "pricing", "pricingV3", "quotas", "inflationPolicy", "trialDays", "templateCode"
        ];
        for (const field of immutableFields) {
            if (this.isModified(field)) {
                throw new Error(
                    `[PlanVersion] Cannot mutate field "${field}" on an active version (${this._id}). ` +
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, planVersionSchema),
};
