...
const platformInvoiceSchema = new mongoose.Schema({
  ...
  invoiceType: {
    ...
    "initial",
    // First invoice for a new subscription
    "scheduled",
    // Pre-paid invoice for pending_activation contract (activates after trial)
    "addon",
    // Add-on purchase
    "credit_note",
  ...
  // v21.0: when invoice was issued to org
  voidedAt: {
    type: Date,
    default: null
  },
  ...
  // Provider invoice ID if applicable
...
platformInvoiceSchema.pre("save", async function () {
  if (this.lineItems && this.lineItems.length > 0) {
    ...
    if (Math.abs(rounded - this.totalAmount) > 0.02) {
      throw new Error(`[PlatformInvoice] Line item integrity violation: ` + `sum(lineItems.total)=${rounded} !== totalAmount=${this.totalAmount} ` + `(invoiceId=${this._id}, invoiceNumber=${this.invoiceNumber || "DRAFT"})`);
    }
  }
});
...
platformInvoiceSchema.pre("save", async function () {
  if (!this.contractId) {
    throw new Error(`PlatformInvoice pre-save guard: contractId is required (invoiceId=${this._id})`);
  }
});
