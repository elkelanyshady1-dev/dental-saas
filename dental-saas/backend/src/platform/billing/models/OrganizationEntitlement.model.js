...
organizationEntitlementSchema.index({ organizationId: 1, createdAt: -1 });

// Per-contract lookup — allows cleaning up stale entitlements when a contract
// is superseded or terminated
organizationEntitlementSchema.index({ contractId: 1 });

// Per-planVersion count — useful for impact analysis before deprecating a version
organizationEntitlementSchema.index({ planVersionId: 1, effectiveUntil: 1 });
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, organizationEntitlementSchema),
};
