...
const billingAuditLogSchema = new mongoose.Schema({
  ...
  eventType: {
    ...
    enum: [
    // Contract lifecycle
    "CONTRACT_CREATED", "CONTRACT_ACTIVATED", "CONTRACT_RENEWED", "CONTRACT_SUPERSEDED", "CONTRACT_TERMINATED", "CONTRACT_CANCELED", "CONTRACT_EXPIRED", "AUTO_RENEW_UPDATED",
    // Payment
    "PAYMENT_SUCCEEDED", "PAYMENT_FAILED", "PAYMENT_REFUNDED",
    // Refund lifecycle
    "REFUND_REQUESTED", "REFUND_APPROVED", "REFUND_REJECTED", "REFUND_PROCESSED", "REFUND_FAILED",
    // Dunning cycle
    ...
    "ENTITLEMENT_CREATED_FROM_PLAN", "ENTITLEMENT_OVERRIDE_APPLIED",
    // Unified Capability Resolver (Sprint 3)
    "CAPABILITY_STATE_CHANGED",
    // Plan catalog lifecycle
    "PLAN_VERSION_PUBLISHED", "PLAN_VERSION_DEPRECATED"]
  },
  // ── State Snapshot ────────────────────────────────────────────────────
...
// ─── Indexes ──────────────────────────────────────────────────────────────────
billingAuditLogSchema.index({
  organizationId: 1,
  createdAt: -1
}); // Primary: per-org history
billingAuditLogSchema.index({
  contractId: 1,
  createdAt: -1
}); // Per-contract timeline
billingAuditLogSchema.index({
  invoiceId: 1
}); // Per-invoice events
billingAuditLogSchema.index({
