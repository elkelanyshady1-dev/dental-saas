...
exports.purchaseAddOn = async (req, res) => {
    try {
        ...
        const { addOnCode, interval = "monthly" } = req.body;

        const [org, addOn] = await Promise.all([
            Organization.findById(organizationId),
            addOnService.getAddOnByCode(addOnCode)
        ]);

        if (!org || !addOn) {
            return res.status(404).json({ message: "Organization or Add-On not found" });
        }

        // Resolve regional price
        const regionalPricing = resolveRegionalPrice(addOn, org.billingCountry);
        if (!regionalPricing) {
            return res.status(400).json({ message: "Add-On not available in your region" });
        }
        ...
        if (existing) {
            return res.status(400).json({ message: "Add-On already active for this organization" });
        }
        ...
        await createAuditRecord({
            ...
            details: { addOnCode, price, currency: regionalPricing.currency },
            success: true
        });
        ...
        res.status(500).json({ message: "Failed to purchase add-on", error: err.message });
    }
};
...
exports.cancelAddOn = async (req, res) => {
    try {
        ...
        res.json({ message: "Add-on subscription cancelled" });
    } catch (err) {
