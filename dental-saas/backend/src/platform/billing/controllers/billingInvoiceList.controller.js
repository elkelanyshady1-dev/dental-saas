...
const { streamInvoicesCsv } = require("../services/financeExport.service");
...
exports.list = async (req, res) => {
    try {
        ...
        const [invoices, total] = await Promise.all([
            PlatformInvoice
                .find(filter)
                .select("invoiceNumber organizationId totalAmount currency status paymentStatus createdAt paidAt dueDate contractId invoiceType")
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .lean(),
        ...
        try {
            ...
            if (orgIds.length > 0) {
                ...
                data = invoices.map(i => ({ ...i, orgName: orgMap[String(i.organizationId)] || null }));
            }
        } catch (orgErr) {
            logger.error({ orgErr }, "[billingInvoiceList] Org enrichment failed");
        }
        ...
        logger.error({ err, requestId: req.requestId }, "[billingInvoiceList] list failed");
...
exports.exportCsv = async (req, res) => {
    try {
        ...
        logger.error({ err, requestId: req.requestId }, "[billingInvoiceList] exportCsv failed");
        if (!res.headersSent) {
...
exports.pdf = async (req, res) => {
    try {
        ...
        if (!mongoose.isValidObjectId(invoiceId)) {
            return res.status(400).json({ success: false, error: "Invalid invoiceId", requestId: req.requestId });
        }

        const invoice = await PlatformInvoice.findById(invoiceId).lean();
        if (!invoice) {
            return res.status(404).json({ success: false, error: "Invoice not found", requestId: req.requestId });
        }
        ...
        logger.error({ err, requestId: req.requestId }, "[billingInvoiceList] pdf failed");
        return res.status(500).json({ success: false, error: "PDF generation failed", requestId: req.requestId });
    }
};
