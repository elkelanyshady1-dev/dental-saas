...
exports.requestRefund = async (req, res) => {
    try {
        ...
        const { invoiceId, amount, reasonCode, idempotencyKey } = req.body;
        ...
        return res.status(statusCode).json({
            ...
            message: result.idempotent
                ? "Refund request already exists (idempotent)"
                : result.requiresApproval
                    ? "Refund request created — manual approval required"
                    : "Refund request created — eligible for processing"
        });
    } catch (err) {
        logger.error({ err }, "[RefundController] requestRefund failed");
        if (err.status) {
...
exports.approveRefund = async (req, res) => {
    try {
        ...
        logger.error({ err }, "[RefundController] approveRefund failed");
        if (err.status) {
            return res.status(err.status).json({ success: false, error: err.message, code: err.code });
        }
        return res.status(500).json({ success: false, error: "Internal server error" });
...
exports.rejectRefund = async (req, res) => {
    try {
        ...
        logger.error({ err }, "[RefundController] rejectRefund failed");
        if (err.status) {
            return res.status(err.status).json({ success: false, error: err.message, code: err.code });
        }
        return res.status(500).json({ success: false, error: "Internal server error" });
...
exports.processRefund = async (req, res) => {
    try {
        ...
        logger.error({ err }, "[RefundController] processRefund failed");
        if (err.status) {
            return res.status(err.status).json({ success: false, error: err.message, code: err.code });
        }
        return res.status(500).json({ success: false, error: "Internal server error" });
...
exports.getRefund = async (req, res) => {
    try {
        const record = await RefundExecutionRecord.findById(req.params.id)
            .populate("invoiceId", "invoiceNumber totalAmount currency status")
            .lean();
        ...
        logger.error({ err }, "[RefundController] getRefund failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
...
exports.listRefundsByInvoice = async (req, res) => {
    try {
        ...
        const records = await RefundExecutionRecord.find({ invoiceId: id })
            .sort({ createdAt: -1 })
            .lean();

        return res.json({ success: true, data: records, count: records.length });
    } catch (err) {
        logger.error({ err }, "[RefundController] listRefundsByInvoice failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
...
exports.listAllRefunds = async (req, res) => {
    try {
        ...
        const VALID_STATUSES = [
            "refund_requested", "refund_under_review", "refund_approved",
            "refund_processing", "refund_completed", "refund_rejected", "refund_failed"
        ];
        ...
        if (!req.query.status && !req.query.all) {
            filter.status = { $in: ["refund_requested", "refund_under_review", "refund_approved", "refund_processing"] };
        }
        ...
        const [records, total] = await Promise.all([
            RefundExecutionRecord
                ...
                .limit(limit)
                .lean(),
            RefundExecutionRecord.countDocuments(filter)
        ]);
        ...
        try {
            ...
            if (orgIds.length > 0) {
                ...
                data = records.map(r => ({ ...r, orgName: orgMap[String(r.organizationId)] || null }));
            }
        } catch (orgErr) {
            logger.warn({ orgErr }, "[RefundController] Org name enrichment failed");
        }

        // ── Pending count for sidebar badge ───────────────────────────────
        const pendingCount = await RefundExecutionRecord.countDocuments({
            status: { $in: ["refund_requested", "refund_under_review", "refund_approved"] }
        });
        ...
        logger.error({ err }, "[RefundController] listAllRefunds failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
