...
const IMMUTABLE_ACTIVE_FIELDS = ["lockedPrice", "currency", "planVersionId", "planCode", "planVersionTag", "effectiveFrom", "trialDays"];
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        logger.error({
          err: e
        }, "[ContractController] Audit log failed (non-fatal)");
      }
    });
    ...
    logger.error({
      err
    }, "[ContractController] createContract failed");
    if (err.code === 11000) {
      return res.status(409).json({
        success: false,
...
exports.createSalesContract = async (req, res) => {
  try {
    ...
    if (!org || org.isArchived) {
      return res.status(404).json({
        success: false,
        error: "Organization not found or archived"
      });
    }
    ...
    if (existingPending) {
      return res.status(409).json({
        success: false,
        error: "Organization already has a pending_activation contract. " + "It must activate or be canceled before creating a new sales contract.",
    ...
    const contract = new OrgContract({
      ...
      lockedPrice: pricing.finalPrice,
      // from engine — never raw customPrice
      currency: pricing.currency,
      // from engine — validated
      billingInterval: pricing.billingInterval,
      // from engine — locked cadence
      providerPriceId: pricing.providerPriceId,
      // from engine — null for manual
      pricingSnapshot: pricing.snapshot,
      // from engine — audit trail
      autoRenew,
      gracePeriodDays,
      salesManaged: true,
    ...
    const message = activeTrial ? `Sales contract scheduled. Will activate on ${effectiveFrom.toISOString()} when trial ends.` : "Sales contract created in draft. Generate invoice and record payment to activate.";
    return res.status(201).json({
      success: true,
      data: contract,
      scheduledForTrialEnd: !!activeTrial,
    ...
    logger.error({
      err
    }, "[ContractController] createSalesContract failed");
    if (err.code === 11000) {
      return res.status(409).json({
        success: false,
...
exports.updateContractStatus = async (req, res) => {
  try {
    ...
    if (!contractStatus) {
      return res.status(400).json({
        success: false,
        error: "contractStatus is required"
      });
    }
    ...
    logger.error({
      err
    }, "[ContractController] updateContractStatus failed");
    return res.status(500).json({
      success: false,
      error: "Internal server error"
    });
...
exports.replaceContract = async (req, res) => {
  try {
    ...
    if (!sourceContract) {
      return res.status(404).json({
        success: false,
        error: "Source contract not found"
      });
    }
    ...
    const {
      ...
      trialDays = 0,
      // Replacements don't restart trials by default
    ...
    logger.error({
      err
    }, "[ContractController] replaceContract failed");
    return res.status(500).json({
      success: false,
      error: "Internal server error"
    });
...
exports.uploadContractDocument = async (req, res) => {
  try {
    ...
    logger.error({
      err
    }, "[ContractController] uploadContractDocument failed");
    return res.status(500).json({
      success: false,
      error: "Internal server error"
    });
...
exports.setAutoRenew = async (req, res) => {
  try {
    ...
    logger.error({
      err
    }, "[ContractController] setAutoRenew failed");
    return res.status(500).json({
      success: false,
      error: "Internal server error"
    });
...
exports.cancelContract = async (req, res) => {
  try {
    ...
    logger.error({
      err,
      stack: err.stack
    }, "[ContractController] cancelContract failed");
    return res.status(500).json({
      success: false,
      error: "Internal server error",
...
exports.getContractChain = async (req, res) => {
  try {
    ...
    logger.error({
      err
    }, "[ContractController] getContractChain failed");
    return res.status(500).json({
      success: false,
      error: "Internal server error"
    });
