...
exports.getMetrics = async (req, res) => {
    try {
        ...
        const churnRate = activeAtStartOfMonth > 0
            ? Math.round((canceledThisMonth / activeAtStartOfMonth) * 10000) / 100 // percentage to 2dp
            : 0;
        ...
        const [revenueAgg] = await RevenueSchedule.aggregate([
            { $match: { status: { $in: ["active", "fully_recognized"] } } },
            {
                $group: {
                    _id: null,
                    recognizedRevenue: { $sum: "$recognizedAmount" },
                    deferredRevenue: { $sum: "$deferredAmount" }
                }
            }
        ]);
        ...
        try {
            ...
            logger.warn({ dashErr }, "[billingRevenue] renewalDashboard metrics unavailable — MRR/ARR defaulted to 0");
        }
        ...
        logger.error({ err, requestId: req.requestId }, "[billingRevenue] getMetrics failed");
