...
exports.generateInvoice = async (req, res) => {
    try {
        ...
        if (billingInterval === "yearly") cycleEnd.setFullYear(cycleEnd.getFullYear() + 1);
        else if (billingInterval === "biennial") cycleEnd.setFullYear(cycleEnd.getFullYear() + 2);
        ...
        const existing = await PlatformInvoice.findOne({ idempotencyKey });
        if (existing) {
        ...
        const effectiveIntervalMap = { monthly: "monthly", yearly: "yearly", biennial: "biennial" };
        ...
        const invoice = await PlatformInvoice.create({
            ...
            invoiceType: contract.contractStatus === "draft"
                ? "initial"
                : contract.contractStatus === "pending_activation"
                    ? "scheduled"    // pre-paid, activates after trial — not a live subscription yet
                    : "subscription",
        ...
        logger.info(
            { invoiceId: invoice._id, contractId: id, totalAmount, currency: contract.currency, actorId },
        ...
        setImmediate(async () => {
            try {
                ...
                logger.error({ err: e }, "[InvoiceController] Audit log failed (non-fatal)");
            }
        });
        ...
        logger.error({ err }, "[InvoiceController] generateInvoice failed");
...
exports.recordPayment = async (req, res) => {
    try {
        ...
        if (MANUAL_PAYMENT_METHODS.has(paymentMethod)) {
            ...
            logger.info(
                { invoiceId: id, paymentMethod, actorId },
            ...
            if (invoice.contractId) {
                ...
                if (contract && contract.contractStatus === "draft") {
                    try {
                        activationResult = await activateContract(
                            invoice.contractId,
                            invoice._id,
                            { activatedBy: actorId }
                        );
                        logger.info(
                            { contractId: invoice.contractId, invoiceId: id },
        ...
        logger.error({ err }, "[InvoiceController] recordPayment failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
...
exports.getInvoice = async (req, res) => {
    try {
        ...
        logger.error({ err }, "[InvoiceController] getInvoice failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
...
exports.listContractInvoices = async (req, res) => {
    try {
        ...
        const [invoices, total] = await Promise.all([
            PlatformInvoice.find({ contractId: id })
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(clampedLimit)
                .lean(),
            PlatformInvoice.countDocuments({ contractId: id })
        ]);
        ...
        logger.error({ err }, "[InvoiceController] listContractInvoices failed");
        return res.status(500).json({ success: false, error: "Internal server error" });
    }
};
