...
exports.listPlanTemplates = async (req, res) => {
    try {
        ...
        const templates = await PlanTemplate.find(filter)
            .sort({ createdAt: -1 })
            .lean();
        ...
        return res.json({ success: true, templates: enriched, total: enriched.length });
    } catch (err) {
        logger.error({ err }, "[PlanTemplateController] listPlanTemplates failed");
        throw err;
    }
};
...
exports.getPlanTemplateById = async (req, res) => {
    try {
        ...
        const versions = await PlanVersion.find({ templateId: template._id })
            .sort({ createdAt: -1 })
            .lean();

        return res.json({ success: true, data: { ...template, versions } });
    } catch (err) {
        logger.error({ err }, "[PlanTemplateController] getPlanTemplateById failed");
        throw err;
    }
};
...
exports.createPlanTemplate = async (req, res) => {
    try {
        const {
            name, code, description, limits, modules, pricing,
        ...
        const existing = await PlanTemplate.findOne({ code: code.toLowerCase().trim() }).lean();
        if (existing) {
            return res.status(409).json({ success: false, message: `A template with code '${code}' already exists.` });
        }
        ...
        const template = await PlanTemplate.create({
            ...
            limits: limits || { maxUsers: 5, maxBranches: 1 },
            modules: modules || {},
            pricing: pricing || { baseCurrency: "USD", regions: [] },
            inflationPolicy: inflationPolicy || { defaultPercent: 0, applyAfterYears: 1 },
            trialDays: trialDays !== undefined ? trialDays : 14,
            visibility: visibility || { isPublic: true, hiddenCountries: [] },
            status: "draft",
            createdBy: req.platformUser._id,
        });

        logger.info(
            { templateId: template._id, code: template.code, actorId: req.platformUser._id },
            "[PlanTemplateController] Template created"
        );

        return res.status(201).json({ success: true, data: template, message: "PlanTemplate created" });
    } catch (err) {
        logger.error({ err }, "[PlanTemplateController] createPlanTemplate failed");
        if (err.code === 11000) {
            return res.status(409).json({ success: false, message: "Template code must be unique." });
        }
...
exports.updatePlanTemplate = async (req, res) => {
    try {
        ...
        if (expectedVersion === undefined) {
            return res.status(400).json({ success: false, message: "expectedVersion is required for OAV" });
        }
        ...
        if (template.status === "archived") {
            return res.status(400).json({ success: false, message: "Archived templates cannot be modified." });
        }
        ...
        const result = await PlanTemplate.findOneAndUpdate(
            ...
            { $set: { ...updates, lastModifiedBy: req.platformUser._id } },
            { new: true }
        );
        ...
        logger.info(
            { templateId: id, actorId: req.platformUser._id },
            "[PlanTemplateController] Template updated"
        );

        return res.json({ success: true, data: result, message: "PlanTemplate updated" });
    } catch (err) {
        logger.error({ err }, "[PlanTemplateController] updatePlanTemplate failed");
        throw err;
    }
};
