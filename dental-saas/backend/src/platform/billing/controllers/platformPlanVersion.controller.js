...
} = require("../services/planRevenueImpact.service");
const {
  logBillingEvent
} = require("../services/billingAuditLog.service");
const {
  validatePricingV3
} = require("../validators/pricingV3.validator");
...
} = require("../services/planProjection.service");
...
exports.listPlanVersions = async (req, res) => {
  try {
    ...
    logger.error({
      err
    }, "[PlanVersionController] listPlanVersions failed");
...
exports.getPlanVersionById = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      data: projectPlanVersion(version)
    });
  } catch (err) {
    logger.error({
      err
    }, "[PlanVersionController] getPlanVersionById failed");
    return res.status(500).json({
      success: false,
      message: err.message
    });
...
exports.createPlanVersion = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      data: version,
      message: "Draft PlanVersion created"
    });
  } catch (err) {
    logger.error({
      err
    }, "[PlanVersionController] createPlanVersion failed");
    if (err.code === 11000) {
...
exports.updatePlanVersion = async (req, res) => {
  try {
    ...
    if (version.status === "active") {
      ...
      if (restrictedFields.length > 0) {
        return res.status(400).json({
          ...
          message: `Active versions only allow updating: [${ACTIVE_ALLOWED_FIELDS.join(", ")}]. ` + `Restricted fields: [${restrictedFields.join(", ")}]. Create a new draft version to modify these.`,
          restrictedFields,
    ...
    const allowed = version.status === "active" ? ["visibility"] : ["label", "limits", "modules", "pricing", "pricingV3", "quotas", "inflationPolicy", "trialDays", "visibility", "changeNotes", "versionTag"];
    ...
    if (version.status === "active") {
      ...
      logBillingEvent({
        ...
        logger.warn({
          auditErr,
          versionId: id
        }, "[PlanVersionController] Visibility audit write failed (non-fatal)");
      });
      return res.json({
        success: true,
        data: version,
    ...
    logger.error({
      err
    }, "[PlanVersionController] updatePlanVersion failed");
    return res.status(500).json({
      success: false,
      message: err.message
    });
  ...
  try {
    ...
    if (regions.length === 0) {
      ...
      return res.status(400).json({
        ...
        message: "Cannot publish a PlanVersion with no pricing regions. " + "Add at least one region with monthly and yearly prices."
      });
    }
    const pricingErrors = [];
    ...
    logBillingEvent({
      ...
      logger.warn({
        auditErr,
        versionId: id
      }, "[PlanVersionController] Audit event write failed (non-fatal)");
    });
    return res.json({
      success: true,
      status: "published",
    ...
    logger.error({
      err
    }, "[PlanVersionController] publishPlanVersion failed");
    return res.status(500).json({
      success: false,
      message: err.message
    });
...
exports.deprecatePlanVersion = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      data: version,
      message: "PlanVersion deprecated"
    });
  } catch (err) {
    logger.error({
      err
    }, "[PlanVersionController] deprecatePlanVersion failed");
    return res.status(500).json({
      success: false,
      message: err.message
    });
...
exports.getPlanRevenueImpactController = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({
        success: false,
        message: "Invalid plan version ID"
      });
    }
    ...
    logger.error({
      err
    }, "[PlanVersionController] getPlanRevenueImpactController failed");
    return res.status(500).json({
      success: false,
      message: err.message
    });
...
exports.duplicatePlanVersion = async (req, res) => {
  try {
    ...
    if (!source) {
      return res.status(404).json({
        success: false,
        message: "Source PlanVersion not found."
      });
    }
    const duplicateData = {
    ...
    logger.error({
      err
    }, "[PlanVersionController] duplicatePlanVersion failed");
    if (err.code === 11000) {
      return res.status(409).json({
        success: false,
        message: "A version with this tag already exists for this template."
