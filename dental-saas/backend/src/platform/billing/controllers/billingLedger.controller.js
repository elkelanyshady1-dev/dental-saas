...
const { LEDGER_EVENT_TYPES } = require("../models/BillingLedger.model");
...
const { streamLedgerCsv } = require("../services/financeExport.service");
...
exports.list = async (req, res) => {
    try {
        ...
        const [entries, total] = await Promise.all([
            BillingLedger
                .find(filter)
                .select("createdAt eventType amount amountMinor currency organizationId contractId invoiceId provider providerEventId source actorType metadata")
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .lean(),
        ...
        try {
            ...
            const txns = await LedgerTransaction
                .find({ billingLedgerRef: { $in: entryIds } })
                .select("billingLedgerRef referenceType referenceId referenceLabel")
                .lean();
            ...
            logger.warn({ enrichErr }, "[billingLedger] Transaction enrichment failed — returning raw entries");
        }
        ...
        try {
            ...
            logger.warn({ orgErr }, "[billingLedger] Org name enrichment failed — continuing without");
        }
        ...
        return res.json({
            ...
            meta: { eventTypes: LEDGER_EVENT_TYPES },
            requestId: req.requestId
        });
    } catch (err) {
        logger.error({ err, requestId: req.requestId }, "[billingLedger] list failed");
...
exports.getTransaction = async (req, res) => {
    try {
        ...
        if (!mongoose.isValidObjectId(id)) {
            return res.status(400).json({ success: false, error: "Invalid transaction id", requestId: req.requestId });
        }

        const txn = await LedgerTransaction.findById(id).lean();
        if (!txn) {
            return res.status(404).json({ success: false, error: "Transaction not found", requestId: req.requestId });
        }
        ...
        if (txn.billingLedgerRef) {
            try {
                originEvent = await BillingLedger
                    .findById(txn.billingLedgerRef)
                    .select("eventType amount currency provider actorType source createdAt organizationId")
                    .lean();
            } catch (_) { /* non-fatal */ }
        }
        ...
        logger.error({ err, requestId: req.requestId }, "[billingLedger] getTransaction failed");
        return res.status(500).json({ success: false, error: "Internal server error", requestId: req.requestId });
    }
};
...
exports.exportCsv = async (req, res) => {
    try {
        ...
        logger.error({ err, requestId: req.requestId }, "[billingLedger] exportCsv failed");
        if (!res.headersSent) {
