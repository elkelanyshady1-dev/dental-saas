...
async function seedFeatureRegistry() {
    ...
    for (const [moduleKey, def] of entries) {
        ...
        // ── Seed Module ─────────────────────────────────────────────────
        const existingModule = await ModuleDefinition.findOne({ key: moduleKey });
        if (!existingModule) {
            ...
            modulesCreated++;
            logger.info({ key: moduleKey }, "[FeatureRegistrySeeder] Module created");
        }
        ...
        if (def.features) {
            for (const [featureKey, featureDef] of Object.entries(def.features)) {
                ...
                const existingFeature = await FeatureDefinition.findOne({ key: fullKey });
    ...
    if (modulesCreated > 0 || featuresCreated > 0) {
        logger.info(
            { modulesCreated, featuresCreated },
    ...
    return { modulesCreated, featuresCreated };
}

module.exports = { seedFeatureRegistry };
