...
class SalesMetricsService {
    ...
    async getMetricsBySalesOwner(salesOwnerId) {
        ...
        const contracts = await OrgContract.find({ salesOwnerId }).lean();
        ...
        const churnedClients = contracts.filter(
            c => c.contractStatus === "terminated" || c.contractStatus === "expired"
        ).length;

        // ── Revenue Calculations (Minor Units) ─────────────────────────────────
        const totalRevenueMinor = paidInvoices.reduce(
            (sum, inv) => sum + (inv.totalAmountMinor || 0), 0
        );

        // MRR (last 30 days approximation)
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const monthlyRevenueMinor = paidInvoices
            .filter(inv => inv.paidAt >= thirtyDaysAgo)
            .reduce((sum, inv) => sum + (inv.totalAmountMinor || 0), 0);

        const avgDealSizeMinor = totalClientsAcquired > 0
            ? Math.floor(totalRevenueMinor / totalClientsAcquired)
            : 0;

        // ── Trial-to-Paid Conversion ───────────────────────────────────────────
        // Sprint 5: trial detection from OrgContract.trialDays > 0 or org.status
        const trials = contracts.filter(c => (c.trialDays || 0) > 0).length
            + orgs.filter(o => o.status === "trial" || o.status === "expired").length;
