...
class PlanAggregateService {
     ...
     * ═══════════════════════════════════════════════════════════════
     */
    async assignPlan({ organizationId, newPlanId, expectedVersion, actorId }) {
        const session = await Organization.startSession();
        session.startTransaction();

        try {
            const [org, newPlan] = await Promise.all([
                Organization.findById(organizationId).session(session),
                Plan.findById(newPlanId).session(session)
            ]);
            ...
            await createAuditRecord({
                ...
                details: { previousPlanId, newPlanId },
                success: true
            }, { session });

            // Ensure newly enabled features are seeded based on the new plan
            await featureService.applyPlanFeatures(org, { session });
