...
function transformPlanForMarketing(plan, detectedCountry = "US") {
  ...
  const moduleLabels = {
    ...
    orthodonticsAdv: "Advanced Orthodontics Module",
    // matches PlanVersion schema
...
exports.getPublicPricing = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Unable to fetch pricing plans at this time."
    });
  }
};
...
exports.getPricingMatrix = async (req, res) => {
  try {
    ...
    const featureKeys = [{
      ...
      path: "limits.maxUsers"
    }, {
      key: "maxBranches",
      label: "Maximum Branches",
      path: "limits.maxBranches"
    }, {
      key: "inventory",
    ...
    res.status(500).json({
      message: "Unable to generate pricing matrix."
    });
  }
};
...
exports.getPublicAddOns = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Unable to fetch add-ons at this time."
    });
  }
};
module.exports = {
