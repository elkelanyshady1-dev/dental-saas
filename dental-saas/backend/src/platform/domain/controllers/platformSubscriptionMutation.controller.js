...
exports.changePlan = async (req, res) => {
    try {
        ...
        const { newPlanId, expectedVersion } = req.body;

        if (!newPlanId) {
            return res.status(400).json({ success: false, error: "newPlanId is required" });
        }
        if (expectedVersion === undefined) {
            return res.status(400).json({ success: false, error: "expectedVersion is required for OAV" });
        }
        ...
        return res.status(status).json({
            success: false,
            error: { code: "MUTATION_ERROR", message: error.message }
        });
    }
};
...
exports.addAddon = async (req, res) => {
    try {
        ...
        const region = addon.pricing.regions.find(r => r.countries.includes(org.billingCountry)) ||
            addon.pricing.regions.find(r => r.regionCode === "GLOBAL") ||
            addon.pricing.regions[0];
        ...
        const orgAddOn = await orgAddOnAggregateService.createOrgAddOn({
            ...
            price: region.monthly, // Assuming monthly for platform manual addition
            interval: "monthly",
            autoRenew: true
        });
        ...
        await createAuditRecord({
            ...
            details: { addOnId, code: addon.code },
            success: true
        });
...
exports.removeAddon = async (req, res) => {
    try {
        const { orgId } = req.params;
        const { orgAddOnId } = req.body;

        if (!orgAddOnId) {
            return res.status(400).json({ success: false, error: "orgAddOnId is required" });
        }
