...
const { getProviderForOrg, getProvider } = require("../../billing/providers/paymentProviderFactory");
...
exports.toggleAutoRenew = async (req, res) => {
    try {
        ...
        const contract = await OrgContract.findOne({ organizationId: id, contractStatus: "active" });
        ...
        await AuditLog.create({
            ...
            regionCode: "GLOBAL",
            metadata: { previousAutoRenew, newAutoRenew: autoRenew },
            success: true
        });

        res.json({ message: `Auto-renew set to ${autoRenew}`, autoRenew });
    } catch (err) {
        res.status(500).json({ message: "Failed to toggle auto-renew", error: err.message });
    }
};
...
exports.recordManualPayment = async (req, res) => {
    try {
        const { id } = req.params;
        const { invoiceId, paymentMethod, reference, notes, amountMinor } = req.body;

        if (!["CASH", "BANK_TRANSFER", "MANUAL"].includes(paymentMethod)) {
            return res.status(400).json({ message: "Invalid manual payment method" });
        }

        // Sprint 6: PlatformInvoice — no BillingInvoice
        const [org, invoice] = await Promise.all([
            Organization.findById(id),
            PlatformInvoice.findById(invoiceId)
        ]);

        if (!org || !invoice) return res.status(404).json({ message: "Org or Invoice not found" });
        ...
        if (invoice.contractId) {
            ...
            if (contract && contract.contractStatus !== "active") {
                try {
                    const { activateContract } = require("../../platform/billing/services/contractActivation.service");
                    ...
                    await AuditLog.create({
                        ...
                        entityId: contract._id,
                        regionCode: "GLOBAL",
                        metadata: { invoiceId, contractStatus: "active" },
                        success: true
                    });
                } catch (activationErr) {
                    ...
                    logger.error(
                        { err: activationErr, contractId: invoice.contractId },
        ...
        await AuditLog.create({
            ...
            entityId: invoiceId,
            regionCode: "GLOBAL",
            metadata: { amountMinor, paymentMethod, reference },
            success: true
        });

        res.json({ message: "Manual payment recorded successfully", invoice });
    } catch (err) {
        res.status(500).json({ message: "Failed to record manual payment", error: err.message });
    }
};
...
exports.generatePaymentLink = async (req, res) => {
    try {
        const { id } = req.params;
        const { planId, interval, currency, autoRenew } = req.body;
        ...
        await AuditLog.create({
            ...
            entityId: id,
            regionCode: "GLOBAL",
            metadata: { planId, interval, currency, autoRenew },
            success: true
        });

        res.json({ url: session.url });
    } catch (err) {
        res.status(500).json({ message: "Failed to generate payment link", error: err.message });
    }
};
...
exports.getPortalUrl = async (req, res) => {
    try {
        ...
        if (!org.subscription.providerCustomerId) {
            return res.status(400).json({ message: "No active billing customer found for this organization." });
        }

        const provider = getProviderForOrg(org);
        const session = await provider.createPortalSession(
            org.subscription.providerCustomerId,
            `${process.env.FRONTEND_URL}/org/billing`
        );

        res.json({ url: session.url });
    } catch (err) {
        res.status(500).json({ message: "Failed to generate portal URL", error: err.message });
    }
};
...
exports.getSalesMetrics = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Failed to fetch sales metrics", error: err.message });
    }
};
