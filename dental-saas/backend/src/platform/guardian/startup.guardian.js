...
const { guardianLogger, safeSerialize, incrementMetric, setMetric } = require("./observability.guardian");
...
async function runCheck(name, fn) {
    try {
        ...
        return { name, pass: false, reason: err.message || String(err) };
    }
}
...
function checkOrganizationPlanIdNotRequired() {
    ...
    if (planIdPath) {
        throw new Error(
            "Organization.planId has been re-introduced in the schema. " +
            "This field was removed (audit B-4) because it was a dangling write to the legacy Plan.model. " +
            "OrgContract.planVersionId is the authoritative commercial reference. Remove planId from Organization.js."
        );
    }
}
...
function checkOrgContractPreviousContractId() {
    ...
    if (!field) {
        throw new Error(
            "OrgContract schema is missing the 'previousContractId' field. " +
            "This field is written by replaceContract() service — its absence causes silent schema strip. " +
            "Add it to OrgContract.model.js."
        );
    }
}
...
function checkOrgContractLockedPriceConstraint() {
    ...
    if (!hasMinValidator) {
        throw new Error(
            "OrgContract.lockedPrice has no min:0 validator. Negative prices can be stored. " +
            "Add 'min: 0' to the lockedPrice field definition."
        );
    }
}
...
function checkRouteManifestIntegrity() {
    ...
    try {
        const { getRegisteredRouters } = require("../../integrity/routerRegistry");
        ...
        if (routers && typeof routers === "object") {
            const platformRegistered = Object.values(routers).some(
                r => r && (r.includes("/api/platform") || r.includes("platform"))
            );
            if (!platformRegistered) {
                throw new Error(
                    "/api/platform route is not registered in routerRegistry. " +
                    "This may indicate the platform routes were not mounted in app.js."
                );
            }
        }
...
async function checkOrgsHaveActiveContract() {
    ...
    if (orgsWithoutContract > 0) {
        throw new Error(
            `${orgsWithoutContract} active organization(s) found without currentContractId. ` +
            `All orgs should have an active contract assigned by provisioning (TDS). ` +
            `Run a migration to backfill trial contracts for legacy orgs.`
        );
    }
}
...
async function checkContractCountInvariants() {
    ...
    if (activeViolations > 0) {
        throw new Error(
            `${activeViolations} organization(s) have more than 1 ACTIVE OrgContract. ` +
            `This violates the unique_active_contract_per_org invariant. Investigate immediately.`
        );
    }

    // Find orgs with >1 pending_activation contract
    const multiplePending = await OrgContractModel.aggregate([
        { $match: { contractStatus: "pending_activation" } },
        { $group: { _id: "$organizationId", count: { $sum: 1 } } },
        { $match: { count: { $gt: 1 } } },
        { $count: "violations" }
    ]);
    ...
    if (pendingViolations > 0) {
        throw new Error(
            `${pendingViolations} organization(s) have more than 1 PENDING_ACTIVATION OrgContract. ` +
            `This violates the unique_pending_contract_per_org invariant.`
        );
    }
}
...
async function checkSalesContractIntegrity() {
    ...
    const multipleSalesDrafts = await OrgContractModel.aggregate([
        { $match: { contractStatus: "draft", source: "sales" } },
        { $group: { _id: "$organizationId", count: { $sum: 1 } } },
        { $match: { count: { $gt: 1 } } },
        { $count: "violations" }
    ]);
    ...
    if (violations > 0) {
        throw new Error(
            `SALES_CONTRACT_INTEGRITY: ${violations} organization(s) have >1 sales-source draft contract. ` +
            `Cancel or finalize existing sales draft before creating a new one.`
        );
    }
}
...
function checkNoLegacyPlanModel() {
    ...
    if (models["Plan"]) {
        throw new Error(
            "NO_LEGACY_PLAN_MODEL_PRESENT violated: The legacy 'Plan' Mongoose model is registered at runtime. " +
            "The Plan model and its routes were removed in the Plan→PlanVersion migration. " +
            "Check for any remaining imports of plan.model.js or shared/models/Plan.model.js."
        );
    }
}
...
function checkLegacyPlanModelTombstone() {
    ...
    if (getPlatformConnection().models["Plan"]) {
        throw new Error(
            "LEGACY_PLAN_MODEL_TOMBSTONE_PRESENT violated: The legacy 'Plan' Mongoose model is registered. " +
            "The migration to PlanTemplate/PlanVersion removed this model. " +
            "Fix: ensure platform/domain/models/plan.model.js exports only a LegacyPlanShim (not a Mongoose model)."
        );
    }

    // Secondary check: require the model file and verify it is NOT a Mongoose Model
    try {
        const planModelExport = require("../domain/models/plan.model").default;
        // A real mongoose model is a function whose .modelName === the registered name
        ...
        if (isMongooseModel) {
            throw new Error(
                "LEGACY_PLAN_MODEL_TOMBSTONE_PRESENT violated: platform/domain/models/plan.model.js exports a Mongoose Model named 'Plan'. " +
                "This file must export the LegacyPlanShim only (no mongoose.model() call). " +
                "The tombstone shim has been replaced or deleted. Restore it."
            );
        }
    } catch (err) {
    ...
    guardianLogger.info(
        { guardian: true, check: "LEGACY_PLAN_MODEL_TOMBSTONE_PRESENT" },
...
async function checkActivePlanVersionPerTemplate() {
    ...
    if (count > 0) {
        throw new Error(
            `ACTIVE_PLAN_VERSION_PER_TEMPLATE violated: ${count} PlanTemplate(s) have more than 1 active PlanVersion. ` +
            `This violates the single-active invariant. Run publishPlanVersion which auto-deprecates the previous.`
        );
    }
}
...
async function checkPlanVersionVisibilityEnum() {
    ...
    if (invalid > 0) {
        throw new Error(
            `PLAN_VERSION_VISIBILITY_ENUM violated: ${invalid} PlanVersion document(s) have an invalid or missing visibility value. ` +
            `Valid values: ${VALID_VISIBILITY.join(", ")}. ` +
            `Run: node scripts/migratePlanVisibility.js to repair.`
        );
    }
}
...
async function checkPublicPlanVersionNoLeakage() {
    ...
    if (nonPublic.length > 0) {
        throw new Error(
            `PUBLIC_PLAN_VERSION_NO_LEAKAGE violated: ${nonPublic.length} document(s) returned ` +
            `by the public query have visibility !== "public". ` +
            `This indicates a MongoDB index or data integrity failure. IDs: ` +
            nonPublic.map(v => v._id).join(", ")
        );
    }
}
...
async function checkPlanRegionPriceCompleteness() {
    ...
    const activeVersions = await PlanVersionModel.find(
        { status: "active" },
        { _id: 1, templateCode: 1, versionTag: 1, "pricing.regions": 1 }
    ).lean();
    ...
    for (const pv of activeVersions) {
        ...
        for (const region of regions) {
            ...
            if (missing.length > 0) {
                violations.push(
                    `PlanVersion ${pv._id} (${pv.templateCode}@${pv.versionTag}) ` +
                    `region "${region.regionCode || "?"}" missing: ${missing.join(", ")}`
                );
            }
        }
    }
    ...
    if (violations.length > 0) {
        throw new Error(
            `PLAN_REGION_PRICE_COMPLETENESS violated: ${violations.length} active PlanVersion(s) have ` +
...
async function checkContractPricingSnapshotPresent() {
    ...
    const paidActiveContracts = await OrgContractModel.find({
        ...
        _id: 1, organizationId: 1, lockedPrice: 1, currency: 1, pricingSnapshot: 1
    }).lean();
    ...
    if (violations.length > 0) {
        throw new Error(
            `CONTRACT_PRICING_SNAPSHOT_PRESENT violated: ${violations.length} paid active contract(s) ` +
            `are missing pricingSnapshot.\n  ` + violations.join("\n  ") +
...
async function checkProviderPriceMappingValid() {
    ...
    const providerContracts = await OrgContractModel.find({
        contractStatus: "active",
        paymentProvider: { $nin: [null, "manual"] }
    }, {
        _id: 1, organizationId: 1, paymentProvider: 1, providerPriceId: 1
    }).lean();
    ...
    if (violations.length > 0) {
        throw new Error(
            `PROVIDER_PRICE_MAPPING_VALID violated: ${violations.length} active contract(s) have ` +
            `paymentProvider set but no providerPriceId.\n  ` +
            violations.map(c =>
                `Contract ${c._id} (org: ${c.organizationId}, provider: ${c.paymentProvider})`
            ).join("\n  ")
        );
    }
}
...
async function checkUniqueActiveContractPerOrg() {
    ...
    if (count > 0) {
        throw new Error(
            `UNIQUE_ACTIVE_CONTRACT_PER_ORG violated: ${count} organization(s) have more than 1 active contract. ` +
            `This violates the unique_active_contract_per_org partial index invariant. ` +
            `The DB write guard may have been bypassed. Investigate immediately.`
        );
    }
}
...
async function checkContractStatusValid() {
    ...
    const { ALL_VALID_STATUSES: VALID_STATUSES } = require("../billing/services/contractStateMachine");

    const invalidCount = await OrgContractModel.countDocuments({
        contractStatus: { $nin: VALID_STATUSES }
    });
    ...
    if (invalidCount > 0) {
        ...
        const details = sample
            .map(c => `Contract ${c._id} (org: ${c.organizationId}): status="${c.contractStatus}"`)
            .join(", ");

        throw new Error(
            `CONTRACT_STATUS_VALID violated: ${invalidCount} contract(s) have an unrecognized contractStatus. ` +
            `Valid statuses: ${VALID_STATUSES.join(", ")}. ` +
            `Sample: ${details}. ` +
            `This indicates a direct DB write bypassed the contract state machine.`
        );
    }
}
...
async function checkContractTimelineIntegrity() {
    ...
    const contracts = await OrgContractModel
        .find(
            {
                contractStatus: { $in: COVERAGE_STATUSES },
                effectiveFrom: { $exists: true, $ne: null }
            },
            { _id: 1, organizationId: 1, effectiveFrom: 1, effectiveTo: 1, contractStatus: 1 }
        )
    ...
    if (schemaGaps.length > 0) {
        guardianLogger.warn(
            ...
            `CONTRACT_TIMELINE_INTEGRITY: ${schemaGaps.length} superseded/expired contract(s) have ` +
            `no effectiveTo set. This is a schema gap — contractActivation does not stamp ` +
            `effectiveTo on superseded contracts. Run a backfill to resolve.`
        );
    }
    ...
    if (violations.length > 0) {
        guardianLogger.error(
            { count: violations.length, sample: violations.slice(0, 3) },
            "CONTRACT_TIMELINE_CORRUPTION detected at startup"
        );

        const details = violations.slice(0, 5).map(v =>
            `Org ${v.orgId}: ` +
            `Contract ${v.contractA} (${v.statusA}, ` +
            `${v.fromA?.toISOString()} → ${v.toA?.toISOString()}) ` +
            `overlaps with Contract ${v.contractB} (${v.statusB}, from ${v.fromB?.toISOString()})`
        ).join(" | ");

        throw new Error(
            `CONTRACT_TIMELINE_INTEGRITY violated: ${violations.length} overlapping coverage window(s) detected. ` +
            details +
...
async function checkContractGapIntegrity() {
    ...
    const superseded = await OrgContractModel.find(
        ...
        { _id: 1, organizationId: 1, effectiveTo: 1, supersededById: 1 }
    ).lean();
    ...
    const successors = await OrgContractModel.find(
        { _id: { $in: successorIds } },
        { _id: 1, effectiveFrom: 1, contractStatus: 1 }
    ).lean();
    ...
    const details = violations.slice(0, 5).map(v =>
        `Org ${v.orgId}: ` +
        `Contract ${v.contractA} ends ${v.contractAEnd.toISOString()} → ` +
        `Contract ${v.contractB} starts ${v.contractBStart.toISOString()} ` +
        `(gap: ${v.gapHours}h)`
    ).join(" | ");
    ...
    throw new Error(
        `CONTRACT_GAP_INTEGRITY violated: ${violations.length} gap(s) detected in supersession chain. ` +
        `Contracts must form a continuous timeline: prev.effectiveTo === next.effectiveFrom. ` +
        details +
        (violations.length > 5 ? ` ... and ${violations.length - 5} more.` : "") +
        ` Fix: node scripts/fixContractGaps.js`
    );
}
...
async function checkOrgCurrentContractPointerIntegrity() {
    ...
    const results = await OrganizationModel.aggregate([
        {
            $match: {
                currentContractId: { $exists: true, $ne: null },
                isArchived: { $ne: true }  // archived orgs may have stale pointers intentionally
            }
        },
        {
            $lookup: {
    ...
    for (const org of results) {
        if (!org.contract) {
            ...
            violations.push(
                `Org ${org._id} (${org.name || "unnamed"}): ` +
                `currentContractId=${org.currentContractId} references a NON-EXISTENT contract`
            );
            continue;
        }
        ...
        if (org.contract.contractStatus !== "active") {
            ...
            violations.push(
                `Org ${org._id} (${org.name || "unnamed"}): ` +
                `currentContractId=${org.currentContractId} references contract with ` +
                `status="${org.contract.contractStatus}" (expected "active")`
            );
        }
    }
    ...
    if (violations.length > 0) {
        ...
        throw new Error(
            `ORG_CURRENT_CONTRACT_POINTER_INTEGRITY violated: ${violations.length} organization(s) ` +
            `have a currentContractId that does not point to an active contract.\n  ` +
            violations.join("\n  ") +
...
async function checkActivePlanVersionPerTemplate() {
    ...
    const details = violations.map(v =>
        `templateCode="${v._id}": ${v.count} active versions — tags: [${v.versionTags.join(", ")}]`
    ).join(" | ");
    ...
    throw new Error(
        `ACTIVE_PLAN_VERSION_PER_TEMPLATE violated: ${violations.length} template(s) have multiple active PlanVersions. ` +
        details +
        ` Fix: deprecate all but the correct active version for each listed template. ` +
        `Prevent recurrence: ensure unique_active_plan_version_per_template index exists in MongoDB.`
    );
}
...
async function checkDeprecatedPublicPlan() {
    ...
    throw new Error(
        ...
        `but still have visibility="public". Deprecated plans must have visibility="sales" or "internal". ` +
        `Fix: set visibility="sales" on each listed version. ` +
        details
    );
}
...
async function checkBillingLedgerIntegrity() {
    ...
    const duplicates = await LedgerModel.aggregate([
        { $match: { providerEventId: { $ne: null } } },
        { $group: { _id: "$providerEventId", count: { $sum: 1 } } },
        { $match: { count: { $gt: 2 } } },
        { $count: "violations" }
    ]);
    ...
    if (violationCount > 0) {
        ...
        guardianLogger.warn(
            { violationCount },
            `BILLING_LEDGER_INTEGRITY: ${violationCount} providerEventId(s) appear more than twice ` +
            `in BillingLedger. This may indicate repeated webhook delivery without idempotency. ` +
            `Investigate canonicalEventProcessor deduplication logic.`
        );
...
async function checkStrandedPendingPaymentContracts() {
    ...
    guardianLogger.error(
        { count: stranded.length, stranded },
        "STRANDED_PENDING_PAYMENT: contract(s) in pending_payment with paid invoice(s) detected — " +
        "payment committed but activation was lost (crash window)"
    );

    throw new Error(
        `STRANDED_PENDING_PAYMENT violated: ${stranded.length} contract(s) are stuck in ` +
        `pending_payment despite having a fully paid invoice. ` +
        `These contracts should be active. Run activateContract() for each, ` +
        `or allow guardian auto-repair in development mode. ` +
        `Contract IDs: ${stranded.map(s => String(s.contractId)).join(", ")}`
    );
}
...
async function checkInvoiceContractIntegrity() {
    ...
    const orphans = await PlatformInvoiceModel.aggregate([
        { $match: { contractId: { $ne: null }, status: { $nin: ["void", "draft"] } } },
        {
            $lookup: {
                from: "orgcontracts",
        ...
        { $match: { contract: { $size: 0 } } },
        { $project: { _id: 1, contractId: 1, organizationId: 1, status: 1 } },
        { $limit: 10 }
    ]);
    ...
    const details = orphans
        .map(i => `Invoice ${i._id} (org: ${i.organizationId}): contractId=${i.contractId} NOT FOUND`)
        .join(", ");
    ...
    throw new Error(
        `INVOICE_CONTRACT_INTEGRITY violated: ${orphans.length} invoice(s) reference non-existent contracts. ` +
        `Sample: ${details}. ` +
        `Investigate: contracts may have been deleted without voiding their invoices.`
    );
}
...
async function checkOrphanPayments() {
    ...
    const orphans = await PaymentAttemptModel.aggregate([
        { $match: { invoiceId: { $ne: null }, status: { $nin: ["voided", "failed"] } } },
        {
            $lookup: {
        ...
        { $project: { _id: 1, invoiceId: 1, organizationId: 1, amount: 1, status: 1 } },
        { $limit: 10 }
    ]);
    ...
    const details = orphans
        .map(p => `Payment ${p._id} (org: ${p.organizationId}): invoiceId=${p.invoiceId} NOT FOUND, amount=${p.amount}`)
        .join(", ");
    ...
    throw new Error(
        `ORPHAN_PAYMENT_INTEGRITY violated: ${orphans.length} payment(s) reference non-existent invoices. ` +
        `Sample: ${details}. ` +
        `Investigate: invoices may have been deleted without cleaning up payment records.`
    );
}
...
async function checkTrialPlanVersionExists() {
    ...
    if (!exists) {
        guardianLogger.error(
            {},
            "TRIAL_PLAN_VERSION_EXISTS: No active PlanVersion with templateCode='trial-tier' found. " +
            "All signup flows will fail with 500. Seed a trial plan immediately."
        );

        throw new Error(
            `TRIAL_PLAN_VERSION_EXISTS violated: No active PlanVersion with templateCode='trial-tier'. ` +
            `Signup will fail for all new organizations. ` +
            `Fix: create a PlanTemplate with code='trial-tier' and publish an active PlanVersion.`
        );
    }
}
...
async function runStartupGuardian() {
    ...
    guardianLogger.info(
        { pid: process.pid, entry: require.main?.filename || "unknown", mode, guardianMode: process.env.PLATFORM_GUARDIAN_MODE || "permissive" },
        "Startup Guardian started"
    );

    // ── Phase 1: Run all invariant checks in parallel ─────────────────────────
    const allChecks = [
        runCheck("NO_DUPLICATE_MODELS", checkNoDuplicateModels),
        runCheck("ORG_PLAN_ID_NOT_REQUIRED", checkOrganizationPlanIdNotRequired),
        runCheck("NO_LEGACY_PLAN_MODEL_PRESENT", checkNoLegacyPlanModel),
        runCheck("LEGACY_PLAN_MODEL_TOMBSTONE_PRESENT", checkLegacyPlanModelTombstone),
        runCheck("CONTRACT_PREVIOUS_ID_IN_SCHEMA", checkOrgContractPreviousContractId),
        runCheck("CONTRACT_LOCKED_PRICE_MIN", checkOrgContractLockedPriceConstraint),
        runCheck("TRANSACTION_SUPPORT", checkTransactionSupport),
        runCheck("ROUTE_MANIFEST_INTEGRITY", checkRouteManifestIntegrity),
        runCheck("ORG_WITHOUT_ACTIVE_CONTRACT", checkOrgsHaveActiveContract),
        runCheck("CONTRACT_COUNT_INVARIANTS", checkContractCountInvariants),
        runCheck("SALES_CONTRACT_INTEGRITY", checkSalesContractIntegrity),
        runCheck("ACTIVE_PLAN_VERSION_PER_TEMPLATE", checkActivePlanVersionPerTemplate),
        runCheck("DEPRECATED_PUBLIC_PLAN", checkDeprecatedPublicPlan),
        runCheck("PLAN_VERSION_VISIBILITY_ENUM", checkPlanVersionVisibilityEnum),
        runCheck("PUBLIC_PLAN_VERSION_NO_LEAKAGE", checkPublicPlanVersionNoLeakage),
        // ── COMMERCIAL BILLING INTEGRITY ──────────────────────────────
        runCheck("UNIQUE_ACTIVE_CONTRACT_PER_ORG", checkUniqueActiveContractPerOrg),
        runCheck("PLAN_REGION_PRICE_COMPLETENESS", checkPlanRegionPriceCompleteness),
        runCheck("CONTRACT_PRICING_SNAPSHOT_PRESENT", checkContractPricingSnapshotPresent),
        runCheck("PROVIDER_PRICE_MAPPING_VALID", checkProviderPriceMappingValid),
        runCheck("CONTRACT_STATUS_VALID", checkContractStatusValid),
        runCheck("CONTRACT_TIMELINE_INTEGRITY", checkContractTimelineIntegrity),
        runCheck("CONTRACT_GAP_INTEGRITY", checkContractGapIntegrity),
        runCheck("ORG_CURRENT_CONTRACT_POINTER_INTEGRITY", checkOrgCurrentContractPointerIntegrity),
        // ── BILLING LEDGER INTEGRITY ─────────────────────────────────────────────────────
    ...
    if (IS_DEV() && failures.length > 0) {
        ...
        if (nonRepairableFailures.length === 0 && repairableFailures.length > 0) {
            ...
            guardianLogger.warn(
                ...
                "[Guardian] Development auto-repair skipped — non-repairable failures present. " +
                "Fix the non-repairable invariants manually before auto-repair can run."
            );
        }
    }
    ...
    passes.forEach(r => {
        guardianLogger.info({ check: r.name }, `✅ ${r.name}`);
    });
    ...
    guardianLogger.critical(
        { failures: failures.length, total: results.length, mode },
    ...
    if (billingCriticalFailures.length > 0) {
        ...
        try {
            const { activateBillingKillSwitch } = require("../billing/services/billingControlService");
            await activateBillingKillSwitch(killReason, "guardian", "guardian").catch(err => {
                guardianLogger.error(
                    { billing: true, err: err.message },
            ...
            guardianLogger.warn(
                { billing: true, importErr: importErr.message },
    ...
    if (STRICT()) {
        guardianLogger.critical(
            { mode: "strict", env: process.env.NODE_ENV },
...
module.exports = { runStartupGuardian };
