...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (decoded.type === "platform") {
      ...
      if (decoded.tokenVersion !== undefined && decoded.tokenVersion !== platformUser.tokenVersion) {
        return res.status(401).json({
          ...
          error: {
            code: "UNAUTHORIZED",
            message: "Session expired due to security update"
          }
        });
      }
    ...
    if (!decoded.organizationId) {
      return res.status(401).json({
        ...
        error: {
          ...
          message: "Identity token is missing organization context."
        }
      });
    }
    ...
    const user = await User.findById(decoded.userId).select("_id name email organizationId roleId tokenVersion branchAccess hasFullBranchAccess primaryBranchId platformDesignation isActive isEmailVerified");
    ...
    Object.defineProperty(req, "organization", {
      get() {
        const err = new Error("❌ FORBIDDEN: req.organization is deprecated (Phase 8). " + "Use req.context.organizationId instead.");
    ...
    if (process.env.AUTH_TRACE === "true") {
      req.logger?.debug({
        event: "AUTH_TRACE_JWT_VERIFY_FAIL",
        name: error.name,
        message: error.message
      }, "[AUTH_TRACE] JWT verify fail");
    }
    return res.status(401).json({
      success: false,
      error: {
