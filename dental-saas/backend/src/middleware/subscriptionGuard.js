...
const { emitInvoiceEmail } = require("../events/email.events");
    ...
    try {
        ...
        try {
                ...
                return res.status(403).json({
                    ...
                    error: {
                        ...
                        hint:    process.env.NODE_ENV !== "production"
                            ? "Run: node scripts/seedTrialPlan.js"
                            : undefined,
                    },
                });
            }
            ...
            return res.status(500).json({
                success: false,
                error: { code: "SUBSCRIPTION_SYSTEM_ERROR", message: "Subscription system error. Please try again." },
            });
        }
        ...
        if (sub.status === "trial") {
            if (sub.trialEndsAt && now > new Date(sub.trialEndsAt)) {
                return res.status(403).json({ message: "TRIAL_EXPIRED" });
            }
        ...
        if (endDate && now > new Date(endDate)) {
            ...
            if (!sub.gracePeriodEnd) {
                ...
                const graceEnd = new Date(Date.UTC(
                    anchor.getUTCFullYear(),
                    anchor.getUTCMonth(),
                    anchor.getUTCDate() + graceDays,
