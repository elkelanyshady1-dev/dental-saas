...
const Region = require("../../platform/domain/models/Region.model").default;
...
async function loadRegionRegistry() {
    const regions = await Region.find({ status: "ACTIVE" }).lean();

    if (!regions.length) {
        throw new Error(
            "[RegionRegistry] EMPTY — No active regions in Control Plane. " +
            "Run: node scripts/seedRegions.js"
        );
    }
...
function validateRegionRegistry(requiredCodes = ["EU", "US", "MEA", "APAC"]) {
    ...
    if (missing.length > 0) {
        throw new Error(
            `[RegionRegistry] INCOMPLETE — Missing regions: ${missing.join(", ")}. ` +
            "Run: node scripts/seedRegions.js"
        );
    }
...
async function refreshRegionRegistry() {
    try {
        ...
        logger.debug({ service: "regionRegistry", action: "REGION_REGISTRY_REFRESHED" }, "[RegionRegistry] Background refresh complete.");
    } catch (err) {
...
function stopRegistryRefresh() {
    if (refreshInterval) {
        ...
        logger.info({ service: "regionRegistry" }, "[RegionRegistry] Background refresh stopped.");
    }
}
