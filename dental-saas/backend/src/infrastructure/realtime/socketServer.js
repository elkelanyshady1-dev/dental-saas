...
const initSocket = (server) => {
    ...
    io.on("connection", (socket) => {
        const { organizationId, user } = socket;
    ...
    collabNs.use(async (socket, next) => {
        ...
        try {
            const shared = await SharedCase.findOne({ token, isRevoked: false }).lean();
    ...
    collabNs.on("connection", (socket) => {
        ...
        logger.info(
            { socketId: socket.id, shareId: socket.shareId, name: socket.collaboratorName },
...
const registerDomainEventListeners = () => {
    ...
    const { emitToOrg } = require("./eventEmitter");
    ...
    eventBus.on(APT_CREATED, (payload) => {
        const { organizationId, appointmentId, branchId, patientId, dentistId, date } = payload;
    ...
    eventBus.on(APT_UPDATED, (payload) => {
        const { organizationId, appointmentId, branchId, date, startTime, endTime } = payload;
    ...
    eventBus.on(APT_STATUS_CHANGED, (payload) => {
        const { organizationId, appointmentId, status, previousStatus, branchId } = payload;
    ...
    Object.entries(PATIENT_EVENT_MAP).forEach(([busEvent, socketEvent]) => {
        eventBus.on(busEvent, (payload) => {
            const { organizationId, patientId } = payload;
...
module.exports = { initSocket, getIO, getConnectionMetrics };
