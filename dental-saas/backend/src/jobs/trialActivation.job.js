...
async function processTrial(trialSummary) {
  ...
  try {
    ...
    let pending = await OrgContract.findOne({
      ...
      effectiveFrom: {
        $lte: now
      } // guard: effective date must have passed
    }).session(session);
    ...
    if (pending) {
      ...
      setImmediate(async () => {
        try {
          await emitBillingTimelineEvent({
            ...
            payload: {
              outcome: "activated",
              pendingContractId: String(pending._id)
            }
          });
          await emitBillingTimelineEvent({
            organizationId: String(organizationId),
      ...
      logger.info({
        organizationId,
        trialContractId: String(trialId),
        pendingContractId: String(pending._id)
...
async function processExpiredTrials() {
  ...
  if (expiredTrials.length === 0) {
    ...
    logger.info({
      durationMs
    }, "[TrialActivationJob] No expired trials — nothing to do");
    return {
      ...stats,
      durationMs
    };
  ...
  for (const trial of expiredTrials) {
    ...
    try {
      ...
      if (outcome === "activated") stats.activated++;else if (outcome === "suspended") stats.suspended++;else stats.skipped++;
    } catch (err) {
      stats.errors++;
...
function start(schedule = "10 0 * * *") {
  ...
  _job = cron.schedule(schedule, async () => {
    ...
    try {
      ...
      if (stats) logger.info({
        stats
      }, "[TrialActivationJob] Scheduled run complete");
    } catch (err) {
      logger.error({
        err: {
          message: err.message
        }
      }, "[TrialActivationJob] Unhandled job error");
    }
  }, {
    scheduled: true,
