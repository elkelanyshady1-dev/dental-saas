...
const RevenueSchedule = require("../platform/finance/models/RevenueSchedule.model").default;
...
async function recognizeRevenue() {
    ...
    logger.info({ action: "revenue_recognition_start", now },
    ...
    logger.info({ count: schedules.length }, "[RevenueRecognition] Schedules to process");

    let processed = 0, fullyRecognized = 0, errors = 0;
    ...
    for (const sched of schedules) {
        try {
            ...
            const normPortion = Math.min(
                sched.normalizedAmountPerPeriod || 0,
                sched.normalizedDeferredAmount || 0
            );
            ...
            try {
                ...
                await PlatformInvoice.findByIdAndUpdate(
                    sched.invoiceId,
                    {
                        $set: {
            ...
            errors++;
            logger.error({ err, scheduleId: sched._id },
                "[RevenueRecognition] Error processing schedule");
        }
    }

    logger.info({ processed, fullyRecognized, errors },
        "[RevenueRecognition] Recognition pass complete");
    return { processed, fullyRecognized, errors };
}
...
function start(schedule = "0 2 1 * *") {
    ...
    _job = cron.schedule(schedule, async () => {
        logger.info({ job: "revenueRecognition", at: new Date().toISOString() },
            "[RevenueRecognitionJob] Starting monthly run");
        try {
            const stats = await recognizeRevenue();
            logger.info({ stats }, "[RevenueRecognitionJob] Complete");
        } catch (err) {
            logger.error({ err }, "[RevenueRecognitionJob] Unhandled error");
        }
    }, { scheduled: true, timezone: process.env.CRON_TIMEZONE || "UTC" });
    logger.info({ schedule }, "[RevenueRecognitionJob] Registered");
}
...
module.exports = { start, stop, runNow, recognizeRevenue };
