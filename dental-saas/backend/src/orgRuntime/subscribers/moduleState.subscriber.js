...
function initModuleStateSubscriber() {
    ...
    eventBus.on("module.enabled", async (payload) => {
        try {
            const { organizationId, moduleKey, actorId, actorType } = payload;

            if (!organizationId || !moduleKey) {
                logger.warn(
                    { event: "module.enabled", payload },
            ...
            logger.error(
                { err: err.message, event: "module.enabled" },
    ...
    eventBus.on("module.disabled", async (payload) => {
        try {
            ...
            logger.error(
                { err: err.message, event: "module.disabled" },
...
module.exports = { initModuleStateSubscriber };
