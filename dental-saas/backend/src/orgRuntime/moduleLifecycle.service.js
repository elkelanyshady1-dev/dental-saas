...
async function enableModule(organizationId, registryKey, opts = {}) {
  ...
  if (!moduleDef) {
    throw Object.assign(new Error(`Unknown module: "${registryKey}". Only registered modules can be toggled.`), {
  ...
  if (moduleDef.isCore) {
    throw Object.assign(new Error(`Module "${registryKey}" is a core module and is always enabled.`), {
      code: "CORE_MODULE_NO_TOGGLE",
      statusCode: 400
    });
  }
  ...
  if (!moduleDef.allowedPlans.includes(currentPlan)) {
    throw Object.assign(new Error(`Module "${registryKey}" requires plan [${moduleDef.allowedPlans.join(", ")}], ` + `but organization is on "${currentPlan}".`), {
      code: "MODULE_PLAN_RESTRICTED",
      statusCode: 403
    });
  }
  ...
  if (moduleDef.dependencies && moduleDef.dependencies.length > 0) {
    ...
    if (missingDeps.length > 0) {
      throw Object.assign(new Error(`Cannot enable "${registryKey}": dependencies not enabled: [${missingDeps.join(", ")}]`), {
...
async function disableModule(organizationId, registryKey, opts = {}) {
  const moduleDef = getModule(registryKey);
  if (!moduleDef) {
    throw Object.assign(new Error(`Unknown module: "${registryKey}".`), {
      code: "MODULE_NOT_FOUND",
      statusCode: 400
    });
  }
  ...
  if (moduleDef.isCore) {
    throw Object.assign(new Error(`Module "${registryKey}" is a core module and cannot be disabled.`), {
      code: "CORE_MODULE_NO_TOGGLE",
      statusCode: 400
    });
  }
  ...
  if (!opts.force) {
    ...
    if (blockers.length > 0) {
      throw Object.assign(new Error(`Cannot disable "${registryKey}": the following enabled modules depend on it: [${blockers.join(", ")}]. ` + `Disable those modules first, or use force=true.`), {
        code: "REVERSE_DEPENDENCY_BLOCK",
        statusCode: 400,
        blockers
      });
    }
...
async function bulkSetModules(organizationId, moduleMap, opts = {}) {
  ...
  for (const [registryKey, shouldEnable] of Object.entries(moduleMap)) {
    ...
    if (moduleDef.isCore) {
      results.push({
        registryKey,
        success: true,
        note: "Core module — always enabled"
      });
      continue;
    }
  ...
  if (Object.keys(updateSet).length > 0) {
    ...
    logger.info({
      ...
      organizationId,
      moduleCount: Object.keys(updateSet).length - 1,
      // exclude modulesUpdatedAt
      actorId: opts.actorId || "system",
      actorType: opts.actorType || "platform"
