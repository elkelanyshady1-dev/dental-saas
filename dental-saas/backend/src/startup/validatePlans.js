...
async function validatePlans() {
    ...
    if (!trial) {
        ...
        if (process.env.NODE_ENV === "production") {
            logger.error({ event: "PLAN_VALIDATION_FAILED" }, `[StartupValidator] ${msg}`);
            throw new Error(msg);
        }

        // Non-production: warn but continue (let devs seed lazily)
        logger.warn(
            { event: "PLAN_VALIDATION_MISSING", hint: "node scripts/seedTrialPlan.js" },
