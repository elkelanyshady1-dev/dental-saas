 ...
 */
async function getContractsNeedingRenewal({ lookaheadDays = 14, page = 1, limit = 50 } = {}) {
    const now = new Date();
    ...
    const query = {
        ...
        $or: [
            ...
            { effectiveTo: { $lte: lookahead } },
            // 2. Currently in dunning
            { "dunning.nextRetryAt": { $exists: true, $ne: null } },
            // 3. Grace period running
            { "dunning.gracePeriodEndsAt": { $exists: true, $ne: null } }
        ]
    };
    ...
    const contracts = await OrgContract.find(query)
        .sort({ effectiveTo: 1 })
        .skip(skip)
        .limit(limit)
        .lean();
    ...
    if (!contracts.length) {
        return {
            ...
            summary: { expiringSoon: 0, inDunning: 0, inGrace: 0 }
        };
    }

    // Batch-load org names
    const orgIds = [...new Set(contracts.map(c => c.organizationId.toString()))];
    const orgs = await Organization.find({ _id: { $in: orgIds } }, { name: 1, status: 1 }).lean();
    ...
    const invoices = await PlatformInvoice.find(
        { contractId: { $in: contractIds }, status: { $in: ["open", "issued"] } },
        { contractId: 1, status: 1, totalAmountMinor: 1, currency: 1, dueDate: 1 }
    ).lean();
    const invoiceMap = {};
    ...
    // Summary counters
    let expiringSoon = 0, inDunning = 0, inGrace = 0;

    const data = contracts.map(contract => {
        const org = orgMap[contract.organizationId.toString()] || {};
        const invoice = invoiceMap[contract._id.toString()] || null;
        const daysToExpiry = contract.effectiveTo
            ? Math.ceil((new Date(contract.effectiveTo) - now) / 86_400_000)
            : null;
        ...
        // Classify priority
        let priority = "low";
        if (isGrace) priority = "critical";
        else if (isDunning) priority = "high";
        else if (daysToExpiry !== null && daysToExpiry <= 3) priority = "high";
        else if (isExpiring) priority = "medium";
...
module.exports = { getContractsNeedingRenewal };
