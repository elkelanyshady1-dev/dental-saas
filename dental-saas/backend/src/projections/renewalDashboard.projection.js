...
async function getRenewalDashboardMetrics() {
    ...
    const [
        ...
        OrgContract.countDocuments({ contractStatus: "active" }),

        // 2. Expiring in next 30 days
        OrgContract.countDocuments({
            contractStatus: "active",
            effectiveTo: { $lte: in30Days, $gte: now }
        }),
        ...
        PlatformInvoice.countDocuments({
            status: "open",
            createdAt: { $gte: ago7Days }
        }),

        // 6. Auto-renew enabled
        OrgContract.countDocuments({ contractStatus: "active", autoRenew: true }),

        // 7. Sales-managed
        OrgContract.countDocuments({ contractStatus: "active", salesManaged: true }),
        ...
        OrgContract.aggregate([
            ...
            { $group: { _id: null, projectedRevenue: { $sum: "$lockedPrice" }, contractCount: { $sum: 1 } } }
        ]),

        // 9. Normalized recognized revenue (active schedules)
        RevenueSchedule.aggregate([
            { $match: { status: { $in: ["active", "fully_recognized"] } } },
            { $group: { _id: null, total: { $sum: "$normalizedRecognizedAmount" } } }
        ]),

        // 10. Normalized deferred revenue (active schedules only — not yet recognized)
        RevenueSchedule.aggregate([
            { $match: { status: "active" } },
            { $group: { _id: null, total: { $sum: "$normalizedDeferredAmount" } } }
        ])
    ]);
    ...
    const mrrAgg = await RevenueSchedule.aggregate([
        { $match: { status: "active", recognitionFrequency: "monthly" } },
        { $group: { _id: null, mrr: { $sum: "$normalizedAmountPerPeriod" } } }
    ]);
...
module.exports = { getRenewalDashboardMetrics };
