 ...
 * Root projection — Financial Print Views (READ MODEL)
 * Phase 3.3 — Strict per-org model resolution via getModel. No fallback.
 *
 * Phase G — Import Remediation: redirected from legacy financeDomain paths
 * (which never existed) to canonical billingDomain imports.
 */
...
function _getInvoice(connection) {
    if (!connection) throw new Error("[FinancialProjection] connection is REQUIRED — per-org mode does not allow fallback");
 ...
 */
async function buildPatientFinancialSummary({ organizationId, patientId, connection }) {
    const snapshot = await _getSnapshot(connection).findOne({ organizationId, patientId }).lean();
 ...
 */
async function buildInvoicePrintView({ invoiceId, organizationId, connection }) {
    const PatientInvoice = _getInvoice(connection);
    const invoice = await PatientInvoice.findOne({ _id: invoiceId, organizationId })
        .populate("patientId", "nameArabic nameEnglish phone")
        .populate("treatmentOperatorId", "name")
    ...
    const allocations = await PaymentAllocation.find({ invoiceId, organizationId }).lean();
 ...
 */
async function buildPaymentPrintView({ paymentId, organizationId, connection }) {
    const PatientPayment = _getPayment(connection);
    const payment = await PatientPayment.findOne({ _id: paymentId, organizationId })
        .populate("patientId", "nameArabic nameEnglish phone")
        .populate("collectedByUserId", "name")
        .lean();

    if (!payment) throw new Error("Payment not found or unauthorized.");

    const PaymentAllocation = _getAllocation(connection);
    const allocations = await PaymentAllocation.find({ paymentId, organizationId })
        .populate("invoiceId", "totalAmount createdAt")
        .lean();
