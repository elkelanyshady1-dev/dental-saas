...
router.get("/", async (req, res) => {
    ...
    const readyState = platformConnection.isReady()
        ? platformConnection.get().readyState
        : 0;
    ...
    try {
        const Outbox = require("@core/outbox/Outbox.model").default;
        outboxLag = await Outbox.countDocuments({ status: "pending" })
            .maxTimeMS(2000)
            .exec();
        outboxHealthy = outboxLag < 100;
    } catch (err) {
        outboxHealthy = false;
        logger.warn({ err: err.message }, "[InternalHealth] Outbox lag probe failed");
    }

    // ── Active socket count ──────────────────────────────────────────────
    let activeSockets = 0;
    try {
        const { getConnectionMetrics } = require("@infra/realtime/socketServer");
