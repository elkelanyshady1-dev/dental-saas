...
const refundExecutionRecordSchema = new mongoose.Schema({
  ...
  invoiceId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "PlatformInvoice",
    // Sprint 7.2: was BillingInvoice (tombstoned)
    required: true
  },
  contractId: {
    type: mongoose.Schema.Types.ObjectId,
  ...
  status: {
    type: String,
    enum: ["refund_requested", "refund_under_review", "refund_approved", "refund_rejected", "refund_processing", "refund_completed", "refund_failed"],
    required: true,
...
refundExecutionRecordSchema.index({
  idempotencyKey: 1
}, {
  unique: true,
  sparse: true
