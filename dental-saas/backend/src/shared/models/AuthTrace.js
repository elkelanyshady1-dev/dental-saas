...
authTraceSchema.index({ createdAt: -1 });

// Denial analytics: denial + time
authTraceSchema.index({ hasDenial: 1, createdAt: -1 });

// Resource audit: resource type + resource id
authTraceSchema.index({ resourceType: 1, resourceId: 1, createdAt: -1 });

// User activity: user + time
authTraceSchema.index({ userId: 1, createdAt: -1 });

// Permission analysis
authTraceSchema.index(
    { "steps.permission": 1 },
    { partialFilterExpression: { "steps.permission": { $exists: true } } }
);
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, authTraceSchema),
};
