...
const auditLogSchema = new mongoose.Schema({
    ...
    actorFirstName: { type: String, default: null },
    actorLastName: { type: String, default: null },
    actorRole: {
        type: String,
        default: null
    ...
    geoLocation: { type: String, default: null }, // "Cairo, Egypt" — display only
    browser: { type: String, default: null }, // "Edge"
    os: { type: String, default: null }, // "Windows"
...
auditLogSchema.pre(["updateOne", "findOneAndUpdate", "replaceOne", "updateMany"], function () {
    throw new Error(
        "[AuditLog] Immutability violation: AuditLog entries are append-only and cannot be updated. " +
...
auditLogSchema.pre(["deleteOne", "findOneAndDelete", "deleteMany"], function () {
    throw new Error(
        "[AuditLog] Immutability violation: AuditLog entries are append-only and cannot be deleted. " +
        "The audit log is a permanent compliance record."
    );
});
...
auditLogSchema.index({ previousHash: 1 }, { unique: true });
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, auditLogSchema),
};
