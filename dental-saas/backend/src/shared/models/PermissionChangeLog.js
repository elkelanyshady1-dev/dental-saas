...
const permissionChangeLogSchema = new mongoose.Schema({
    ...
    changeType: {
        type: String,
        enum: [
            "AUTO_HEAL",           // autoFixPermissions added missing fields
            "SSOT_MIGRATION",      // SSOT version bumped, role updated
            "MANUAL_EDIT",         // Admin manually changed permissions
...
permissionChangeLogSchema.pre(["updateOne", "findOneAndUpdate", "replaceOne", "updateMany"], function () {
    throw new Error(
        "[PermissionChangeLog] Immutability violation: Permission change logs are append-only and cannot be updated."
    );
});

permissionChangeLogSchema.pre(["deleteOne", "findOneAndDelete", "deleteMany"], function () {
    throw new Error(
        "[PermissionChangeLog] Immutability violation: Permission change logs are append-only and cannot be deleted."
    );
});

// Per-org DB: indexes optimized for per-database queries.
permissionChangeLogSchema.index({ createdAt: -1 });
permissionChangeLogSchema.index({ roleId: 1, createdAt: -1 });
permissionChangeLogSchema.index({ changeType: 1, createdAt: -1 });
permissionChangeLogSchema.index({ actorType: 1, createdAt: -1 });
permissionChangeLogSchema.index({ correlationId: 1 }, { sparse: true });
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, permissionChangeLogSchema),
};
