...
domainEventOutboxSchema.index({ aggregateType: 1 });
domainEventOutboxSchema.index({ aggregateId: 1 });
domainEventOutboxSchema.index({ eventType: 1 });
domainEventOutboxSchema.index({ status: 1 });
domainEventOutboxSchema.index({ claimedBy: 1 });
domainEventOutboxSchema.index({ correlationId: 1 });

// Geopolitical Sovereignty Indexes (v13.0)
domainEventOutboxSchema.index({ regionCode: 1, createdAt: 1 });
domainEventOutboxSchema.index({ regionCode: 1, status: 1 });
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, domainEventOutboxSchema),
};
