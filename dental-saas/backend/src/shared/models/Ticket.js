...
const ticketSchema = new mongoose.Schema({
    ...
    conversationThread: [{
        ...
        actorType: { type: String, enum: ["platform_user", "tenant_user", "system"] },
        message: String,
        createdAt: { type: Date, default: Date.now }
    }],
    internalNotes: [{
        actorId: mongoose.Schema.Types.ObjectId, // Always platform_user
        note: String,
        createdAt: { type: Date, default: Date.now }
    }],
    resolvedAt: {
...
ticketSchema.index({ linkedInvoiceId: 1 });
ticketSchema.index({ linkedSubscriptionId: 1 });
ticketSchema.index({ linkedMutationId: 1 });
ticketSchema.index({ providerDisputeId: 1 }, { unique: true, sparse: true });
ticketSchema.index({ slaDeadline: 1 });
ticketSchema.index({ breachFlag: 1 });
ticketSchema.index({ escalationLevel: 1 });
ticketSchema.index({ regionCode: 1, createdAt: 1 });
...
module.exports = {
    modelName,
    schema: ticketSchema,
    default: mongoose.models[modelName] || mongoose.model(modelName, ticketSchema),
};
