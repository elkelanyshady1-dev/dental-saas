...
const {
  ...
  getResourceRoles,
  hasFullAccess
} = require("../../rbac/fieldAccessRegistry");
const {
  matrix,
  getMatrixStats
} = require("../../rbac/permissionMatrix");
...
const {
  getDenialStats: fetchDenialStats
} = require("../../rbac/denialTracker");
const {
  getShadowConfig
...
async function getOverview(req, res) {
  try {
    ...
    const fieldCoveragePercent = totalFieldSlots > 0 ? Math.round(definedFieldSlots / totalFieldSlots * 1000) / 10 : 100;
    ...
    try {
      ...
      logger.warn({
        err
      }, "[Security] Weekly chart aggregation failed");
      weeklyChart = dayNames.map(name => ({
        name,
        allowed: 0,
        denied: 0
      }));
    ...
    const response = {
      ...
      data: {
        kpis: {
          policyCoverage: writePerms.length > 0 ? Math.round(coveredWritePerms.length / writePerms.length * 1000) / 10 : 100,
...
async function getCoverage(req, res) {
  ...
  const response = {
    ...
    data: {
      ...
      covered,
      uncovered,
      coveragePercent: writePerms.length > 0 ? Math.round(covered.length / writePerms.length * 1000) / 10 : 100
    },
    meta: {
      lastUpdated: new Date().toISOString(),
...
async function getLogs(req, res) {
  try {
    ...
    const [logs, total] = await Promise.all([AuditLog.find(filter).sort({
      createdAt: -1
    }).skip(skip).limit(safeLimit).lean(), AuditLog.countDocuments(filter)]);
    return res.json({
      success: true,
      data: {
        logs: logs.map(log => ({
    ...
    logger.error({
      err
    }, "[Security] getLogs failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load access logs"
    });
  }
}
...
function simulateAccess(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] simulateAccess failed");
    return res.status(500).json({
      success: false,
...
async function exportLogs(req, res) {
  try {
    ...
    const rows = logs.map(log => [log.createdAt ? new Date(log.createdAt).toISOString() : "", `${log.actorFirstName || ""} ${log.actorLastName || ""}`.trim() || "System", log.actorRole || "unknown", log.action || "", log.entity || log.entityType || "", log.success ? "allowed" : "denied", (log.description || log.action || "").replace(/"/g, '""'), log.ipAddress || "", log.browser || ""]);
...
function exportPolicies(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] exportPolicies failed");
    return res.status(500).json({
      success: false,
...
async function getDenialStats(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getDenialStats failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load denial stats"
    });
  }
}
...
async function getAlertSummary(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getAlertSummary failed");
    return res.status(500).json({
      success: false,
...
async function acknowledgeAlert(req, res) {
  try {
    const alertsService = require("./securityAlerts.service");
    const alert = await alertsService.acknowledgeAlert(req.params.id, req.user._id, req.organizationId);
    if (!alert) {
    ...
    logger.error({
      err
    }, "[Security] acknowledgeAlert failed");
    return res.status(500).json({
      success: false,
...
async function resolveAlert(req, res) {
  try {
    const alertsService = require("./securityAlerts.service");
    const alert = await alertsService.resolveAlert(req.params.id, req.user._id, req.organizationId);
    if (!alert) {
      return res.status(404).json({
        success: false,
        message: "Alert not found"
...
async function getPolicyHistory(req, res) {
  try {
    ...
    const safePage = Math.max(Number(page) || 1, 1);
    const skip = (safePage - 1) * safeLimit;
    const [versions, total] = await Promise.all([PolicyVersion.find({}).sort({
      version: -1
    }).skip(skip).limit(safeLimit).lean(), PolicyVersion.countDocuments({})]);
    return res.json({
      success: true,
      data: {
        versions: versions.map(v => ({
          ...
          policyCount: v.policies ? Object.keys(v.policies).length : 0
        })),
        pagination: {
          page: safePage,
          limit: safeLimit,
    ...
    logger.error({
      err
    }, "[Security] getPolicyHistory failed");
    return res.status(500).json({
      success: false,
...
function getSecurityMetrics(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getSecurityMetrics failed");
    return res.status(500).json({
      success: false,
...
async function getEntitlementStatus(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getEntitlementStatus failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load entitlement status"
    });
  }
}
...
async function getAuthAnalytics(req, res) {
  try {
    ...
    const analytics = await analyticsService.getAuthAnalytics(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAuthAnalytics failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load auth analytics"
    });
  }
}
...
async function getAuthTraces(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getAuthTraces failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load auth traces"
    });
  }
}
...
async function getAuthTraceByRequestId(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getAuthTraceByRequestId failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load auth trace"
    });
  }
}
...
async function getAnalyticsSummary(req, res) {
  try {
    ...
    const data = await analyticsService.getSummary(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsSummary failed");
    return res.status(500).json({
      success: false,
...
async function getAnalyticsTimeline(req, res) {
  try {
    ...
    const data = await analyticsService.getTimeline(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsTimeline failed");
    return res.status(500).json({
      success: false,
...
async function getAnalyticsDistribution(req, res) {
  try {
    ...
    const data = await analyticsService.getDistribution(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsDistribution failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load analytics distribution"
    });
  }
}
...
async function getAnalyticsDeniedPermissions(req, res) {
  try {
    ...
    const data = await analyticsService.getDeniedPermissions(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsDeniedPermissions failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load denied permissions"
    });
  }
}
...
async function getAnalyticsRecentDenials(req, res) {
  try {
    ...
    const data = await analyticsService.getRecentDenials(req.organizationId, {
      startDate,
      endDate,
      limit
    });
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsRecentDenials failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load recent denials"
    });
  }
}
...
async function getAnalyticsRiskUsers(req, res) {
  try {
    ...
    const data = await analyticsService.getRiskUsers(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsRiskUsers failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load risk users"
    });
  }
}
...
async function getAnalyticsLayerPerformance(req, res) {
  try {
    ...
    const data = await analyticsService.getLayerPerformance(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsLayerPerformance failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load layer performance"
    });
  }
}
...
async function getAnalyticsFieldViolations(req, res) {
  try {
    ...
    const data = await analyticsService.getFieldViolations(req.organizationId, {
      startDate,
      endDate
    });
    return res.json({
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsFieldViolations failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load field violations"
    });
  }
}
...
async function getAnalyticsQueueHealth(req, res) {
  try {
    ...
    logger.error({
      err
    }, "[Security] getAnalyticsQueueHealth failed");
    return res.status(500).json({
      success: false,
      message: "Failed to load queue health"
    });
  }
}
