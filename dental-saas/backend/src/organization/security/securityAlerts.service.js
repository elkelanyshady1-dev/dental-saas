...
//   - SecurityAlert (./models/SecurityAlert) — tenant + req present but no _getModels(req) helper
/**
...
async function evaluateDenialThresholds(denial) {
  ...
  if (userId) {
    ...
    if (userDenials.length >= THRESHOLDS.BRUTE_FORCE_ATTEMPT.denials_per_user_per_minute) {
      await _createAlertIfNotDuped(organizationId, {
        ...
        details: {
          ...
          endpoints: [...new Set(userDenials.map(d => d.endpoint))]
        }
      });
    }
  }
  ...
  const adminPermissions = ["security.manage", "staff.manage", "users.delete", "branches.delete"];
  if (adminPermissions.includes(permission)) {
    const adminDenials = recentDenials.filter(d => adminPermissions.includes(d.permission));
    if (adminDenials.length >= THRESHOLDS.PRIVILEGE_ESCALATION.admin_denials_per_minute) {
      await _createAlertIfNotDuped(organizationId, {
        ...
        details: {
          ...
          users: [...new Set(adminDenials.map(d => d.userId))].filter(Boolean),
          permissions: [...new Set(adminDenials.map(d => d.permission))]
        }
      });
    }
  }
  ...
  if (criticalDenials.length >= THRESHOLDS.CRITICAL_DENIAL_SPIKE.critical_denials_per_minute) {
    await _createAlertIfNotDuped(organizationId, {
  ...
  if (unknownDenials.length >= THRESHOLDS.UNKNOWN_DENIAL_SPIKE.unknown_denials_per_minute) {
    await _createAlertIfNotDuped(organizationId, {
 ...
 */
async function recordPatientAccess({
  userId,
  patientId,
  organizationId
}) {
  ...
  const cutoff = Date.now() - WINDOW_SIZE_MS;
  const recentAccess = window.patientAccess.get(userId).filter(a => a.timestamp > cutoff);
  const uniquePatients = new Set(recentAccess.map(a => a.patientId));
  if (uniquePatients.size >= THRESHOLDS.SUSPICIOUS_ACCESS.unique_patients_per_minute) {
    await _createAlertIfNotDuped(organizationId, {
...
async function getAlerts(organizationId, options = {}, req = null) {
  ...
  const [alerts, total] = await Promise.all([Alert.find(filter).sort({
    createdAt: -1
  }).skip(skip).limit(safeLimit).lean(), Alert.countDocuments(filter)]);
  return {
    alerts,
    pagination: {
      page: safePage,
...
async function _createAlertIfNotDuped(organizationId, alertData) {
  const dedupKey = `${organizationId}:${alertData.type}:${Math.floor(Date.now() / DEDUP_COOLDOWN_MS)}`;
  ...
  try {
    ...
    logger.error({
      err
    }, "[SecurityAlerts] Failed to create alert");
    return null;
  }
}
...
function _topN(entries, field, n) {
  ...
  return Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, n).map(([name, count]) => ({
    name,
    count
  }));
}
