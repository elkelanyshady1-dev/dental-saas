...
policyVersionSchema.index({ organizationId: 1, version: -1 });
...
policyVersionSchema.pre(
    ["updateOne", "findOneAndUpdate", "replaceOne", "updateMany"],
    function () {
        throw new Error(
            "[PolicyVersion] Immutability violation: PolicyVersion entries are append-only and cannot be updated."
        );
    }
);
...
policyVersionSchema.pre(
    ...
    function () {
        throw new Error(
            "[PolicyVersion] Immutability violation: PolicyVersion entries are append-only and cannot be deleted."
        );
    }
);
