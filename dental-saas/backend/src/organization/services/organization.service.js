...
const { createContract } = require("@billing/services/contractEngine.service");
...
async function bootstrapOrganizationData({
    ...
    if (UserModel.db.name === "saasdental") {
        throw new Error(
            `RLS VIOLATION: bootstrapOrganizationData resolved UserModel on platform DB ` +
            `(expected dental_org_${organization._id}). Aborting to prevent cross-plane data leakage.`
        );
    }
    ...
    const [adminUser] = await UserModel.create([{
        ...
        roleId: roles.doctor?._id || roles.org_admin._id,  // fallback to org_admin if doctor not seeded
...
exports.provisionOrganization = async (data, platformUserId, ip, userAgent) => {
    const {
        ...
        trialPlanCode = "trial-tier",           // allow caller to override trial plan templateCode
        postTrialPlanVersionId,                  // optional: schedule paid contract after trial
    ...
    try {
        ...
        const REGION_MAP = {
            EG: "MEA", SA: "MEA", AE: "MEA",
            KW: "MEA", QA: "MEA", BH: "MEA", OM: "MEA",
            GB: "EU",
            US: "US",
        };
        ...
        logger.info(
            { country, derivedRegionCode, derivedBillingCurrency },
        ...
        if (!trialPlanVersion) {
            throw new Error(
                `[Provision] FATAL: NO_ACTIVE_TRIAL_PLAN_VERSION. ` +
                `No active PlanVersion found with templateCode='${trialPlanCode}'. ` +
                `Seed a PlanTemplate with code='${trialPlanCode}' and publish an active PlanVersion before provisioning.`
            );
        }
        ...
        if (postTrialPlanVersionId) {
            // @rls-platform-service — org creation/provisioning, no org-scoped req context
            selectedPlan = await PlanVersion.findById(postTrialPlanVersionId)
                .session(session)
                .lean();

            if (!selectedPlan) {
                throw new Error(
                    `[Provision] POST_TRIAL_PLAN_NOT_FOUND: PlanVersion ${postTrialPlanVersionId} not found.`
                );
            }
            if (selectedPlan.status !== "active") {
                throw new Error(
                    `[Provision] POST_TRIAL_PLAN_NOT_ACTIVE: PlanVersion ${postTrialPlanVersionId} ` +
                    `has status='${selectedPlan.status}'. Only active versions can be scheduled.`
                );
            }
        }
        ...
        await featureService.applyPlanFeatures(organization, { session });
        ...
        logger.info(
            { contractId: draftContract._id, orgId: organization._id },
        ...
        logger.info(
            { invoiceId: trialInvoice._id, orgId: organization._id },
            "[Provision] Trial $0 invoice created (pre-paid)"
        );

        // 9c. Activate contract — sets org.currentContractId + subscription.status
        ({ contract: activeContract, organization: updatedOrg } = await activateContract(
            draftContract._id,
            trialInvoice._id,
            { activatedBy: platformUserId, session }
        ));

        logger.info(
            { contractId: activeContract._id, currentContractId: updatedOrg.currentContractId },
        ...
        if (!updatedOrg.currentContractId) {
            throw new Error(
                `[Provision] FATAL: currentContractId is null after trial contract activation ` +
                `(orgId=${organization._id}, contractId=${activeContract._id}). ` +
                `This indicates a bug in contractActivation.service.`
            );
        }
        ...
        if (selectedPlan) {
            try {
                ...
                if (process.env.NODE_ENV === "production") {
                    throw new Error(
                        `PRICE_NOT_CONFIGURED_FOR_REGION: computePrice failed for PlanVersion ` +
                        `${selectedPlan._id} in region derived from country='${derivedBillingCountry}'. ` +
                        `Original error: ${priceErr.message}`
                    );
                }
                logger.warn(
                    { err: priceErr.message, planVersionId: selectedPlan._id, orgId: organization._id },
            ...
            if (preview.finalPrice == null) {
                if (process.env.NODE_ENV === "production") {
                    throw new Error(
                        `PRICE_NOT_CONFIGURED_FOR_REGION: computePrice returned null finalPrice ` +
                        `for PlanVersion ${selectedPlan._id} country='${derivedBillingCountry}'.`
                    );
                }
                logger.warn(
                    { planVersionId: selectedPlan._id, orgId: organization._id },
            ...
            await pendingContract.save({ session });
        ...
        logger.info({ orgId: organization._id }, `[Provision] Phase A committed — platform entities persisted for: ${organizationName}`);

    } catch (error) {
        try { await session.abortTransaction(); } catch (_) { /* already committed or ended */ }
        try { session.endSession(); } catch (_) { /* already ended */ }
        logger.error(
            { errName: error.name, errMsg: error.message?.substring(0, 300) },
    ...
    try {
        ...
        logger.info(
            { orgId, dbName: `dental_org_${orgId}` },
            "[Provision] Phase B — Per-org DB connection established"
        );

        const { branch, roles, adminUser } = await bootstrapOrganizationData({
            organization,
            organizationName,
        ...
        logger.info(
            { orgId, branchId: branch._id, adminUserId: adminUser._id, roleCount: Object.keys(roles).length },
        ...
        if (pendingContract && selectedPlan) {
            setImmediate(async () => {
                try {
                    ...
                    logger.warn(
                        { err: evtErr.message, orgId: organization._id },
        ...
        try {
            ...
            logger.error(
                { err: markErr.message, orgId },
                "[Provision] CRITICAL: Failed to mark org as PROVISION_FAILED"
            );
        }

        throw new Error(
            `Organization ${organizationName} was created in the platform DB but org bootstrap failed: ${bootstrapError.message}. ` +
            `The organization has been marked as PROVISION_FAILED. Please retry the bootstrap.`
        );
    } finally {
