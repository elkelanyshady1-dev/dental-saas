...
exports.getOrgProfile = async (req, res) => {
  try {
    ...
    const org = await Organization.findById(orgId).select("name slug logoUrl organizationSettings defaultLanguage createdAt").lean();
    if (!org) {
...
exports.uploadOrgLogo = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({
        success: false,
        message: "No file uploaded. Field name must be `logo`."
      });
    }
    const {
      buffer,
    ...
    if (!ALLOWED_MIME.has(mimetype)) {
      return res.status(422).json({
        success: false,
        message: "Invalid file type. Allowed: PNG, JPG, SVG."
      });
    }
    ...
    if (useS3) {
      ({
        logoUrl,
        logoKey
      } = await uploadToS3(buffer, mimetype, String(org._id)));
    } else {
      ({
        logoUrl,
        logoKey
      } = saveLocally(buffer, mimetype, String(org._id)));
    }
    ...
    if (err instanceof multer.MulterError) {
      const msg = err.code === "LIMIT_FILE_SIZE" ? "File size must be under 2 MB." : err.message;
      return res.status(422).json({
        success: false,
        message: msg
      });
