...
const {
  resolveCountry
} = require("@core/geo/countryResolver");
const {
  consumePricingToken,
...
exports.signup = async (req, res) => {
  try {
    ...
    const tokenResult = pricingToken ? await consumePricingToken(pricingToken) : {
      valid: false
    };
    ...
    if (!trialPlanVersion) {
      logger.error({
        service: "PublicController",
        action: "signup_trial_plan_missing"
      }, "FATAL: No active PlanVersion with templateCode='trial-tier' found. Cannot provision trial org.");
      return res.status(500).json({
        success: false,
        message: "No active trial plan configured. Please contact support."
      });
    }
    ...
    try {
      await session.withTransaction(async () => {
        ...
        [trialContract] = await OrgContract().create([{
          ...
          // placeholder, updated in Phase B
          pricingSnapshot: {
    ...
    try {
      ...
      try {
        ...
        logger.error({
          err: markErr.message,
          orgId
        }, "[Signup] Failed to mark org as PROVISION_FAILED");
      }
      return res.status(500).json({
        success: false,
        message: "Registration partially failed. Please contact support."
      });
    } finally {
      if (orgConn) {
        try {
    ...
    setImmediate(async () => {
      try {
        ...
        logger.warn({
          err: timelineErr.message,
          orgId: signupResult.orgId
  ...
  try {
    ...
    try {
      ...
      logger.error({
        err: dbError,
        service: "PublicController",
        action: "site_content_db_error"
      }, "DB error fetching site content");
    }
    ...
    logger.error({
      err: error,
      service: "PublicController",
      action: "get_site_content_fatal"
    }, "Fatal error in getSiteContent");
    return res.status(200).json({
      success: true,
...
exports.getPublicPlans = async (req, res) => {
  try {
    ...
    logger.error({
      err: error,
      service: "PublicController",
      action: "get_public_plans"
    }, "Error fetching public plans");
    return res.status(500).json({
      success: false,
...
exports.submitWhatsAppLead = async (req, res) => {
  try {
    ...
    }, "Error recording lead");
    return res.status(500).json({
      success: false,
      message: "Unable to record enquiry"
    });
  }
};