...
exports.updateOrganizationProfile = async (req, res) => {
    try {
        ...
        const FORBIDDEN = [
            "slug", "providerCustomerId", "providerSubscriptionId",
            "stripeCustomerId", "stripeSubscriptionId", // legacy field names also blocked
            "features", "subscription", "organizationId",
            "createdAt", "ownerId", "isVerified", "isActive",
        ];
        ...
        if (!name || name.length === 0) {
            return res.status(400).json({ success: false, message: "Name must not be empty or only whitespace." });
        }
        if (name.length < 3) {
            return res.status(400).json({ success: false, message: "Name must be at least 3 characters." });
        }
        if (name.length > 120) {
            return res.status(400).json({ success: false, message: "Name must be at most 120 characters." });
        }
