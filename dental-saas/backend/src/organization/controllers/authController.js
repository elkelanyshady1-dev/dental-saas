...
exports.loginUser = asyncHandler(async (req, res) => {
  ...
  const requestCountry = req.headers["cf-ipcountry"] || req.headers["x-edge-country"] || null;
  ...
  if (isPhoneVerificationRequired() && !organization.isVerified) {
    return res.status(403).json({
      success: false,
      message: "Account not verified. Please verify phone."
    });
  }
  const {
    accessToken,
    refreshToken,
    userProfile
  } = await authService.validateLogin({
  ...
  res.cookie("csrf_token", csrfToken, {
    httpOnly: false,
    // Must be accessible by frontend
    secure: isProdLogin,
    sameSite: isProdLogin ? "strict" : "lax",
    path: "/",
...
exports.refresh = asyncHandler(async (req, res) => {
  ...
  } = await authService.refreshToken(refreshToken, req.ip, req.headers["user-agent"], req.body.organizationId || null // BUG-1 FIX: Client sends orgId for O(1) lookup
  );
  const csrfToken = crypto.randomBytes(32).toString("hex");
...
exports.changePassword = asyncHandler(async (req, res) => {
  ...
  const {
    accessToken,
    refreshToken
  } = await authService.changePassword(req.user._id, currentPassword, newPassword, req.ip, req.context?.organizationId // Pass orgId for per-org model resolution
  );
...
exports.forgotPassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    }, "[Auth] Password reset verification engine failed");
  }
...
exports.resetPassword = asyncHandler(async (req, res) => {
  ...
  try {
    const result = await verificationEngine.verifyToken({
      purpose: "PASSWORD_RESET",
      identifier: "",
      // We don't know the email from the token alone
      rawToken: token
    });
    ...
    if (engineErr.statusCode !== 400) {
      logger.warn({
        err: engineErr.message
      }, "[Auth] Engine verify for password reset failed");
    }
  }
