...
exports.requestOtp = async (req, res) => {
    try {
        ...
        logger.debug(
            { phone: phone ? phone.slice(0, 6) + "****" : "MISSING", DEV_AUTH_MODE },
        ...
        const { country, isValid } = extractCountryFromPhone(phone);
        ...
        try {
            engineResult = await verificationEngine.requestVerification({
                purpose: "PHONE_OTP",
                identifier: phone,
                metadata: { email, name: email.split("@")[0], country },
            });
        } catch (engineErr) {
            ...
            logger.error({ err: engineErr.message, phone }, "[OTP] Verification engine requestVerification failed");
        ...
        logger.info({ phone: phone.slice(0, 6) + "****", country, channel: engineResult.channel }, "[OTP] OTP requested via Verification Engine");
        ...
        logger.error({ err: error, service: "OTPController", action: "request_otp" }, "OTP request failed");
        return res.status(500).json({ success: false, message: "Unable to send verification code." });
    }
};
...
exports.verifyOtp = async (req, res) => {
    try {
        ...
        logger.debug(
            { phone: phone ? phone.slice(0, 6) + "****" : "MISSING", otpLength: otp ? otp.length : 0, DEV_AUTH_MODE },
        ...
        if (DEV_AUTH_MODE && otp === "123456") {
            ...
            logger.warn(
                { phone: phone.slice(0, 6) + "****", country, region },
            ...
            if (devEmail) {
                try {
                    ...
                    await VerificationToken.updateMany(
                        { purpose: "EMAIL_VERIFY", identifier: devEmail.toLowerCase().trim(), isUsed: false },
                        { $set: { isUsed: true } }
                    );
                    ...
                    await VerificationToken.create({
                        ...
                        maxAttempts: 5,
                        isUsed: false,
                        metadata: { name: devEmail.split("@")[0], phone },
                    });
                    ...
                    emailOtpSent = true;
                    logger.warn(
                        { email: devEmail.slice(0, 4) + "****", otp: emailOtp },
        ...
        try {
            ...
            if (engineErr.statusCode === 429) {
                ...
                logger.error({ err: engineErr.message }, "[OTP] Verification Engine verify failed");
        ...
        if (email) {
            try {
                ...
                await VerificationToken.updateMany(
                    { purpose: "EMAIL_VERIFY", identifier: email.toLowerCase().trim(), isUsed: false },
                    { $set: { isUsed: true } }
                );
                ...
                await VerificationToken.create({
                    ...
                    maxAttempts: 5,
                    isUsed: false,
                    metadata: { name: email.split("@")[0], phone },
                });
        ...
        logger.error({ err: error, service: "OTPController", action: "verify_otp" }, "OTP verification failed");
...
exports.verifyEmailOtpPublic = async (req, res) => {
    try {
        ...
        logger.error({ err: error.message }, "[OTP] verifyEmailOtpPublic error");
        return res.status(500).json({ success: false, message: "Verification failed." });
    }
};
...
exports.resendEmailOtpPublic = async (req, res) => {
    try {
        ...
        await VerificationToken.create({
            ...
            maxAttempts: 5,
            isUsed: false,
            metadata: { name: normalizedEmail.split("@")[0] },
        });
        ...
        logger.error({ err: error.message }, "[OTP] resendEmailOtpPublic error");
