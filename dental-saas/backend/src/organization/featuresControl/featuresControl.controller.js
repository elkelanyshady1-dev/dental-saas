...
const TTL = {
    MODULES: 30,      // 30s — includes live counts
    FEATURES: 30,     // 30s — decision chains
    PERMISSIONS: 120,  // 2min — role matrix rarely changes mid-session
...
async function getModules(req, res) {
    try {
        ...
        const MODULE_KEYS = [
            { key: "patients", name: "Patients", icon: "👤", dependency: null },
            { key: "appointments", name: "Appointments", icon: "📅", dependency: "patients" },
            { key: "calendar", name: "Calendar", icon: "🗓️", dependency: "appointments" },
            { key: "treatments", name: "Clinical Treatments", icon: "🦷", dependency: "patients" },
            { key: "orthodontics", name: "Orthodontics", icon: "🔬", dependency: "patients" },
            { key: "finance", name: "Finance & Billing", icon: "💳", dependency: null },
            { key: "inventory", name: "Inventory", icon: "📦", dependency: null },
            { key: "lab", name: "Lab Management", icon: "🧪", dependency: null },
            { key: "communication", name: "Communication", icon: "💬", dependency: "patients" },
            { key: "analytics", name: "Analytics & Reports", icon: "📊", dependency: null },
            { key: "dashboard", name: "Dashboard", icon: "🏠", dependency: null },
            { key: "security", name: "Security Center", icon: "🛡️", dependency: null },
            { key: "portal", name: "Patient Portal", icon: "🌐", dependency: "patients" },
            { key: "users", name: "Staff Management", icon: "👥", dependency: null },
            { key: "branches", name: "Branches", icon: "🏢", dependency: null },
        ];
        ...
        try {
            const pipeline = [
                {
                    $match: {
                        organizationId: req.organizationId_obj || orgId,
        ...
        logger.error({ err }, "[FeaturesControl] getModules failed");
...
async function getFeatures(req, res) {
    try {
        ...
        const FEATURE_DEFS = [
            ...
            { key: "orthodontics.full", name: "Manage Cases (All Clinical Operations)", moduleKey: "orthodontics", module: "Orthodontics", risk: "medium", source: "plan" },
        ...
        const features = FEATURE_DEFS.map(feat => {
            ...
            // Decision chain
            const decisionChain = [
                { label: "Plan (Entitlement)", passed: moduleEnabled, detail: moduleEnabled ? "allowed" : "module locked" },
                { label: "Feature Flag", passed: flagEnabled, detail: flagEnabled ? "enabled" : "disabled by platform" },
                { label: "RBAC (Role)", passed: rbacGranted, detail: rbacGranted ? "granted" : "not in role" },
                { label: "Policy (Resource)", passed: true, detail: hasPolicyRules ? `${policies[feat.key].length} rules active` : "no restrictions" },
            ];
        ...
        logger.error({ err }, "[FeaturesControl] getFeatures failed");
...
async function getPermissions(req, res) {
    try {
        ...
        for (const role of dbRoles) {
            ...
            roles.push({ key: roleKey, name: roleName, id: role._id.toString() });
        ...
        const response = {
            success: true,
            data: { roles, permissions: uniquePermissions, matrix },
            meta: { lastUpdated: new Date().toISOString() },
        };
        ...
        logger.error({ err }, "[FeaturesControl] getPermissions failed");
...
async function getConflicts(req, res) {
    try {
        ...
        const modulePermMap = {
            ...
            orthodontics: ["orthodontics.full", "orthodontics.read"],  // Phase 30: two-permission model
        ...
        logger.error({ err }, "[FeaturesControl] getConflicts failed");
...
async function simulateDecision(req, res) {
    try {
        ...
        let policyResult = { allowed: true, reason: "no policy rules" };
        try {
            if (policies[permission] && policies[permission].length > 0) {
                const evalResult = evaluatePolicy(
                    permission,
                    { userId: user?._id, role: user?.role?.slug || user?.role },
                    resourceId ? { _id: resourceId } : null
                );
                policyResult = evalResult;
            }
        } catch {
            policyResult = { allowed: true, reason: "policy evaluation skipped" };
        }

        const steps = [
            { layer: "Entitlement", passed: entitlementPassed, detail: entitlementPassed ? `Module "${moduleKey}" is enabled` : `Module "${moduleKey}" is locked by plan` },
            { layer: "Feature Flag", passed: flagPassed, detail: flagPassed ? "No flag override" : "Disabled by platform feature flag" },
            { layer: "RBAC", passed: rbacPassed, detail: rbacPassed ? `Permission "${permission}" granted to role` : `Permission "${permission}" not in role` },
            { layer: "Policy (PBAC)", passed: policyResult.allowed !== false, detail: policyResult.reason || (policyResult.allowed ? "allowed" : "denied") },
        ];
        ...
        logger.error({ err }, "[FeaturesControl] simulateDecision failed");
...
async function toggleModule(req, res) {
    try {
        ...
        await cache.invalidate(
            ...
            featuresKey(req.organizationId),
            conflictsKey(req.organizationId)
        );
        ...
        logger.info({ orgId: req.organizationId, moduleKey: key, enabled }, "[FeaturesControl] Module toggled");

        return res.json({
            success: true,
            data: { moduleKey: key, enabled },
            message: `Module "${key}" ${enabled ? "enabled" : "disabled"} successfully`,
        });
    } catch (err) {
        logger.error({ err }, "[FeaturesControl] toggleModule failed");
