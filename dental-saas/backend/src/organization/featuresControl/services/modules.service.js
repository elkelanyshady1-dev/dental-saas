...
async function computeUsageStats(organizationId) {
    ...
    try {
        const pipeline = [
            { $match: { organizationId, createdAt: { $gte: todayStart } } },
        ...
        logger.warn({ err: err.message }, "[ModulesService] Usage stats aggregation failed — fallback to 0");
        return {};
    }
}
...
async function resolveModules(org) {
    ...
    const modules = MODULE_KEYS.map(mod => {
        ...
        const flagDisabled =
            orgFeatures[`${mod.key}.disabled`] === true ||
            orgFeatures[`DISABLE_${mod.key.toUpperCase()}`] === true;
 ...
 */
async function toggleModule({ orgId, key, enabled, user, req }) {
    // 1. Validate key
    const validKeys = MODULE_KEYS.map(m => m.key);
    if (!validKeys.includes(key)) {
        return { success: false, error: `Invalid module key: "${key}"`, code: 400 };
    }
    ...
    if (enabled) {
        const flagDisabled =
            orgFeatures[`${key}.disabled`] === true ||
            orgFeatures[`DISABLE_${key.toUpperCase()}`] === true;
        if (flagDisabled) {
            return {
                success: false,
    ...
    auditService.createAuditRecord({
        ...
        entity: "Organization",
        entityId: orgId,
        details: { moduleKey: key, enabled, affectedDependents },
        description: `Module "${key}" ${enabled ? "enabled" : "disabled"} by admin`
    }).catch(err => {
        logger.warn({ err: err.message, orgId, moduleKey: key }, "[ModulesService] Audit log failed (non-blocking)");
    });

    logger.info({ orgId, moduleKey: key, enabled, affectedDependents }, "[ModulesService] Module toggled");
...
async function getUsageAnalytics(organizationId, days = 7) {
    ...
    const entityMap = {
        ...
        Treatment: "treatments", Invoice: "finance",
        Payment: "finance", OrthodonticCase: "orthodontics",
        User: "users", Branch: "branches", Role: "security",
    };

    try {
        const pipeline = [
            { $match: { organizationId, createdAt: { $gte: startDate } } },
            { $group: { _id: { entity: "$entityType", day: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } } }, count: { $sum: 1 }, lastUsed: { $max: "$createdAt" } } },
        ];
        const results = await AuditLog.aggregate(pipeline).exec();
        ...
        for (const r of results) {
            ...
            if (!usage[moduleKey]) usage[moduleKey] = { total: 0, days: {}, lastUsed: null };
        ...
        return { success: true, data: usage, meta: { days, startDate: startDate.toISOString() } };
    } catch (err) {
        logger.error({ err }, "[ModulesService] getUsageAnalytics failed");
        return { success: false, data: {}, meta: { error: err.message } };
    }
}
