...
function simulateDecision({
  ...
  // ── Step 2: Feature Flag ───────────────────────────────────────────────
  const flagPassed = orgFeatures[`${moduleKey}.disabled`] !== true && orgFeatures[`DISABLE_${moduleKey.toUpperCase()}`] !== true;
  ...
  try {
    if (policies[permission] && policies[permission].length > 0) {
      policyResult = evaluatePolicy(permission, {
        user,
        userId: user?._id,
    ...
    logger.warn({
      err: err.message,
      permission
    }, "[InspectorService] Policy evaluation error");
    policyResult = {
      allowed: true,
      reason: "policy evaluation skipped (error)",
      effect: "no_policy",
      evaluatedRules: []
    };
  }
