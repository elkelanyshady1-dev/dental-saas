...
const { MODULE_REGISTRY } = require("../platform/featureRegistry");
...
function checkRegistryIntegrity() {
    ...
    const currentHash = crypto
        .createHash("sha256")
        .update(JSON.stringify(MODULE_REGISTRY))
        .digest("hex");
...
function enforceGlobalPrecedence(app) {
    ...
    if (!global.__ORG_RUNTIME_REGISTERED__) {
        throw new Error(
            "Sovereign Guard Violation: Org runtime routes were not registered via moduleLoader. " +
            "Direct route mounting is strictly prohibited. Exiting."
        );
    }
    ...
    if (!global.__PATIENT_AGGREGATE_ACTIVE__) {
        throw new Error(
            "Sovereign Guard Violation: Patient Aggregate routing is NOT active. " +
            "Direct mutation danger detected. Exiting."
        );
    }

    // 4. Verify Event Schema Registry (v3.1)
    const { SCHEMA_REGISTRY } = require("../eventContracts/schemaRegistry");
...
function checkDomainIsolation() {
    ...
    const legacyModelPaths = [
        path.join(__dirname, "../models/Appointment.js"),
    ];
