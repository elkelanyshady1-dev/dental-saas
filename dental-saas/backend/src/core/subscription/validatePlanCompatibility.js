...
async function validatePlanCompatibility(organizationId, newPlan, actorId) {
    ...
    if (isOverLimit) {
        ...
        await createAuditRecord({
            organizationId,
            branchId: "000000000000000000000000", // Platform/System constant
            actorId,
            ...
            details: {
                ...
                currentUsage: { users: userCount, branches: branchCount }
            },
            success: true
        });

        // Mark org as overLimit (future creations will be blocked)
        await Organization.findByIdAndUpdate(organizationId, { overLimit: true });
    } else {
        // Clear overLimit flag if moving to a compatible plan
        await Organization.findByIdAndUpdate(organizationId, { overLimit: false });
    }
...
module.exports = { validatePlanCompatibility };
