...
async function sendPlatformEmail({
  ...
  try {
    ...
    logger.error({
      err,
      service: "EmailService",
      action: "platform_email_failed",
      recipient: user.email
    }, "Failed to send platform email");
  }
}
...
async function sendEmailSafely({
  ...
  try {
    ...
    logger.error({
      err,
      service: "EmailService",
      action: "unhandled_preparation_error",
      orgId: org._id,
      type
    }, "Unhandled error preparing email");
    return null;
  }
}
...
exports.sendGraceEmail = async (org, contract = null) => {
  ...
  const cycleId = contract?.effectiveTo ? new Date(contract.effectiveTo).toISOString() : org._id.toString();
  return sendEmailSafely({
    org,
    invoiceId: null,
    type: "GRACE_STARTED",
...
exports.sendTemporaryPasswordEmail = async (user, tempPassword) => {
  ...
  if (user.organizationId) {
    ...
    const Organization = getPlatformModel(OrganizationDef); // @rls-platform-service — cross-org email delivery, no org-scoped req
