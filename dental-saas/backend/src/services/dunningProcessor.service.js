...
async function processDunningContracts() {
    ...
    logger.info({ action: "dunning_scan_start", now },
    ...
    const dueCAndContracts = await OrgContract.find({
        ...
        "dunning.retryCount": { $lt: settings.maxRetries }
    }).lean();

    logger.info({ count: dueCAndContracts.length },
        "[DunningProcessor] Contracts due for retry");

    const stats = { retried: 0, recovered: 0, exhausted: 0, errors: 0 };

    for (const contract of dueCAndContracts) {
        try {
            const result = await _processRetry(contract, { now, settings });
            if (result === "recovered") stats.recovered++;
            if (result === "retried") stats.retried++;
            if (result === "exhausted") stats.exhausted++;
        } catch (err) {
            stats.errors++;
            logger.error({ err, contractId: contract._id }, "[DunningProcessor] Retry error");
        }
    }
...
// ─── _processRetry ────────────────────────────────────────────────────────────
async function _processRetry(contract, { now, settings }) {
    ...
    if (!invoice) {
        ...
        await OrgContract.findByIdAndUpdate(contract._id, { $set: { dunning: null } });
        return "retried";
    }
    ...
    if (contract.paymentProvider && contract.providerSubscriptionId) {
        try {
            ...
            chargeResult = await provider.chargeSubscription({
                ...
                metadata: { contractId: contract._id.toString(), invoiceId: invoice._id.toString() }
            });
        } catch (err) {
            chargeError = err;
            logger.warn({ err, contractId: contract._id }, "[DunningProcessor] Charge attempt failed");
        }
    } else {
    ...
    if (chargeResult?.status === "succeeded" || chargeResult?.status === "paid") {
        return _onDunningRecovered(contract, invoice, { now, chargeResult });
    } else {
        return _onDunningRetryFailed(contract, invoice, { now, settings, chargeError });
    }
}

// ─── _onDunningRecovered ──────────────────────────────────────────────────────
async function _onDunningRecovered(contract, invoice, { now, chargeResult }) {
    ...
    try {
        ...
        if (!lc || !lInv) {
            await session.abortTransaction(); session.endSession(); return "retried";
        }
        ...
        await Organization.findByIdAndUpdate(lc.organizationId, {
            $set: { currentContractId: newContract._id, status: "active" }
        }, { session });
        ...
        await _audit({
            ...
            entityId: lInv._id,
            details: { oldContractId: lc._id, newContractId: newContract._id }
        }, session);
        ...
        try {
            await logBillingEvent({
                ...
                metadata: { oldContractId: lc._id.toString(), recoveredAt: now.toISOString() }
            });
            await logBillingEvent({
                organizationId: lc.organizationId,
                contractId: newContract._id,
                ...
                performedBy: SYSTEM_ACTOR,
                metadata: { source: "dunningProcessor", invoiceId: lInv._id.toString() }
            });
        } catch (auditErr) {
            logger.warn({ auditErr, contractId: newContract._id }, "[DunningProcessor] BillingAuditLog write failed (non-fatal)");
        }

        logger.info({ contractId: lc._id, invoiceId: lInv._id },
...
async function _onDunningRetryFailed(contract, invoice, { now, settings, chargeError }) {
    const currentRetryCount = (contract.dunning?.retryCount || 0);
    const nextRetryCount = currentRetryCount + 1;
    const failureReason = chargeError?.message || "provider_declined";
    const { retryScheduleDays, maxRetries } = settings;
    ...
    try {
        ...
        if (nextRetryCount >= maxRetries) {
            ...
            newDunning = {
                ...
                nextRetryAt: null,   // no more retries — grace enforcer will suspend
            ...
            const nextRetryAt = addDays(lc.dunning?.gracePeriodEndsAt
                ? new Date(lc.dunning.gracePeriodEndsAt.getTime() - (settings.gracePeriodDays * 86_400_000))
                : now, nextDayOffset);
        ...
        await _audit({
            ...
            entity: "PLATFORM_INVOICE",
            entityId: lInv._id,
            details: { retryCount: nextRetryCount, nextRetryAt: newDunning.nextRetryAt, reason: failureReason }
        }, session);
        ...
        try {
            ...
            logger.warn({ auditErr, contractId: lc._id }, "[DunningProcessor] BillingAuditLog RETRY_ATTEMPT write failed (non-fatal)");
        }

        logger.warn({ contractId: lc._id, retryCount: nextRetryCount },
...
async function reactivateOnPayment(invoiceId) {
    ...
    try {
        ...
        await _audit({
            ...
            details: { invoiceId, wasInDunning, wasSuspended, wasExpired }
        }, session);

        await session.commitTransaction(); session.endSession();
        logger.info({ contractId: contract._id, invoiceId },
            "[DunningProcessor] Contract reactivated on payment");

    } catch (err) {
        await session.abortTransaction(); session.endSession();
        logger.error({ err, contractId: contract._id }, "[DunningProcessor] Reactivation failed");
    }
}

// ─── Audit helper ─────────────────────────────────────────────────────────────
async function _audit({ organizationId, action, entity, entityId, details }, session = null) {
    try {
        ...
        logger.error({ err, action }, "[DunningProcessor] Audit write failed (non-fatal)");
    }
}

module.exports = { processDunningContracts, reactivateOnPayment };
