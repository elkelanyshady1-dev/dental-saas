...
exports.computeRevenueIntelligence = async () => {
    ...
    const mrrAgg = await RevenueSchedule.aggregate([
        ...
        { $sort: { createdAt: -1 } },
        {
            $group: {
                ...
                monthlyContrib: { $first: "$normalizedAmountPerPeriod" }
            }
        },
        {
            $group: {
                _id: null,
                mrr: { $sum: "$monthlyContrib" }
            }
        }
    ]);
    ...
    const deferredAgg = await RevenueSchedule.aggregate([
        { $match: { status: "active" } },
        {
            $group: {
                _id: null,
                deferredRevenue: { $sum: "$normalizedDeferredAmount" }
            }
        }
    ]);
    ...
    const riskAgg = await PlatformInvoice.aggregate([
        ...
        { $sort: { createdAt: -1 } },
        {
            $group: {
                _id: "$organizationId",
                latestAmount: { $first: "$totalAmount" }
            }
        },
        {
            $group: {
                _id: null,
                revenueAtRisk: { $sum: "$latestAmount" }
            }
        }
    ]);
    ...
    const activeOrgsInvoices = await PlatformInvoice.aggregate([
        ...
        { $sort: { createdAt: -1 } },
        {
            $group: {
                _id: "$organizationId",
                invoices: {
                    $push: { amount: "$totalAmount", createdAt: "$createdAt" }
                }
            }
        }
    ]);
    ...
    for (const log of churnLogs) {
        ...
        const lastPaid = await PlatformInvoice.findOne({
            ...
            status: "paid"
        }).sort({ createdAt: -1 }).select("totalAmount").lean();
    ...
    const activeContracts = await OrgContract.find({
        contractStatus: { $in: ["active", "in_grace"] },
        endDate: { $gte: now }
    })
        .select("organizationId lockedPrice endDate autoRenew pricingOverride appliedCoupon")
        .lean();

    for (const contract of activeContracts) {
        const daysUntilRenewal = (new Date(contract.endDate).getTime() - now.getTime())
            / (1000 * 60 * 60 * 24);
    ...
    logger.info(
        { mrr, arr, revenueAtRisk, churnRate: churnRate.toFixed(4), deferredRevenue },
