...
const CACHE_TTL = {
    ...
    TIMELINE:           30,   // 30s — chart data
    DISTRIBUTION:       30,   // 30s — donut chart
    DENIED_PERMISSIONS: 60,   // 1min — rarely changes fast
    RECENT_DENIALS:     10,   // 10s — near real-time
    RISK_USERS:         60,   // 1min — computed metric
    LAYER_PERFORMANCE:  60,   // 1min — structural metric
    FIELD_VIOLATIONS:   60,   // 1min — config-level metric
...
async function getTimeline(organizationId, options = {}) {
    ...
    const bucketFormat = msRange > 7 * 86400000
        ? "%Y-%m-%dT00:00:00Z"   // Daily buckets for > 7 days
    ...
    const result = await AuthTrace.aggregate([
        { $match: filter },
        {
            $group: {
                ...
                allowed: { $sum: { $cond: [{ $eq: ["$hasDenial", false] }, 1, 0] } },
                denied: { $sum: { $cond: [{ $eq: ["$hasDenial", true] }, 1, 0] } },
                avgDuration: { $avg: "$duration" },
            },
        },
        { $sort: { _id: 1 } },
        { $limit: 168 },
        {
            $project: {
                ...
                allowed: 1,
                denied: 1,
                avgDurationMs: { $round: [{ $ifNull: ["$avgDuration", 0] }, 0] },
            },
        },
    ]);
...
async function getDeniedPermissions(organizationId, options = {}) {
    ...
    // @rls-platform-analytics — cross-org metrics aggregation, no org-scoped req
    const result = await AuthTrace.aggregate([
        { $match: { ...filter, hasDenial: true } },
        { $unwind: "$steps" },
        { $match: { "steps.result": "DENY", "steps.permission": { $ne: null } } },
        {
            $group: {
...
async function getRecentDenials(organizationId, options = {}) {
    ...
    const cacheKey = _cacheKey("recent-denials", organizationId, { limit: safeLimit });
    const cached = await _getCache(cacheKey);
    if (cached) return cached;

    const filter = { organizationId, hasDenial: true };
    ...
    const traces = await AuthTrace.find(filter)
        .sort({ createdAt: -1 })
        .limit(safeLimit)
        .select("requestId method path userId role resourceType hasDenial denialLayer duration steps createdAt")
        .lean();
...
async function getRiskUsers(organizationId, options = {}) {
    ...
    // @rls-platform-analytics — cross-org metrics aggregation, no org-scoped req
    const result = await AuthTrace.aggregate([
        { $match: { ...filter, hasDenial: true, userId: { $ne: null } } },
        {
            $group: {
                _id: "$userId",
                denialCount: { $sum: 1 },
                role: { $first: "$role" },
                lastDenial: { $max: "$createdAt" },
        ...
        { $limit: 20 },
        {
            $project: {
                ...
                role: 1,
                lastDenial: 1,
                uniquePathCount: { $size: "$uniquePaths" },
            },
        },
    ]);
    ...
    const result = await AuthTrace.aggregate([
        ...
        { $unwind: "$steps" },
        {
            $group: {
                _id: "$steps.layer",
                total: { $sum: 1 },
                denied: { $sum: { $cond: [{ $eq: ["$steps.result", "DENY"] }, 1, 0] } },
                allowed: { $sum: { $cond: [{ $eq: ["$steps.result", "ALLOW"] }, 1, 0] } },
                avgDuration: { $avg: "$steps.durationMs" },
            },
        },
        { $sort: { total: -1 } },
        {
            $project: {
                ...
                denyRate: {
                    $cond: [
                        { $gt: ["$total", 0] },
                        { $round: [{ $multiply: [{ $divide: ["$denied", "$total"] }, 100] }, 2] },
                        0,
                    ],
                },
                avgDurationMs: { $round: [{ $ifNull: ["$avgDuration", 0] }, 1] },
            },
        },
    ]);
    ...
    const result = await AuthTrace.aggregate([
        {
            $match: {
                ...filter,
                hasDenial: true,
                "steps.layer": { $in: ["FIELD_READ", "FIELD_WRITE"] },
            },
        },
        { $unwind: "$steps" },
        {
...
async function getAuthAnalytics(organizationId, options = {}) {
    ...
    const [
        ...
        _getOverview(dateFilter),
        _getDenialsByLayer(dateFilter),
        _getDenialsByUser(dateFilter),
        _getDenialsByResource(dateFilter),
        _getDenialsByPath(dateFilter),
        _getHourlyTrend(dateFilter),
        _getPerformanceStats(dateFilter),
    ]);
...
async function _getDenialsByLayer(filter) {
    ...
    return result.map(r => ({ layer: r._id, count: r.count }));
}
...
async function _getDenialsByResource(filter) {
    // @rls-platform-analytics — cross-org metrics aggregation, no org-scoped req
    const result = await AuthTrace.aggregate([
        { $match: { ...filter, hasDenial: true, resourceType: { $ne: null } } },
        {
            $group: { _id: "$resourceType", count: { $sum: 1 } },
        },
        { $sort: { count: -1 } },
    ]);
    return result.map(r => ({ resourceType: r._id, count: r.count }));
}
...
async function _getHourlyTrend(filter) {
    ...
    const result = await AuthTrace.aggregate([
        { $match: filter },
        {
            $group: {
                ...
                totalRequests: { $sum: 1 },
                totalDenials: {
                    $sum: { $cond: [{ $eq: ["$hasDenial", true] }, 1, 0] },
                },
                avgDuration: { $avg: "$duration" },
            },
        },
    ...
    return result.map(r => ({
        ...
        denialRate: r.totalRequests > 0
            ? Math.round((r.totalDenials / r.totalRequests) * 10000) / 100
            : 0,
...
async function _getPerformanceStats(filter) {
    ...
    if (result.length === 0) {
        return { avgDurationMs: 0, maxDurationMs: 0, minDurationMs: 0, avgStepCount: 0, p95DurationMs: 0 };
    }
