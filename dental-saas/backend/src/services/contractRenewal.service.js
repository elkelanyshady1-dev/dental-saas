...
const PlatformConfig = require("../platform/models/PlatformConfig").default;
...
const { logBillingEvent } = require("../platform/billing/services/billingAuditLog.service");
...
const { writeLedgerEntry } = require("../platform/billing/models/BillingLedger.model");
...
function _buildLineItems(contract, price) {
    const { finalAmount, totalMinor, baseMinor, couponDiscount, validCoupon } = price;
    return {
        lineItems: [{
...
async function _writeAudit({ organizationId, action, entity, entityId, details }, session = null) {
    try {
        ...
        logger.error({ err, action }, "[ContractRenewal] Audit write failed (non-fatal)");
    }
}
...
async function renewExpiringContracts(opts = {}) {
    ...
    logger.info({ action: "renewal_scan_start", now, correlationId },
    ...
    logger.info({ count: expiredContracts.length, correlationId },
        "[ContractRenewal] Contracts due for renewal");

    const stats = { processed: 0, expired: 0, invoiced: 0, charged: 0, failed: 0, errors: 0 };

    for (const contract of expiredContracts) {
        stats.processed++;
        try {
            const result = await _processOne(contract, { now, settings, correlationId });
            stats[result] = (stats[result] || 0) + 1;
        } catch (err) {
            stats.errors++;
            logger.error({ err, contractId: contract._id, orgId: contract.organizationId, correlationId },
                "[ContractRenewal] Unhandled error");
        }
    }

    logger.info({ ...stats, correlationId }, "[ContractRenewal] Renewal scan complete");
    return stats;
}

// ─── _processOne ───────────────────────────────────────────────────────────────
async function _processOne(contract, { now, settings, correlationId }) {
    ...
    if (!contract.autoRenew || contract.contractStatus === "canceled") {
        await OrgContract.findByIdAndUpdate(contract._id, {
            $set: {
                ...
                terminatedAt: now,
                terminationReason: contract.autoRenew === false
                    ? "auto_renew_disabled"
                    : "contract_canceled"
            }
        });
        ...
        await _writeAudit({
            ...
            details: { reason: "autoRenew_false_or_canceled", correlationId }
        });
        ...
        try {
            ...
            logger.warn({ auditErr, contractId: contract._id, correlationId }, "[ContractRenewal] BillingAuditLog write failed (non-fatal)");
        }

        logger.info({ contractId: contract._id, correlationId }, "[ContractRenewal] Expired — autoRenew=false");
        return "expired";
    }
    ...
    // @rls-platform-cron — cross-org contract lifecycle processing, no org-scoped req
    const existing = await PlatformInvoice.findOne({ idempotencyKey }).lean();
    if (existing) {
        logger.info({ contractId: contract._id, invoiceId: existing._id, correlationId },
    ...
    if (contract.salesManaged) {
        return _handleSalesManaged(contract, { billingStart, billingEnd, price, idempotencyKey, now, gracePeriodEndsAt, settings, correlationId });
    }

    // Gate 3: Self-service → attempt charge
    return _handleSelfService(contract, { billingStart, billingEnd, price, idempotencyKey, now, gracePeriodEndsAt, settings, correlationId });
}

// ─── _handleSalesManaged ──────────────────────────────────────────────────────
async function _handleSalesManaged(contract, { billingStart, billingEnd, price, idempotencyKey, now, gracePeriodEndsAt, settings, correlationId }) {
    ...
    try {
        ...
        if (!lc || lc.contractStatus !== "active") {
            await session.abortTransaction(); session.endSession(); return "expired";
        }
        ...
        const invoice = new PlatformInvoice({
            ...
            nextRetryAt: null,   // sales-managed: no auto-retry
            idempotencyKey,
            metadata: new Map([
                ["renewalMode", "sales_managed"],
        ...
        await _writeAudit({
            ...
            details: { contractId: contract._id, totalAmountMinor: price.totalMinor, gracePeriodEndsAt, correlationId }
        }, session);

        await session.commitTransaction(); session.endSession();
        logger.info({ contractId: contract._id, invoiceId: invoice._id, correlationId },
...
// ─── _handleSelfService ───────────────────────────────────────────────────────
async function _handleSelfService(contract, { billingStart, billingEnd, price, idempotencyKey, now, gracePeriodEndsAt, settings, correlationId }) {
    // Attempt charge
    ...
    if (contract.paymentProvider && contract.providerSubscriptionId) {
        try {
            const { getProvider } = require("../platform/billing/providers/paymentProviderFactory");
            ...
            chargeResult = await provider.chargeSubscription({
                ...
                currency: contract.currency,
                metadata: { contractId: contract._id.toString(), idempotencyKey, correlationId }
            });
        } catch (err) {
            chargeError = err;
            logger.warn({ err, contractId: contract._id }, "[ContractRenewal] Charge failed");
        }
    } else {
        chargeError = new Error("No payment provider configured");
        logger.warn({ contractId: contract._id, correlationId }, "[ContractRenewal] No provider — entering dunning");
    }
    ...
    try {
        ...
        if (chargeResult?.status === "succeeded" || chargeResult?.status === "paid") {
            ...
            await OrgContract.findByIdAndUpdate(lc._id, {
                $set: { contractStatus: "superseded", supersededById: newContract._id, supersededAt: now }
            }, { session });

            await Organization.findByIdAndUpdate(lc.organizationId, {
                $set: { currentContractId: newContract._id }
            }, { session });

            if (price.validCoupon) {
                await OrgContract.findByIdAndUpdate(newContract._id,
                    { $inc: { "appliedCoupon.usedCount": 1 } }, { session });
            }
            ...
            await _writeAudit({
                ...
                entity: "PLATFORM_INVOICE",
                entityId: invoice._id,
                details: { oldContractId: lc._id, newContractId: newContract._id, totalAmountMinor: price.totalMinor }
            }, session);
            ...
            try {
                ...
                logger.error(
                    { err: ledgerErr, contractId: newContract._id, invoiceId: invoice._id },
            ...
            try {
                ...
                logger.warn({ auditErr, contractId: newContract._id }, "[ContractRenewal] BillingAuditLog write failed (non-fatal)");
            }

            await session.commitTransaction(); session.endSession();
            logger.info({ oldContractId: lc._id, newContractId: newContract._id, correlationId },
            ...
            await _writeAudit({
                ...
                entity: "PLATFORM_INVOICE",
                entityId: invoice._id,
                details: { contractId: lc._id, reason: failureReason, nextRetryAt: firstRetryAt, gracePeriodEndsAt }
            }, session);
            ...
            try {
                ...
                logger.warn({ auditErr, contractId: lc._id }, "[ContractRenewal] BillingAuditLog DUNNING_STARTED write failed (non-fatal)");
            }

            await session.commitTransaction(); session.endSession();
            logger.warn({ contractId: lc._id, invoiceId: invoice._id, correlationId },
