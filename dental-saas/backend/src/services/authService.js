...
const {
  resolveAuthority
} = require("@core/security/authorityBridge");
...
exports.registerUser = async (data, organizationId) => {
  ...
  const {
    User
  } = _resolveOrgModels(organizationId);
  ...
  const user = await User.create({
    ...
    // org-level registration — no platform escalation
...
exports.validateLogin = async ({
  ...
  } = require("@middleware/orgSubscriptionGuard");
  ...
  } = classifySubscriptionState(organization, activeContract);
  ...
  if (!organization.regionCode) {
    ...
    }, "Critical Sovereignty Breach: Organization missing regionCode during login");
...
exports.refreshToken = async (token, ipAddress, userAgent, organizationId) => {
  ...
  if (organizationId) {
    ...
    try {
      ...
      refreshDoc = await RefreshToken.findOne({
        tokenHash
      });
    } catch (err) {
      logger.warn({
        orgId,
        err: err.message
      }, "[Auth] Per-org DB lookup failed for refresh");
    }
  }
  ...
  if (!refreshDoc) {
    ...
    refreshDoc = await RefreshTokenGlobal.findOne({
      tokenHash
    });
    if (refreshDoc && refreshDoc.organizationId) {
      ...
      const perOrgDoc = await RefreshToken.findOne({
        tokenHash
      });
      if (perOrgDoc) {
    ...
    if (refreshDoc && isRevoked && refreshDoc.replacedByToken === null) {
      ...
      if (targetUser) {
        ...
        logger.warn({
          ...
          ip: ipAddress,
          userAgent
        }, "Session high-jacking attempt detected (re-use of revoked token). Global kill-switch triggered.");
      }
    }
  ...
  if (!regionCode) {
    logger.error({
      userId: user._id
    }, "Refresh failed: Could not determine regionCode for token re-issue");
  ...
  if (organizationId) {
    try {
      ...
      logger.warn({
        organizationId,
        err: err.message
      }, "[Auth] Per-org DB lookup failed for logout");
    }
  }
  ...
  if (!refreshDoc) {
    ...
    if (refreshDoc && refreshDoc.organizationId) {
      try {
        const {
          RefreshToken
        } = _resolveOrgModels(refreshDoc.organizationId);
        const perOrgDoc = await RefreshToken.findOne({
          tokenHash
        });
...
exports.forgotPassword = async (email, clinicCode) => {
  ...
  // Sliding-window rate limit (3 per 10 minutes per email)
  const {
    checkRateLimit
  } = require("./../../modules/auth/rateLimiter");
  await checkRateLimit("reset", email);
  const OrganizationDef = require("@shared/models/Organization");
  const Organization = getPlatformModel(OrganizationDef); // @rls-auth-flow — cross-org credential lookup, pre-authentication (no JWT context)
  ...
  // Single-use: remove any prior unexpired reset tokens for this user
  await PasswordResetToken.deleteMany({
    userId: user._id
  });

  // Reduced expiry: 15 minutes (was 1 hour)
  ...
  // Dispatch reset email via communication layer
  ...
  try {
    ...
    // Log but don't crash — user should not know if email delivery failed
    logger.error({
      userId: user._id,
      err: err.message
...
async function _buildOrgToken(user, org) {
  if (!org.regionCode) {
    throw Object.assign(new Error("Geopolitical resolution failure: Organization has no regionCode."), {
      statusCode: 500
    });
  }
 ...
 */
async function _buildOrgSession(user, org, {
  ipAddress,
  userAgent
} = {}) {
  if (!org.regionCode) {
  ...
  const {
    RefreshToken
  } = _resolveOrgModels(String(user.organizationId));
...
exports.smartLogin = async ({
  ...
  // @rls-auth-flow — cross-org credential lookup, pre-authentication (no JWT context)
  const lookupResults = await Promise.allSettled(candidateOrgs.map(async org => {
    const {
      User
    } = _resolveOrgModels(String(org._id));
    const user = await User.findOne({
      email: normalizedEmail,
      isActive: true,
      deletedAt: null
    }).select("+password").populate("roleId").lean();
    return user ? {
      user,
      org
    } : null;
  }));
  const rejections = lookupResults.map((r, i) => ({
    r,
    org: candidateOrgs[i]
  })).filter(x => x.r.status === "rejected");
  if (rejections.length) {
    logger.warn({
      event: "SMART_LOGIN_LOOKUP_FAILURES",
      count: rejections.length
    }, "[SmartLogin] Some org lookups failed during fan-out");
  }

  // Collect fulfilled non-null results (user found in that org)
  const candidates = lookupResults.filter(r => r.status === "fulfilled" && r.value !== null).map(r => r.value);
  ...
  if (validUsers.length === 1) {
    ...
    try {
      ({
        accessToken,
        refreshToken
      } = await _buildOrgSession(user, org, {
        ipAddress,
        userAgent
      }));
    } catch (sessionErr) {
      logger.error({
        event: "SMART_LOGIN_SESSION_FAIL",
        userId: user._id,
        err: sessionErr.message
      }, "[SmartLogin] Session build failed");
      throw sessionErr;
    }
    logger.info({
...
exports.selectOrg = async ({
  ...
  if (!email || !password || !organizationId) {
    throw Object.assign(new Error("email, password and organizationId are required"), {
      statusCode: 400
    });
  }
  const normalizedEmail = email.trim().toLowerCase();
  ...
  if (!user) {
    throw Object.assign(new Error("Invalid organization selection"), {
      statusCode: 401
    });
  }
  ...
  if (user.accountLockedUntil && user.accountLockedUntil > new Date()) {
    ...
    throw Object.assign(new Error(`Account temporarily locked. Try again in ${remainingMin} minutes.`), {
      statusCode: 423
    });
  }
  const isValid = await bcrypt.compare(password, user.password || "");
  ...
  if (!org) {
    throw Object.assign(new Error("Organization is no longer active"), {
      statusCode: 403
    });
  }
...
/**
 * Passwordless login — issue a full org session for a VERIFIED email.
 *
 * Intended caller: magic-link controller, AFTER the magic token has been
 * verified via magic.service.verifyMagicToken(). The magic token IS the
 * auth proof here; no password check is performed.
 *
 * Flow mirrors smartLogin's post-password branch:
 *   1. Fan-out across active orgs for this email.
 *   2. If exactly one match  → issue full session (access + refresh).
 *   3. If multiple matches   → return MULTI_ORG for client disambiguation.
 *   4. If no match           → 404.
 *
 * @param {string} email
 * @param {{ ipAddress?: string, userAgent?: string }} [meta]
 * @returns {Promise<
 *   | { type: "SINGLE_ORG", accessToken: string, refreshToken: string,
 *       organizationId: string, user: { id: string, email: string } }
 *   | { type: "MULTI_ORG", orgs: Array<{ organizationId, name, slug, logoUrl }> }
 * >}
 */
exports.issueOrgTokenByEmail = async (email, meta = {}) => {
  if (!email) {
  ...
  // @rls-auth-flow — cross-org lookup after passwordless proof of possession
  const candidateOrgs = await Organization.find({
    isActive: true,
    isArchived: false
  }).select("_id regionCode name slug logoUrl organizationSettings").lean();
  ...
  const matches = lookupResults.filter(r => r.status === "fulfilled" && r.value !== null).map(r => r.value);
  if (!matches.length) {
    throw Object.assign(new Error("No account found for this email"), {
      statusCode: 404
    });
  ...
  // Exclude locked accounts — magic link must not circumvent account lockout
  const unlocked = matches.filter(({
    user
  }) => !user.accountLockedUntil || user.accountLockedUntil <= new Date());
  if (!unlocked.length) {
    throw Object.assign(new Error("Account locked — try again later"), {
      statusCode: 423
    });
  }
  if (unlocked.length === 1) {
    ...
    logger.info({
      event: "MAGIC_LOGIN_SINGLE",
      userId: user._id,
      organizationId: user.organizationId
    }, "[MagicLogin] Single-org passwordless session issued");
    return {
      type: "SINGLE_ORG",
      accessToken,
      refreshToken,
  ...
  logger.info({
    event: "MAGIC_LOGIN_MULTI",
    email: normalizedEmail,
    orgCount: unlocked.length
  }, "[MagicLogin] Multi-org — returning org selector");
  return {
    type: "MULTI_ORG",
    orgs: unlocked.map(({
      user,
...
exports.resetPassword = async (token, newPassword, organizationId) => {
  ...
  if (organizationId) {
    try {
      ...
      logger.warn({
        organizationId,
        err: err.message
      }, "[Auth] Per-org DB lookup failed for password reset");
    }
  }
  ...
  if (resolvedOrgId) {
    ...
    const {
      User: OrgUser
    } = _resolveOrgModels(resolvedOrgId);
    ...
    if (user && user.organizationId) {
      const {
        User: OrgUser
      } = _resolveOrgModels(user.organizationId);
