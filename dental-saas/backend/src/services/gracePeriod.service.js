...
async function enforceGraceExpiration() {
    ...
    logger.info({ action: "grace_scan_start", now },
    ...
    logger.info({ count: overdue.length }, "[GracePeriod] Contracts past grace period");

    let suspended = 0, errors = 0;
    ...
    for (const contract of overdue) {
        try {
            ...
            errors++;
            logger.error({ err, contractId: contract._id }, "[GracePeriod] Enforcement error");
        }
    }

    logger.info({ suspended, errors }, "[GracePeriod] Grace expiration scan complete");
...
async function _enforceOne(contract, now) {
    ...
    try {
        ...
        await logBillingEvent({
            ...
            performedBy: SYSTEM_ACTOR,
            metadata: { reason: "grace_period_expired", suspendedAt: now.toISOString() }
        });
    } catch (auditErr) {
        logger.warn({ auditErr, contractId: contract._id }, "[GracePeriod] BillingAuditLog write failed (non-fatal)");
    }
...
// ─── Audit helper ─────────────────────────────────────────────────────────────
async function _audit({ organizationId, action, entity, entityId, details }) {
    try {
        ...
        logger.error({ err, action }, "[GracePeriod] Audit write failed (non-fatal)");
    }
}

module.exports = { enforceGraceExpiration };
