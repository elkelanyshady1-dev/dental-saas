...
class StageTriggerSubscriber {
    ...
    init() {
        eventBus.on("STAGE_COMPLETED", async (data) => {
            try {
                ...
                logger.error({ service: "StageTriggerSubscriber", error: err.message }, "Error triggering stage billing.");
            }
        });
    }
    ...
    async handleStageBilling(data) {
        const { organizationId, treatmentCaseId, stageName, order, operatorUserId, eventId } = data;

        if (!eventId) {
            logger.warn({ service: "StageTriggerSubscriber", stageName }, "Missing eventId. Skipping idempotency wrapper.");
    ...
    async _executeBillingLogic(organizationId, treatmentCaseId, stageName, session) {
        ...
        if (execution && execution.billingTrigger && !execution.billingExecuted) {
            ...
            logger.info({ service: "StageTriggerSubscriber", stageName }, "Triggering automated billing for stage.");
