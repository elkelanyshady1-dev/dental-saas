  ...
  init() {
    eventBus.on(FINANCIAL_SNAPSHOT_REQUESTED, async event => {
      try {
        ...
        logger.error({
          service: "FinancialSnapshotSubscriber",
          eventId: event.eventId,
          error: err.message
        }, "Error processing financial event.");
      }
    });
    logger.info({
      service: "FinancialSnapshotSubscriber"
  ...
  async handleEvent(event) {
    ...
    if (!eventId || !organizationId || !patientId) {
      logger.warn({
        service: "FinancialSnapshotSubscriber",
        event
      }, "Missing required fields for idempotency.");
      return;
    }
    await idempotencyService.process({
      ...
      handler: async session => {
        ...
        const eventMoney = amountMinor !== undefined ? Money.fromMinor(amountMinor, currency) : Money.fromDecimal(amount, currency);
        ...
        if (branchId) {
          ...
          if (!branchData) {
            branchData = {
              ...
              totalTreatmentRevenueMinor: 0,
              totalDiagnosticRevenue: 0,
              totalDiagnosticRevenueMinor: 0,
