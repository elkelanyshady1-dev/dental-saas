...
driftAlertSchema.index({ organizationId: 1, status: 1, createdAt: -1 });
driftAlertSchema.index({ organizationId: 1, type: 1, status: 1 });
 ...
 */
async function createAlert({ organizationId, severity, type, account, details, connection }) {
    ...
    if (existing) {
        ...
        existing.details = { ...existing.details, ...details, updatedCount: (existing.details?.updatedCount || 0) + 1 };
        await existing.save();
        return existing;
    }
...
async function getOpenAlerts(organizationId, connection = null) {
    ...
    return _getSecureDriftAlert(connection).find({ status: "open" })
        .sort({ createdAt: -1 })
        .lean();
}
...
async function acknowledgeAlert(alertId, connection = null) {
    const DriftAlert = _getDriftAlert(connection);
    return DriftAlert.findByIdAndUpdate(
        alertId,
...
async function getAlertStats(organizationId, connection = null) {
    ...
    const stats = { open: 0, acknowledged: 0, resolved: 0 };
