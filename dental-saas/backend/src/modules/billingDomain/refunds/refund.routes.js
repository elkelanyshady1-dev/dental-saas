...
const { processRefundSchema } = require("./refund.validator");
...
router.post(
    "/",
    requireOrgPermission(P.REFUNDS_CREATE),
    policyMiddleware(P.REFUNDS_CREATE),
    ...
    async (req, res, next) => {
        try {
            ...
            logger.info(
                { refundId: refund._id, paymentId, amount },
...
router.get(
    "/",
    requireOrgPermission(P.REFUNDS_READ),
    policyMiddleware(P.REFUNDS_READ),
    async (req, res, next) => {
        try {
            const { patientId, limit, skip } = req.query;
