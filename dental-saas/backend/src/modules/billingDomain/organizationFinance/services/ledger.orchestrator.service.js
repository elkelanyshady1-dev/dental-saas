...
const { deriveInvoiceStatus } = require("./invoiceStatus.service");
...
class FinancialOrchestrator {
    ...
    async createInvoice(params, req) {
        ...
        try {
            ...
            const patientExists = await patientReadService.existsPatient({ rls: { organizationId } }, patientId, session);
            ...
            let totalMoney = subtotalMoney
                .subtract(discountMoney)
                .subtract(insuranceMoney)
                .add(taxMoney);
            ...
            try {
                ...
                logger.error({ err: journalErr, invoiceId: invoice._id }, "[Orchestrator] Journal entry failed for invoice — queued for retry");
    ...
    async recordPayment(params, req) {
        const { organizationId, branchId, patientId, invoiceId, amount, paymentMethod, collectedByUserId, expectedVersion, isInternalEvent } = params;
        const currency = "AED";
        ...
        try {
            ...
            if (invoiceId) {
                ...
                const invoice = await PatientInvoice.findOne({ _id: invoiceId }).session(session);
                ...
                const result = await PatientInvoice.updateOne(
                    { _id: invoice._id, version: targetVersion },
                    {
            ...
            try {
                ...
                logger.error({ err: journalErr, paymentId: payment._id }, "[Orchestrator] Journal entry failed for payment — queued for retry");
                await journalRetryService.enqueue({
                    organizationId,
    ...
    async voidInvoice(params, req) {
        const { organizationId, invoiceId, voidedByUserId, voidedReason, expectedVersion, isInternalEvent } = params;
        ...
        try {
            ...
            try {
                ...
                logger.error({ err: journalErr, invoiceId: invoice._id }, "[Orchestrator] Journal entry failed for void — queued for retry");
                await journalRetryService.enqueue({
                    organizationId,
                    referenceType: "void",
                    referenceId: invoice._id,
                    payload: { invoiceId: invoice._id, voidedByUserId },
                    error: journalErr,
                });
            }
    ...
    async creditWallet(params) {
        const { organizationId, patientId, amount, branchId, performedByUserId } = params;
        const currency = "AED";
        const amountMoney = Money.fromDecimal(amount, currency);
