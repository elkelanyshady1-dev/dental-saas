...
const patientInvoiceSchema = new mongoose.Schema(
    {
        ...
        treatments: [
            {
                treatmentId: { type: mongoose.Schema.Types.ObjectId },
        ...
        insuranceCoveredMinor: { type: Number, default: 0 },
        totalAmountMinor: { type: Number },
        downpaymentAmountMinor: { type: Number, default: 0 },
        currency: { type: String, required: true, default: "AED" }, // Default for patient domain in this region
    ...
    if (this.isModified() && !this.isNew) {
        ...
        if (immutableStatuses.includes(this.status)) {
            const monetaryFields = [
                "subtotal", "tax", "discount", "insuranceCovered", "totalAmount",
                "subtotalMinor", "taxMinor", "discountMinor", "insuranceCoveredMinor", "totalAmountMinor"
            ];
...
patientInvoiceSchema.index({ invoiceNumber: 1 }, { unique: true, sparse: true });
patientInvoiceSchema.index({ patientId: 1 });
patientInvoiceSchema.index({ branchId: 1 });
patientInvoiceSchema.index({ status: 1 });
patientInvoiceSchema.index({ createdAt: -1 });
patientInvoiceSchema.index({ regionCode: 1, createdAt: 1 });
