...
const journalLineSchema = new mongoose.Schema({
  account: {
    type: String,
    required: true,
    enum: ["accounts_receivable", "cash", "revenue", "refunds", "insurance_receivable", "wallet_liability", "discount_expense"]
  },
  type: {
    type: String,
    required: true,
...
journalEntrySchema.pre("validate", function (next) {
  ...
  if (debitMinor !== creditMinor) {
    return next(new Error(`Journal entry unbalanced: debit(${debitMinor}) ≠ credit(${creditMinor}) in minor units.`));
  }
