...
class PatientUpdateService {
     ...
     */
    async updatePatient({ regionCode, organizationId, actorId, patientId, data, ipAddress, expectedVersion, isInternalEvent = false, auditContext = {}, req }) {
        await withRegionalTransaction(regionCode, async (session, models) => {
            ...
            const ALLOWED_FIELDS = [
                "firstName", "middleName", "lastName", "nameArabic", "nameEnglish",
                "phone", "secondaryPhone", "email",
                "gender", "dateOfBirth", "address",
                "nationality", "nationalId", "maritalStatus",
                "job", "photo", "insurance", "emergencyContact",
                "status",
            ];
            ...
            for (const field of ALLOWED_FIELDS) {
                if (data[field] !== undefined && data[field] !== patient[field]) {
                    changes[field] = { from: patient[field], to: data[field] };
     ...
     */
    async changePrimaryBranch({ regionCode, organizationId, actorId, patientId, newBranchId, ipAddress, expectedVersion, isInternalEvent = false, auditContext = {}, req }) {
        await withRegionalTransaction(regionCode, async (session, models) => {
            const { Patient } = models;
            ...
            const result = await Patient.updateOne(
                { _id: patientId, version: targetVersion },
                { $set: { primaryBranchId: newBranchId, allowedBranchIds: patient.allowedBranchIds }, $inc: { version: 1 } },
                { session }
            );
            if (result.modifiedCount === 0) {
                throw new VersionConflictError("Aggregate version mismatch");
            ...
            await eventBus.emitViaOutbox(
                PATIENT_BRANCH_UPDATED,
                {
                    organizationId, patientId, actorId, newBranchId,
                    _audit: {
                        action: "PATIENT_BRANCH_UPDATED",
                        entityId: patientId,
                        metadata: { from: oldBranchId, to: newBranchId, type: "PRIMARY" },
                        ipAddress,
                        regionCode: auditContext.regionCode || regionCode,
                        branchId: auditContext.branchId,
                        actorId,
     ...
     */
    async updateBranchAccess({ regionCode, organizationId, actorId, patientId, allowedBranchIds, ipAddress, expectedVersion, isInternalEvent = false, auditContext = {}, req }) {
        await withRegionalTransaction(regionCode, async (session, models) => {
            const { Patient } = models;
            ...
            const result = await Patient.updateOne(
                { _id: patientId, version: targetVersion },
                { $set: { allowedBranchIds: newAllowed }, $inc: { version: 1 } },
                { session }
            );
            if (result.modifiedCount === 0) {
                throw new VersionConflictError("Aggregate version mismatch");
            ...
            await eventBus.emitViaOutbox(
                PATIENT_BRANCH_UPDATED,
                {
                    ...
                    _audit: {
                        action: "PATIENT_BRANCH_UPDATED",
                        entityId: patientId,
                        metadata: { from: oldAccess, to: newAllowed, type: "ACCESS_LIST" },
                        ipAddress,
                        regionCode: auditContext.regionCode || regionCode,
                        branchId: auditContext.branchId,
                        actorId,
     ...
     */
    async setPortalEnabled({ regionCode, organizationId, patientId, enabled, expectedVersion, isInternalEvent = false, req }) {
        await withRegionalTransaction(regionCode, async (session, models) => {
            const { Patient } = models;
            ...
            const result = await Patient.updateOne(
                { _id: patientId, version: targetVersion },
                { $set: { portalEnabled: enabled }, $inc: { version: 1 } },
                { session }
            );
            if (result.modifiedCount === 0) {
                throw new VersionConflictError("Aggregate version mismatch");
