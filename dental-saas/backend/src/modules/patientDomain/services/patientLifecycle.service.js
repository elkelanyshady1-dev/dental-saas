...
class PatientLifecycleService {
     ...
     */
    async softDeletePatient({ regionCode, organizationId, actorId, patientId, reason, ipAddress, expectedVersion, isInternalEvent = false, auditContext = {}, req }) {
        let deletedAt;

        await withRegionalTransaction(regionCode, async (session, models) => {
            const { Patient, PatientUser } = models;
            ...
            const result = await Patient.updateOne(
                ...
                { $set: { isActive: false, deletedAt }, $inc: { version: 1 } },
                { session }
            );
        ...
        try { await orgUsageService.decrementPatients(organizationId); } catch (_) { /* logged inside service */ }
     ...
     */
    async changeStatus({ regionCode, organizationId, actorId, patientId, isActive, ipAddress, expectedVersion, isInternalEvent = false, auditContext = {}, req }) {
        await withRegionalTransaction(regionCode, async (session, models) => {
            const { Patient, PatientUser } = models;
            ...
            await eventBus.emitViaOutbox(
                PATIENT_STATUS_CHANGED,
                {
                    organizationId, patientId, actorId, isActive,
                    _audit: {
                        action: "PATIENT_STATUS_CHANGED",
                        entityId: patientId,
                        metadata: { from: oldStatus, to: isActive },
                        ipAddress,
                        regionCode: auditContext.regionCode || regionCode,
                        branchId: auditContext.branchId,
                        actorId,
     ...
     */
    async assignDoctor({ regionCode, organizationId, actorId, patientId, doctorId, ipAddress, auditContext = {}, req }) {
        await withRegionalTransaction(regionCode, async (session, models) => {
            ...
            await eventBus.emitViaOutbox(
                DOCTOR_ASSIGNED_TO_PATIENT,
                {
                    organizationId, patientId, doctorId,
                    _audit: {
