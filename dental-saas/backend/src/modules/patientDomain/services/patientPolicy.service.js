...
class PatientPolicyService {
     ...
     */
    async updatePolicy({ regionCode, organizationId, actorId, data, ipAddress, expectedVersion, isInternalEvent = false, auditContext = {}, req }) {
        let policy;

        await withRegionalTransaction(regionCode, async (session, models) => {
            const { PatientPolicy } = models;
            ...
            if (!policy) {
                ...
                const result = await PatientPolicy.updateOne(
                    { _id: policy._id, version: targetVersion },
