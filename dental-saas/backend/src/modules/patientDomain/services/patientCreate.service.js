...
class PatientCreateService {
     ...
     */
    async createPatient({ regionCode, organizationId, actorId, data, ipAddress, auditContext = {}, req }) {
        if (!req?.dbConnection) {
        ...
        const { nameArabic, nameEnglish } = _detectNameLanguage(data);

        // 2. Phone Normalization
        const { phoneE164, phoneDigits, phoneRaw } = _normalizePhone(data.phone, data.country);
        ...
        try {
            await session.withTransaction(async () => {
                ...
                if (customCode) {
                    const exists = await Patient.findOne({ patientCode }).session(session).lean();
                    if (exists) {
                ...
                if (!customCode) {
                    ...
                    const branch = await BranchModel.findById(data.primaryBranchId)
                        .select("name clinicType").lean();
                ...
                await eventBus.emitViaOutbox(
                    PATIENT_CREATED,
                    {
                        ...
                        _audit: {
                            ...
                            metadata: { patientCode, branchId: data.primaryBranchId },
                            ipAddress,
                    ...
                    { session, EventOutboxModel: EventOutbox }
                );
            }, {
        ...
        try { await orgUsageService.incrementPatients(organizationId); } catch (_) { /* logged inside service */ }

        return { patientId: createdPatientId, patientCode };
    }
}
...
function _detectNameLanguage(data) {
    ...
    if (!name) return { nameArabic: null, nameEnglish: null };
