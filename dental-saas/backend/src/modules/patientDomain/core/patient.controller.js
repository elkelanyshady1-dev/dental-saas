...
const { createPatientSchema, updatePatientSchema } = require("../../../validation/patient.schema");
...
class PatientController {
    async getProfile(req, res) {
        try {
            ...
            const accessCheck = await Patient.countDocuments({ _id: patientId });
            if (accessCheck === 0) {
            ...
            const aggregate = await patientService.getPatientAggregate({ organizationId, patientId, req });
    ...
    async delete(req, res) {
        try {
            ...
            return successResponse(res, { message: "Patient deleted successfully" });
        } catch (error) {
    ...
    async list(req, res) {
        try {
            ...
            const result = await patientListService.listPatients({
                req,            // ← FIXED: was missing, caused ReferenceError
                scopedQuery: {},
                search,
                sort,
                page,
