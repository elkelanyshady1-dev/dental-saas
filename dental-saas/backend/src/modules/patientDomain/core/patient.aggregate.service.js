...
const { buildPatientCoreDTO } = require("../../../dto/patient.dto");
...
class PatientAggregateService {
     ...
     */
    async createPatient({ organizationId, actorId, data, ipAddress, req }) {
        ...
        return this.getPatientAggregate({ organizationId, patientId: result.patientId, req });
    }

    /**
     * UPDATE ARCHETYPE — delegates to patientUpdate.service
     */
    async updatePatient({ organizationId, actorId, patientId, data, ipAddress, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
        ...
        return this.getPatientAggregate({ organizationId, patientId, req });
    }

    /**
     * BRANCH GOVERNANCE — delegates to patientUpdate.service
     */
    async changePrimaryBranch({ organizationId, actorId, patientId, newBranchId, ipAddress, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
    ...
    async updateBranchAccess({ organizationId, actorId, patientId, allowedBranchIds, ipAddress, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
     ...
     */
    async setPortalEnabled({ organizationId, patientId, enabled, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
     ...
     */
    async softDeletePatient({ organizationId, actorId, patientId, reason, ipAddress, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
     ...
     */
    async changeStatus({ organizationId, actorId, patientId, isActive, ipAddress, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
     ...
     */
    async updateMedicalHistory({ organizationId, actorId, patientId, medicalHistory, notes, ipAddress, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
     ...
     */
    async updatePolicy({ organizationId, actorId, data, ipAddress, expectedVersion, isInternalEvent = false, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
     ...
     */
    async assignDoctor({ organizationId, actorId, patientId, doctorId, ipAddress, req }) {
        const regionCode = _resolveRegion(req);
        const auditContext = _resolveAuditContext(req);
     ...
     */
    async getPatientAggregate({ organizationId, patientId, req }) {
        const { Patient, ClinicalRecord, PatientPolicy } = _getModels(req);

        const patient = await Patient.findById(patientId).lean();
        if (!patient) throw new Error("Patient not found.");

        const clinical = await ClinicalRecord.findOne({ patientId }).lean();
        ...
        if (clinical?.medicalHistory?.allergies?.length > 0) alerts.push({ type: "MEDICAL_ALLERGY", data: clinical.medicalHistory.allergies });
        if (clinical?.medicalHistory?.chronicConditions?.length > 0) alerts.push({ type: "CHRONIC_CONDITION", data: clinical.medicalHistory.chronicConditions });
        ...
        const COMPLETION_FIELDS = [
            { key: "dateOfBirth",     label: "Date of Birth",       weight: 15, check: v => !!v },
            { key: "gender",          label: "Gender",              weight: 10, check: v => !!v },
            { key: "address",         label: "Address",             weight: 10, check: v => !!v },
            { key: "email",           label: "Email",               weight: 10, check: v => !!v },
            { key: "nationality",     label: "Nationality",         weight: 10, check: v => !!v },
            { key: "nationalId",      label: "National ID",         weight: 10, check: v => !!v },
            { key: "emergencyContact",label: "Emergency Contact",   weight: 20, check: v => !!(v?.name && v?.phone) },
            { key: "insurance",       label: "Insurance",           weight: 15, check: v => !!(v?.provider) },
        ];
