  ...
  try {
    ...
    if (phone) {
      ...
      if (digits.length >= 6) {
        ...
        }).select("nameEnglish nameArabic phone phoneDigits patientCode gender dateOfBirth primaryBranchId createdAt").limit(parseInt(limit)).lean();
    ...
    if (name && name.trim().length >= 2) {
      ...
      }).select("nameEnglish nameArabic phone phoneDigits patientCode gender dateOfBirth primaryBranchId createdAt").limit(parseInt(limit) * 2).lean();
...
async function linkFamily(req, res) {
  try {
    ...
    const Patient = _getPatient(req);
    const [patient, familyMember] = await Promise.all([Patient.findOne({
      _id: patientId,
      isActive: true
    }), Patient.findOne({
      _id: familyMemberId,
    ...
    const alreadyLinked = patient.familyMembers?.some(fm => fm.patientId.toString() === familyMemberId);
    if (alreadyLinked) {
    ...
    await Promise.all([Patient.updateOne({
      _id: patientId
    }, {
      $push: {
        familyMembers: {
    ...
    // 6. Event
    eventBus.emit("patient.family.linked", {
      patientId,
      familyMemberId,
      relationship
    });
    ...
    const updatedPatient = await _getPatient(req).findOne({
      _id: patientId
    }).populate("familyMembers.patientId", "nameEnglish nameArabic phone patientCode gender").lean();
...
async function getFamilyMembers(req, res) {
  try {
    ...
    const patient = await _getPatient(req).findOne({
      _id: patientId,
      isActive: true
    }).populate("familyMembers.patientId", "nameEnglish nameArabic phone patientCode gender dateOfBirth").lean();
    if (!patient) return errorResponse(res, "Patient not found", "NOT_FOUND", 404);
    return successResponse(res, {
