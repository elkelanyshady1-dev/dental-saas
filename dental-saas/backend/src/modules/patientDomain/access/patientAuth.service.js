...
class PatientAuthService {
    async activatePortal({ inviteToken, otp, password }) {
        ...
        // @rls-auth-flow
        const invite = await PortalInvite.findOne({ tokenHash, usedAt: null });
    ...
    async login({ clinicCode, email, password, OrganizationModel }) {
        // @rls-auth-flow — Organization lookup is pre-auth (no JWT context)
        const org = await OrganizationModel.findOne({ slug: clinicCode, isActive: true });
        ...
        if (!patientUser || !patientUser.isActive) {
            eventBus.emit(PATIENT_LOGIN_FAILED, { organizationId: org._id, email });
