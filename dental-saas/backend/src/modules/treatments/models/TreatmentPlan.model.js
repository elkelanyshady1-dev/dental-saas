...
treatmentPlanSchema.pre("save", function () {
    if (this.isModified("planItems")) {
        this.estimatedTotal = this.planItems.reduce((sum, item) => {
            return item.status !== "cancelled" && item.status !== "declined"
...
treatmentPlanSchema.index({ branchId: 1, status: 1 });
...
module.exports = {
    ...
    default: mongoose.models[modelName] || mongoose.model(modelName, treatmentPlanSchema),
};
