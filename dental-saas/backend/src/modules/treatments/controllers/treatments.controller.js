...
async function listTreatments(req, res) {
    try {
        ...
        const { patientId, appointmentId, status, branchId, treatmentPlanId, page = 1, limit = 50 } = req.query;
        const result = await treatmentService.listTreatments({
            organizationId: req.context.organizationId,
            filters: { patientId, appointmentId, status, branchId, treatmentPlanId },
            page: parseInt(page),
            limit: Math.min(parseInt(limit), 100)
        });
        return res.json({ success: true, data: result.treatments, pagination: result.pagination });
    } catch (err) {
...
async function listTreatmentPlans(req, res) {
    try {
        ...
        return res.json({ success: true, data: result.plans, pagination: result.pagination });
    } catch (err) {
        return res.status(err.statusCode || 500).json({ success: false, error: { code: "LIST_ERROR", message: err.message } });
    }
}
