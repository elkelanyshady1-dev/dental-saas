...
function validateTreatmentTransition(current, next) {
  ...
  if (!allowed) return {
    valid: false,
    message: `Unknown current status: ${current}`
  };
...
class TreatmentService {
  ...
  async listTreatments({
    ...
    const [treatments, total] = await Promise.all([Treatment.find(query).populate("procedureId", "code name category defaultPrice").populate("performedBy", "name email").sort({
      createdAt: -1
    }).skip(skip).limit(limit).lean(), Treatment.countDocuments(query)]);
    return {
      treatments,
      pagination: {
        page,
  ...
  async getTreatmentById({
    ...
    const treatment = await Treatment.findById(treatmentId).populate("procedureId", "code name category defaultPrice").populate("performedBy", "name email").populate("createdBy", "name email").lean();
    if (!treatment) {
  ...
  async listTreatmentPlans({
    ...
    if (filters.status) query.status = filters.status;
    const skip = (page - 1) * limit;
    const [plans, total] = await Promise.all([TreatmentPlan.find(query).sort({
      createdAt: -1
    }).skip(skip).limit(limit).lean(), TreatmentPlan.countDocuments(query)]);
    return {
      plans,
      pagination: {
        page,
  ...
  async getTreatmentPlanById({
    ...
    const plan = await TreatmentPlan.findById(planId).populate("planItems.procedureId", "code name category").populate("createdBy", "name email").populate("approvedBy", "name email").lean();
    if (!plan) {
