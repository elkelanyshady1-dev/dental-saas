 ...
 */
async function listByStatus(conn, {
  organizationId,
  branchId,
  status,
  page = 1,
  ...
  const [data, total] = await Promise.all([BookingRequest.find(query).sort({
    requestedDate: 1,
    requestedTime: 1
  }).skip(skip).limit(limit).lean(), BookingRequest.countDocuments(query)]);
  return {
    data,
    pagination: {
      total,
...
async function getById(conn, {
  ...
  if (!request) {
    const e = new Error("Booking request not found");
    e.statusCode = 404;
    throw e;
  }
  return request;
