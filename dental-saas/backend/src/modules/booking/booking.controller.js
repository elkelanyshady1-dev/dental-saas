...
const { SLOTS_VIEWED } = require("../../core/domainEvents");
...
class BookingController {
    ...
    async getSlots(req, res) {
        try {
            ...
            eventBus.emit(SLOTS_VIEWED, { organizationId, branchId, patientId: req.user.patientId });
    ...
    async executeBooking(req, res) {
        try {
            const { branchId, date, time, notes } = req.body;
            const { organizationId, patientId } = req.user;
