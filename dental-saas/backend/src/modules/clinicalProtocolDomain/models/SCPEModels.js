...
const protocolDefinitionSchema = new mongoose.Schema({
    ...
    specialty: { type: String, required: true }, // e.g., "orthodontics", "endodontics"
    ...
    stages: [{
        ...
        requiredMedia: [{
            slot: { type: String }, // e.g., "Pan-Xray", "Ceph"
            tag: { type: String }
        }]
    }],
    rules: [{
        type: { type: String }, // e.g., "RISK_CHECK", "MEDIA_LOCK"
...
const clinicalCaseSchema = new mongoose.Schema({
    ...
    protocolId: { type: mongoose.Schema.Types.ObjectId, ref: "ProtocolDefinition", required: true },
    status: { type: String, enum: ["ACTIVE", "COMPLETED", "ON_HOLD", "CANCELLED"], default: "ACTIVE" },
    currentStageOrder: { type: Number, default: 0 },
    extensionData: { type: mongoose.Schema.Types.Mixed }, // Storage for specialty-specific fields
    createdAt: { type: Date, default: Date.now },
    responsibleDoctorId: { type: mongoose.Schema.Types.ObjectId, ref: "User" },
    version: { type: Number, default: 0 }
});

// Per-org DB: caseNumber uniqueness is per-database
clinicalCaseSchema.index({ caseNumber: 1 }, { unique: true, sparse: true });

// Standardized single-field indexes
clinicalCaseSchema.index({ responsibleDoctorId: 1 });
