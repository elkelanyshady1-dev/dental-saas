...
async function resolveShareByToken(token) {
    ...
    if (!rawShared) return { error: 404, message: "Share link not found" };
    if (new Date() > rawShared.expiresAt) return { error: 410, message: "This share link has expired" };

    return { shared: rawShared, organizationId: rawShared.organizationId.toString() };
}
...
async function createShareLink(conn, organizationId, userId, caseId, options, getOrCreateShareSnapshot) {
    const { expiresIn = "3d", type = "case", recordIds = [], recordSetIds = [], permissions = {}, hidePatientName = false } = options;
    ...
    if (!orthoCase) return { error: 404, message: "Case not found" };
    ...
    if (type === "records") {
        ...
        if (!hasRecordSetIds && !hasRecordIds) {
            return { error: 400, message: "recordSetIds or recordIds must be provided when type is 'records'" };
        }
    }
    ...
    if (typeof getOrCreateShareSnapshot === "function") {
        try {
            snapshotId = await getOrCreateShareSnapshot({ organizationId, caseId, userId });
        } catch (snapErr) {
    ...
    logger.info(
        { caseId, token, type, expiresAt, organizationId },
...
async function getSharedCase(token) {
    ...
    const { shared, organizationId } = result;

    const conn      = await dbManager.getConnection(organizationId);
    const OrthoCase = getModel(conn, OrthodonticCaseDef);
    const Snapshot  = getModel(conn, WorkflowSnapshotDef);

    const orthoCase = await OrthoCase.findById(shared.caseId)
        .select("treatmentCaseId malocclusionClass status workflowData")
        .populate({
            path: "treatmentCaseId",
            select: "patientId procedureType",
            populate: { path: "patientId", select: "firstName lastName dateOfBirth gender" },
        });

    if (!orthoCase) return { error: 404, message: "Case no longer exists" };

    const patient    = orthoCase.treatmentCaseId?.patientId;
    const patientInfo = shared.hidePatientName
        ? { name: "Anonymous Patient", age: null }
        : {
    ...
    if (shared.type === "records") {
        if (shared.recordSetIds?.length > 0) {
            workflowData = { ...workflowData, recordSets: (workflowData.recordSets || []).filter(s => shared.recordSetIds.includes(s.id)) };
    ...
    if (!shared.permissions.canViewAnalysis) {
        const stripped = (workflowData.recordSets || []).map(set => ({
            ...set,
            photos: (set.photos || []).map(p => ({ ...p, analysis: undefined })),
        }));
        workflowData = { ...workflowData, recordSets: stripped };
    }
 ...
 */
async function addComment(token, { authorName, role = "doctor", text, audioUrl }) {
    if (!authorName || (!text && !audioUrl)) {
        return { error: 400, message: "authorName and either text or audioUrl are required" };
    }
    ...
    if (!shared.permissions.canComment) {
        return { error: 403, message: "Comments are not allowed on this share" };
    }
    ...
    logger.info(
        { shareId: shared._id, commentId: comment._id, role: sanitizedRole },
...
async function getComments(token) {
    ...
    const comments = await Comment.find({ shareId: shared._id })
        .sort({ createdAt: -1 })
        .limit(100)
        .lean();
 ...
 */
async function joinCollaborator(token, { name, email = "", role = "doctor" }) {
    ...
    if (existing) return { collaborators: shared.collaborators };
    ...
    await SharedCase.findOneAndUpdate(
        { _id: shared._id },
        { $push: { collaborators: { name: name.substring(0, 100), email: sanitizedEmail, role: sanitizedRole } } },
        { new: true }
    );

    logger.info(
        { shareId: shared._id, name, role: sanitizedRole },
        "[sharedCase.service] Collaborator joined"
    );

    const updated = await SharedCase.findById(shared._id).lean();
    return { collaborators: updated?.collaborators || [] };
}
