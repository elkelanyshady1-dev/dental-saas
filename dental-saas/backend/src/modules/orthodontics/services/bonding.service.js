    ...
    await session.withTransaction(async () => {
      results = await Promise.all(
        teeth.map(async (tooth) => {
          const existing = await Bonding.findOne({ organizationId, caseId, tooth });

          if (existing) {
            // ── Rebonding: update fields + append event ──────────────────────
            const historyEntry = {
              action: "REBONDED",
              value: { ...data, linkedTadIds },
          ...
          const [created] = await Bonding.create(
            [{
              ...
              source: data.source || { type: "manual", referenceGroup: null },
              linkedTadIds,
              status: "ACTIVE",
              history: [
                {
...
const debondTooth = async (req, context, bondingId, reason = null) => {
  ...
  const bonding = await Bonding.findOne({ _id: bondingId, organizationId });
  if (!bonding) throw Object.assign(new Error("Bonding record not found"), { statusCode: 404 });
  if (bonding.status === "DEBONDED") {
    throw Object.assign(new Error("Tooth is already debonded"), { statusCode: 409 });
  }
  ...
  try {
    await _debondSession.withTransaction(async () => {
      saved = await bonding.save({ session: _debondSession });
      await clinicalEventService.logEventSync(req, {
        ...
        severity: "info",
        payload: { reason, tooth: bonding.tooth },
        metadata: { toothId: bonding.tooth, relatedEntityId: bonding._id, relatedEntityType: "Bonding" },
...
const repositionBracket = async (req, context, bondingId, newPosition) => {
  ...
  if (!bonding) throw Object.assign(new Error("Bonding record not found"), { statusCode: 404 });
  if (bonding.status === "DEBONDED") {
    throw Object.assign(new Error("Cannot reposition a debonded bracket"), { statusCode: 409 });
  }
  ...
  try {
    await _repoSession.withTransaction(async () => {
      saved = await bonding.save({ session: _repoSession });
      await clinicalEventService.logEventSync(req, {
        caseId: bonding.caseId,
        patientId: bonding.patientId,
        visitId:   req.activeVisit?._id,
        ...
        severity: "info",
        payload: { tooth: bonding.tooth, bondingHeight: newPosition.bondingHeight, bondingPosition: newPosition.bondingPosition },
        metadata: { toothId: bonding.tooth, relatedEntityId: bonding._id, relatedEntityType: "Bonding" },
...
const getBondingsByCase = async (req, context, caseId) => {
  ...
  return Bonding.find({ organizationId, caseId }).sort({ tooth: 1 }).lean();
};
...
const getDebondRate = async (req, context, caseId) => {
  const { Bonding } = _getModels(req);
  const { organizationId } = context;
  const all = await Bonding.find({ organizationId, caseId }).lean();

  const total = all.length;
  if (total === 0) return { rate: 0, debonded: 0, total: 0 };

  const debonded = all.filter((b) => b.status === "DEBONDED").length;
  const rate = Math.round((debonded / total) * 100);

  return { rate, debonded, total };
};
...
const getSettings = async (req, context) => {
  const { BondingSettings } = _getModels(req);
  const { organizationId } = context;
  const settings = await BondingSettings.findOne({ organizationId }).lean();
  return settings || { ...DEFAULT_SETTINGS, organizationId };
};
...
const getBondingById = async (req, context, bondingId) => {
  const { Bonding } = _getModels(req);
  const { organizationId } = context;
  return Bonding.findOne({ _id: bondingId, organizationId }).lean();
};
