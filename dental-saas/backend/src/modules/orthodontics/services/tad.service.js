...
const createTad = async (req, {
  ...
  if (existing) {
    const err = new Error(`An active TAD already exists at tooth ${toothNumber}` + (chartPosition?.anchorType ? ` (${chartPosition.anchorType})` : ""));
    err.statusCode = 409;
    throw err;
  }
 ...
 */
const markForRemoval = async (req, {
  tadId,
  reason,
  healingWeeks,
  notes,
  ...
  const scheduledDate = healingWeeks ? new Date(Date.now() + healingWeeks * 7 * 24 * 60 * 60 * 1000) : null;
    ...
    await _session.withTransaction(async () => {
      ...
      await clinicalEventService.logEventSync(req, {
        ...
        payload: {
          reason,
          healingWeeks,
          scheduledReinsertAt: tad.scheduledReinsertAt
        },
        metadata: {
          toothId: tad.toothNumber,
          relatedEntityId: tad._id,
          relatedEntityType: "Tad"
        }
      }, {
 ...
 */
const getFailureRate = async (req, {
  caseId
}) => {
  const {
    Tad
...
const listByCase = async (req, {
  ...
  if (!includeRemoved) filter.status = {
    $ne: "REMOVED"
  };
  return Tad.find(filter).sort({
    createdAt: 1
  }).lean();
};
...
const getSettings = async (req, organizationId) => {
  ...
  let settings = await TadSettings.findOne({}).lean();
  if (!settings) {
    ...
    settings = {
      ...
      alertThresholds: {
        failureRateWarning: 20,
        failureRateCritical: 40
      }
    };
  }
  return settings;
};
const updateSettings = async (req, organizationId, {
  brands,
  diameters,
  lengths,
  alertThresholds
...
const removeAllByCase = async (req, {
  caseId,
  performedBy
}) => {
  const {
