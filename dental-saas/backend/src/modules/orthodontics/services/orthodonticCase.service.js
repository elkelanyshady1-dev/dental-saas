...
function validateCaseTransition(current, next) {
  ...
  if (!allowed) return {
    valid: false,
    message: `Unknown status: ${current}`
  };
  if (!allowed.includes(next)) return {
    valid: false,
    message: `Cannot transition from '${current}' to '${next}'. Allowed: [${allowed.join(", ")}]`
  };
  return {
    valid: true
  };
...
class OrthodonticCaseService {
  ...
  async listCases({
    ...
    const [cases, total] = await Promise.all([OrthoCase.find(query).sort({
      createdAt: -1
    }).skip(skip).limit(limit).lean(), OrthoCase.countDocuments(query)]);
    return {
      cases,
      pagination: {
        page,
  ...
  async deleteCase({
    ...
    try {
      ...
      await Promise.all([Bonding.updateMany({
        caseId,
        status: {
          $ne: "DEBONDED"
        }
  ...
  async triggerSegmentation({
    ...
    const scanFile = await ScanFile.findOne({
      _id: scanFileId,
      caseId
    });
    if (!scanFile) {
    ...
    logger.info({
      caseId,
      scanFileId
    }, "[OrthoService] Segmentation record created — pending AI engine integration");
    return {
      segmentationId: segmentation._id,
      jobId: null
    };
  }
  async triggerCephAnalysis({
    req,
    caseId,
    scanFileId,
    analysisType,
  ...
  async getSegmentationResults({
    ...
    return Segmentation.find({
      caseId
    }).sort({
      createdAt: -1
    }).lean();
  }
  async getCephResults({
    req,
    caseId
  }) {
    const organizationId = this._ensureContext(req);
  ...
  async getAlignerPlanById({
    ...
    const plan = await AlignerPlan.findOne({
      _id: planId
  ...
  async saveWorkflowData({
    ...
    // Hardening §1.5 — double enforcement: block legacy plan fields at the
    // service boundary too, so any non-HTTP caller cannot bypass the guard.
    assertNoLegacyPlanWrites(workflowData, "workflowData");
    const organizationId = this._ensureContext(req);
    const {
      OrthoCase,
      Snapshot
    ...
    if (typeof expectedVersion === 'number' && expectedVersion >= 0) {
      ...
      if (expectedVersion !== currentVersion) {
        const err = new Error(`VERSION_CONFLICT: Expected v${expectedVersion} but case is at v${currentVersion}. ` + `Another save occurred. Refresh and retry.`);
        err.statusCode = 409;
  ...
  async getWorkflowData({
    ...
    const orthoCase = await OrthoCase.findOne({
      _id: caseId
    }).select("workflowData workflowVersion currentSnapshotId latestVersion").lean();
    if (!orthoCase) {
      const err = new Error("Orthodontic case not found.");
      err.statusCode = 404;
      throw err;
  ...
  async updateRecordSet({
    ...
    // Hardening §1.5 — double enforcement: block legacy plan fields at the
    // service boundary too (defense in depth beyond the controller guard).
    assertNoLegacyPlanWrites(recordSetData, "recordSetData");
    const organizationId = this._ensureContext(req);
    const {
      OrthoCase
    } = _getModels(req);
    ...
    const updated = await OrthoCase.findOneAndUpdate({
      _id: caseId,
      "workflowData.recordSets._id": recordSetId
    }, {
      $set: {
      ...
      arrayFilters: [{
        "elem._id": new mongoose.Types.ObjectId(recordSetId)
      }],
      new: true,
      runValidators: true
    }).lean();
   ...
   */
  async getOrCreateShareSnapshot({
    req,
    caseId,
    userId
  }) {
    ...
    const orthoCase = await OrthoCase.findOne({
      _id: caseId
    }).select("currentSnapshotId workflowVersion latestVersion").lean();
    if (!orthoCase) {
      const err = new Error("Orthodontic case not found.");
      err.statusCode = 404;
      throw err;
    ...
    const fullCase = await OrthoCase.findOne({
      _id: caseId
    }).lean();
  ...
  async listSnapshots({
    ...
    const skip = (page - 1) * limit;
    const [snapshots, total] = await Promise.all([Snapshot.find({
      caseId
    }).select("-recordSets -problemList -treatmentGoals -treatmentOptions -finalPlan").sort({
      version: -1
    }).skip(skip).limit(limit).lean(), Snapshot.countDocuments({
      caseId
    })]);
    return {
      snapshots,
