...
class ClinicAnalyticsService {
     ...
     */
    async getDailySummary({ req, date, branchId }) {
        const dayStart = new Date(`${date}T00:00:00.000Z`);
        const dayEnd   = new Date(`${date}T23:59:59.999Z`);

        const matchBase = { createdAt: { $gte: dayStart, $lte: dayEnd } };
        if (branchId) matchBase.branchId = new mongoose.Types.ObjectId(branchId);

        const [invoiceSummary, paymentSummary] = await Promise.all([
            _getInvoice(req).aggregate([
                { $match: { ...matchBase, status: { $in: ["issued", "partially_paid", "paid"] } } },
                {
                    $group: {
                        _id:          null,
        ...
        const inv = invoiceSummary[0] || {
            totalBilled: 0, totalPaid: 0, invoiceCount: 0, paidCount: 0, partialCount: 0
        };
        ...
        return {
            ...
            collections: { totalCollected, byMethod }
        };
    }
     ...
     */
    async getMonthlySummary({ req, year, month, branchId }) {
        const monthStart = new Date(year, month - 1, 1);
        const monthEnd   = new Date(year, month, 0, 23, 59, 59, 999);

        const matchBase = { createdAt: { $gte: monthStart, $lte: monthEnd } };
        if (branchId) matchBase.branchId = new mongoose.Types.ObjectId(branchId);

        const [invoiceSummary, paymentSummary, dailyRevenue] = await Promise.all([
            _getInvoice(req).aggregate([
                { $match: { ...matchBase, status: { $in: ["issued", "partially_paid", "paid"] } } },
                {
                    $group: {
        ...
        const inv = invoiceSummary[0] || { totalBilled: 0, invoiceCount: 0, paidCount: 0 };
        ...
        return {
            ...
            collections: {
                totalCollected,
                byMethod,
                byDay: dailyRevenue.map(d => ({ date: d._id, revenue: d.revenue, count: d.count }))
            }
        };
    }
     ...
     */
    async getOutstandingBalances({ req, branchId, limit = 50 }) {
        const matchBase = { status: { $in: ["issued", "partially_paid"] } };
        if (branchId) matchBase.branchId = new mongoose.Types.ObjectId(branchId);
        ...
        const results = await _getInvoice(req).aggregate([
            { $match: matchBase },
            {
                $group: {
                    _id:              "$patientId",
                    outstandingTotal: { $sum: "$totalAmount" },
                    invoiceCount:     { $sum: 1 },
                    oldestInvoice:    { $min: "$createdAt" }
                }
            },
            { $sort: { outstandingTotal: -1 } },
