...
async function retryFailedEvents(dbConnection, organizationId, options = {}) {
  ...
  const failedEvents = await DlqModel.find({
    orgId,
    status: {
      $in: ["pending", "retrying"]
    }
  ...
  for (const evt of failedEvents) {
    try {
      ...
      const backoffMs = Math.min(1000 * Math.pow(2, evt.retryCount || 0), 30_000);
...
async function getDlqSummary(dbConnection, organizationId) {
  const orgId = organizationId.toString();
  const DlqModel = getDlqModel(dbConnection);
  const [pending, retrying, abandoned] = await Promise.all([DlqModel.countDocuments({
    orgId,
    status: "pending"
 ...
 */
async function getDlqEvents(dbConnection, organizationId, {
  limit = 20,
  status
} = {}) {
  const orgId = organizationId.toString();
  ...
  const events = await DlqModel.find(filter).sort({
    failedAt: -1
