...
async function _runRebuild(dbConnection, orgId) {
  logger.info({
    orgId
  }, "[accounting:replay] Starting projection rebuild");
  ...
  const [revDel, cashDel, logDel] = await Promise.all([RevModel.deleteMany({}), CashModel.deleteMany({}), LogModel.deleteMany({
    orgId
  })]);
  logger.info({
    orgId,
  ...
  logger.info({
    ...stats
  }, "[accounting:replay] Projection rebuild complete");
  return stats;
}
  ...
  for (const {
    ...
    try {
      ...
      logger.error({
        orgId,
        err: err.message
      }, "[accounting:replay] Org rebuild failed");
      results.push({
        orgId,
        success: false,
        error: err.message
