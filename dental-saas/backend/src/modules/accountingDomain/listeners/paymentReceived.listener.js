...
async function onPaymentReceived(payload) {
    const { eventId, organizationId, dbConnection, payment } = payload;
    ...
    // ── eventId guard ─────────────────────────────────────────────────────────
    if (!eventId) {
        logger.warn({ event: EVENT, paymentId: payment?._id?.toString() },
    ...
    await batchEnqueue(
        ...
        async (err, pmt) => {
            ...
            await writeToDlq(dbConnection, organizationId, eventId, { payment: pmt }, err);
        }
    );
}
