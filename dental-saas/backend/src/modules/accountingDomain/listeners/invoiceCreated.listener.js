...
const { validateEventPayload } = require("../eventContracts/accountingEventContracts");
...
async function isAlreadyProcessed(dbConnection, orgId, eventId) {
    ...
    return !!(await Log.findOne({ orgId, eventId, eventType: EVENT }).lean());
}
...
async function writeToDlq(dbConnection, orgId, eventId, payload, err) {
    ...
    try {
        ...
        metrics.set("eventsInDlq", await DlqModel.countDocuments({ orgId, status: { $in: ["pending", "retrying"] } }));
    } catch (dlqErr) {
        if (dlqErr.code !== 11000) { // Ignore duplicate DLQ entries
            logger.error({ orgId, eventId, err: dlqErr.message },
...
async function onInvoiceCreated(payload) {
    const { eventId, organizationId, dbConnection, invoice } = payload;

    // ── Contract Validation ───────────────────────────────────────────────────
    const validation = validateEventPayload(EVENT, payload);
    if (!validation.valid) {
        logger.warn({ event: EVENT, errors: validation.errors },
    ...
    // ── eventId guard ─────────────────────────────────────────────────────────
    if (!eventId) {
        logger.warn({ event: EVENT, invoiceId: invoice?._id?.toString() },
    ...
    await batchEnqueue(
        ...
        async (inv, db) => {
            ...
            metrics.touch();
            metrics.logProcessed(EVENT, eventId, organizationId, { amount: inv.totalAmount });
        ...
        async (err, inv) => {
            ...
            await writeToDlq(dbConnection, organizationId, eventId, { invoice: inv }, err);
        }
    );
}
...
function register() {
    eventBus.on(EVENT, (payload) => {
        onInvoiceCreated(payload).catch((err) => {
            metrics.increment("eventsFailed");
            logger.error({ event: EVENT, err: err.message },
