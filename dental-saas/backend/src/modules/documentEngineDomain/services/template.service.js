...
class TemplateService {
    ...
    async createTemplate(data, req) {
        ...
        const { type, branchId, isActive } = data;
            ...
            await session.withTransaction(async () => {
                ...
                if (isActive) {
                    ...
                    await DocumentTemplate.updateMany(
                        { type, branchId: branchId || null, isActive: true },
                        { $set: { isActive: false } },
                        { session }
                    );
                }
                ...
                await createdTemplate.save({ session });
            });
    ...
    async createNewVersion(templateId, data, req) {
        ...
        try {
            ...
            await session.withTransaction(async () => {
                // Per-org DB: connection scopes to org database
                const baseTemplate = await DocumentTemplate.findOne({ _id: templateId }).session(session);
                ...
                await DocumentTemplate.updateMany(
                    { type: baseTemplate.type, branchId: baseTemplate.branchId, isActive: true },
                    { $set: { isActive: false } },
                    { session }
                );
    ...
    async restoreTemplateVersion(templateId, req) {
        ...
        try {
            ...
            await session.withTransaction(async () => {
                // Per-org DB: connection scopes to org database
                const targetTemplate = await DocumentTemplate.findOne({ _id: templateId }).session(session);
                ...
                // Atomic switch
                await DocumentTemplate.updateMany(
                    { type: targetTemplate.type, branchId: targetTemplate.branchId, isActive: true },
                    { $set: { isActive: false } },
                    { session }
                );

                targetTemplate.isActive = true;
                await targetTemplate.save({ session });
