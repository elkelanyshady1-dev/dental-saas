...
exports.getNotifications = async (req, res) => {
  try {
    ...
    const nextCursor = hasMore && visible.length > 0 ? visible[visible.length - 1].createdAt : null;
...
exports.getUnreadCount = async (req, res) => {
  try {
    ...
    const count = await Notification.countDocuments(orgFilter(req, {
      isRead: false
    }));
    return res.json({
      success: true,
...
exports.markAsRead = async (req, res) => {
  try {
    ...
    const Notification = _getSecureNotification(req);
    const notification = await Notification.findOne(orgFilter(req, {
      _id: id
    }));
    if (!notification) {
...
exports.markAllRead = async (req, res) => {
  try {
    const Notification = _getSecureNotification(req);
    const result = await Notification.updateMany(orgFilter(req, {
      userId: req.user._id,
      isRead: false
    }), {
      $set: {
