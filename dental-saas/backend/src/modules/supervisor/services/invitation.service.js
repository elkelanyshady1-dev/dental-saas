...
class InvitationService {
     ...
     */
    async createInvitation({ caseId, organizationId, invitedBy, inviteeEmail, role, permissions }) {
        const { SupervisorInvitation, CaseAccess, SupervisorUser } = _getPlatformModels();
     ...
     */
    async acceptInvitation({ invitationId, supervisorId }) {
        const { SupervisorInvitation, CaseAccess, SupervisorUser } = _getPlatformModels();
        // @rls-supervisor-plane — separate auth model, no org-scoped req context
     ...
     * @returns {{ invitation: object, access: object }}
     */
    async acceptByToken({ token, supervisorId }) {
        const { SupervisorInvitation } = _getPlatformModels();
        // @rls-supervisor-plane — separate auth model, no org-scoped req context
        ...
        return this.acceptInvitation({ invitationId: invitation._id, supervisorId });
    }
     ...
     * @param {{ invitationId: string, supervisorId: string }} params
     */
    async declineInvitation({ invitationId, supervisorId }) {
        const { SupervisorInvitation, SupervisorUser } = _getPlatformModels();
        // @rls-supervisor-plane — separate auth model, no org-scoped req context
        const invitation = await SupervisorInvitation.findById(invitationId);
        if (!invitation) {
            const err = new Error("Invitation not found.");
    ...
    async listPendingInvitations(email) {
        ...
        return SupervisorInvitation.find({
            ...
            .populate("caseId", "patientId status caseType malocclusionClass")
            .sort({ createdAt: -1 })
            .lean();
    }
     ...
     * @returns {object[]} invitations
     */
    async listCaseInvitations({ caseId, organizationId }) {
        const { SupervisorInvitation } = _getPlatformModels();
        // @rls-supervisor-plane — separate auth model, no org-scoped req context
        return SupervisorInvitation.find({
            caseId,
