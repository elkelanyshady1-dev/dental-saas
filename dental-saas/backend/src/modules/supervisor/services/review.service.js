...
class ReviewService {
  ...
  async createReviewStage({
    ...
    if (existing) {
      const err = new Error(`A ${stageType} review is already ${existing.status.toLowerCase()}. ` + "Complete or cancel it before creating a new one.");
      err.statusCode = 409;
      throw err;
    }
    ...
    // @rls-supervisor-plane — separate auth model, no org-scoped req context
    const previousCount = await ReviewStage.countDocuments({
      caseId,
      stageType
    });
  ...
  async listReviewStages({
    ...
    return ReviewStage.find(query).populate("requestedBy", "name email").populate("reviewedBy", "name email title").sort({
      stageType: 1,
      stageNumber: 1
    }).lean();
  }
  ...
  async getReviewStage(reviewId) {
    ...
    // @rls-supervisor-plane — separate auth model, no org-scoped req context
    const review = await ReviewStage.findById(reviewId).populate("requestedBy", "name email").populate("reviewedBy", "name email title").lean();
    if (!review) {
  ...
  async submitDecision({
    ...
    if (!allowed || !allowed.includes(decision)) {
      const err = new Error(`Cannot transition from '${currentStatus}' to '${decision}'. ` + `Allowed: [${(allowed || []).join(", ")}]`);
      err.statusCode = 400;
      throw err;
    }
  ...
  async getReviewSummary(caseId) {
    ...
    // @rls-supervisor-plane — separate auth model, no org-scoped req context
    const stages = await ReviewStage.find({
      caseId
    }).sort({
      stageType: 1,
      stageNumber: -1
    ...
    const commentCounts = await ReviewComment.aggregate([{
      $match: {
        caseId: require("mongoose").Types.ObjectId.createFromHexString(caseId)
      }
    }, {
      $group: {
        _id: "$reviewStageId",
