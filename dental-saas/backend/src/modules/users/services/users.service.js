...
async function resolveUniqueOrgEmail(UserModel, baseUsername, orgSlug) {
  ...
  const err = new Error(`Cannot generate a unique username for "${baseUsername}" in this organization — ` + "too many users share this name. Please use a distinguishing middle name or initial.");
  err.statusCode = 409;
...
async function createUser(data, req, actorId) {
  ...
  try {
    ({
      email,
      username
    } = await resolveUniqueOrgEmail(User, baseUsername, orgSlug));
  } catch (uniqueErr) {
  ...
  const specialty = isPractitionerFlag ? (data.specialty || data.speciality || "general").toLowerCase() : null;
  let user;
  try {
    const doc = new User({
      ...
      email,
      realEmail: null,
      // staff have no personal email in system
      ...
      // legacy root field (kept for compat)
  ...
  try {
    await orgUsageService.incrementUsers(organizationId);
...
async function listUsers(req, options = {}) {
  ...
  const [users, total] = await Promise.all([User.find(filter).select("-password").populate("roleId", "name permissions isSystemRole").populate("branchAccess", "name isActive").sort({
    createdAt: -1
  }).skip(skip).limit(limit).lean(), User.countDocuments(filter)]);
  return {
    data: users,
    pagination: {
      page: Number(page),
...
async function getUserById(userId, req) {
  ...
  const user = await User.findById(userId).select("-password").populate("roleId", "name permissions isSystemRole").populate("branchAccess", "name isActive").lean();
  if (!user) {
...
async function deleteUser(userId, req, actorId) {
  ...
  try {
    await orgUsageService.decrementUsers(organizationId);
  } catch (_) {/* logged inside service */}
  return {
    success: true,
