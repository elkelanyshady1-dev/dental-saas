...
const {
  calculateDPI
} = require("../engines/doctorPerformance.engine");
const {
  calculateEfficiency
} = require("../engines/clinicalEfficiency.engine");
const {
  calculateRisk
} = require("../engines/riskScoring.engine");
...
async function generateIntelligence(organizationId, req) {
  ...
  let weights = await IntelligenceWeights.findOne({}).lean().exec();
  if (!weights) {
  ...
  const financials = await FinancialSnapshot.find({}).lean().exec();
  let totalRevenue = 0;
  ...
  const caseCosts = await CaseCostSnapshot.find({}).lean().exec();
  ...
  const aggregationReq = req || {}; // Fallback for internal/batch callers
  const cases = await clinicalReadService.aggregateCases(aggregationReq, [{
    $match: {}
  } // organizationId auto-injected by secureModel
  ]);
