...
const {
  enqueueNotification
} = require("../../notificationDomain/notification.service");
...
class PortalMonitoringService {
  ...
  async getProgressById({
    ...
    if (!entry) {
      const err = new Error("Progress entry not found.");
      err.statusCode = 404;
      throw err;
    }
    return entry;
  ...
  async registerPhoto({
    ...
    logger.info({
      photoId: photo._id
    }, "[PortalMonitoring] Photo registered \u2014 pending AI engine integration");
  ...
  async listPhotos({
    ...
    return PatientPhoto.find(query).sort({
      createdAt: -1
    }).lean();
  }
  ...
  async submitMonitoringSession({
    ...
    if (!progressEntry) {
      const err = new Error("Aligner progress entry not found for this stage.");
      err.statusCode = 404;
      throw err;
    }
  ...
  async listMonitoringSessions({
    ...
    const [sessions, total] = await Promise.all([MonitoringSession.find(query).sort({
      createdAt: -1
    }).skip(skip).limit(limit).lean(), MonitoringSession.countDocuments(query)]);
    return {
      sessions,
      pagination: {
        page,
  ...
  async getMonitoringSession({
    ...
    const session = await MonitoringSession.findOne({
      _id: sessionId
    }).populate("photoIds").populate("reviewedBy", "name email").lean();
    if (!session) {
      const err = new Error("Monitoring session not found.");
      err.statusCode = 404;
      throw err;
    }
    return session;
  }
  async reviewMonitoringSession({
    req,
    sessionId,
    newStatus,
    reviewedBy,
    ...
    // Notify patient
    const notifMessage = newStatus === "approved" ? "Your monitoring session has been approved by the doctor." : "Your doctor has requested additional photos. Please check your portal.";
    await enqueueNotification({
  ...
  async listMessages({
    ...
    if (caseId) query.caseId = caseId;
    const skip = (page - 1) * limit;
    const [messages, total] = await Promise.all([PatientMessage.find(query).sort({
      createdAt: -1
    }).skip(skip).limit(limit).lean(), PatientMessage.countDocuments(query)]);
    return {
      messages,
      pagination: {
        page,
