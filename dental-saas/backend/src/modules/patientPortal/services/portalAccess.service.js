...
class PortalAccessService {
     ...
     */
    async sendAccessLink({ req, patientId, type = "magic_link", deliveryChannel }) {
        const { PatientUser, PortalInvite } = _getModels();
        ...
        const patient = await Patient
            ...
            .select("nameArabic nameEnglish phone email patientCode")
            .lean();
        ...
        if (type === "magic_link") {
            ...
            if (!existingUser) {
                const err = new Error(
                    "No portal account exists for this patient. Use 'setup_link' to create one."
                );
                err.statusCode = 404;
                throw err;
            }
        ...
        // ── Generate token ───────────────────────────────────────────────
        const { rawToken, tokenHash } = _generateToken();
    ...
    async verifyAccessToken({ req, token }) {
        ...
        if (invite.type === "magic_link") {
            return this._handleMagicLinkVerify({ req, invite, PatientUser });
        } else if (invite.type === "setup_link") {
            return this._handleSetupLinkVerify({ req, invite });
        }
     ...
     * @private
     */
    async _handleMagicLinkVerify({ req, invite, PatientUser }) {
     ...
     * @private
     */
    async _handleSetupLinkVerify({ req, invite }) {
        const Patient = getModel(req.dbConnection, PatientDef);

        const patient = await Patient
            .findOne({ _id: invite.patientId, isActive: true })
            .select("nameArabic nameEnglish phone email dateOfBirth gender patientCode")
            .lean();
        ...
        return {
            type: "setup_link",
            setupToken: invite._id.toString(), // Reference for completeSetup
            patient: {
     ...
     */
    async completeSetup({ req, setupToken, password, medicalHistory }) {
        const { PatientUser, PortalInvite } = _getModels();
        ...
        const invite = await PortalInvite.findOne({
            _id: setupToken,
            type: "setup_link",
            usedAt: { $ne: null }, // Must have been verified first
        });
