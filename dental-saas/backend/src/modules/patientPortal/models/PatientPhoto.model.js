...
const patientPhotoSchema = new mongoose.Schema(
    {
        ...
        aiFindings: {
            alignerFit: { type: String, enum: ["good", "poor", "unknown"], default: "unknown" },
            toothMovement: { type: String, enum: ["on_track", "behind", "unknown"], default: "unknown" },
            flags: [{ type: String }],
...
// ─── Indexes ─────────────────────────────────────────────────────────────────
patientPhotoSchema.index({ organizationId: 1, patientId: 1, caseId: 1 });
patientPhotoSchema.index({ organizationId: 1, monitoringSessionId: 1 });
patientPhotoSchema.index({ organizationId: 1, aiAnalysisStatus: 1 });
