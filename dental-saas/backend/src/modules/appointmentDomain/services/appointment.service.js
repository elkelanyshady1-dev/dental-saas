...
const { APPOINTMENT_CREATED, APPOINTMENT_UPDATED, APPOINTMENT_STATUS_CHANGED } = require("../../../core/domainEvents");
...
class AppointmentService {
    async createAppointment({ organizationId, branchId, patientId, dentistId, chairId, startTime, endTime, duration, status, notes, actorId, externalRequestId, req }) {
        if (req) {
            ...
            eventBus.emit(APPOINTMENT_CREATED, {
                ...
                patientId,
                branchId,       // Phase 13: include branchId for calendar room targeting
                dentistId,
    ...
    async handleBookingApproval(payload) {
        const { requestId, organizationId } = payload;
        ...
        if (requestId) {
            ...
            if (existing) {
                logger.info({ requestId }, "[AppointmentService] Skipping — Appointment already exists for this requestId");
                return existing;
            }
        }
    ...
    async updateStatus({ organizationId, appointmentId, status, actorId, expectedVersion, isInternalEvent = false, req }) {
        let appointment;
        if (req) {
            const Appointment = _getSecureAppointment(req);
            appointment = await Appointment.findOne({ _id: appointmentId, isActive: true });
        } else {
        ...
        const updateDoc = {
            $set: { status },
            $push: {
                statusHistory: { status, changedBy: actorId, changedAt: new Date() }
            },
            $inc: { version: 1 }
        };
        ...
        if (req) {
            ...
            result = await Appointment.updateOne(
                { _id: appointmentId, version: targetVersion },
                updateDoc
            );
        } else {
            // Internal event — resolve connection via connectionResolver
        ...
        if (status === "completed") {
            eventBus.emit("appointment.completed", {
                organizationId: organizationId || req?.rls?.organizationId,
                appointmentId,
        ...
        eventBus.emit(APPOINTMENT_STATUS_CHANGED, {
            organizationId: organizationId || req?.context?.organizationId || req?.rls?.organizationId,
            appointmentId,
            status: status,         // normalized (for socket schema field)
            previousStatus: oldStatus,  // for status_changed schema
     ...
     */
    async getUpcomingAppointment({ organizationId, patientId, req }) {
        if (req) {
            const Appointment = _getSecureAppointment(req);
    ...
    async getPatientAppointments({ organizationId, patientId, req }) {
        if (req) {
            const Appointment = _getSecureAppointment(req);
