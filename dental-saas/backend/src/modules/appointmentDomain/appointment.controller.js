...
const {
  ACTIVE_STATUSES,
  detectOverlaps
} = require("./utils/overlapDetection");
const {
  validateTransition
...
const {
  buildScopedQuery: buildSmartQuery
} = require("../../core/authorization/scopedQueryBuilder");
...
async function validatePractitioner({
  ...
  const doctor = await User.findOne({
    _id: dentistId,
    deletedAt: null
  }).select("_id firstName lastName isPractitioner isActive profile branchAccess hasFullBranchAccess").lean();
  if (!doctor) {
  ...
  if (!doctor.isPractitioner) {
    throw domainError("INVALID_PRACTITIONER", `${doctor.firstName || "Selected user"} is not authorized for appointment scheduling`);
  }

  // Rule 2: must be active
  if (!doctor.isActive) {
    throw domainError("PRACTITIONER_INACTIVE", `${doctor.firstName || "Practitioner"} account is inactive — cannot schedule`);
  }

  // Rule 3: must have a specialty
  if (!doctor.profile?.specialty) {
    throw domainError("PRACTITIONER_NO_SPECIALTY", `${doctor.firstName || "Practitioner"} has no specialty assigned — update their profile first`);
  }

  // Rule 4: must have branch access
  const hasAccess = doctor.hasFullBranchAccess || (doctor.branchAccess || []).some(id => id.toString() === branchId.toString());
  if (!hasAccess) {
    throw domainError("PRACTITIONER_NO_BRANCH_ACCESS", `${doctor.firstName || "Practitioner"} does not have access to the selected branch`);
  }
  return doctor;
}
...
const validateBranchAccess = async (branchId, organizationId, allowedBranches, req) => {
  ...
  if (allowedBranches !== null && allowedBranches !== undefined && !req.context?.hasFullBranchAccess) {
    const allowed = allowedBranches.some(id => id.toString() === branchId.toString());
    if (!allowed) {
      return {
...
const validateBranchPatientCompatibility = async (patientId, branchId, req) => {
  ...
  const [patient, branch] = await Promise.all([Patient.findById(patientId).select("careType").lean(), Branch.findById(branchId).select("clinicType").lean()]);
  if (!patient) throw Object.assign(new Error("Patient not found."), {
    statusCode: 404
  });
  if (!branch) throw Object.assign(new Error("Branch not found."), {
    statusCode: 404
  });
  ...
  if (patientType !== branchType) {
    const err = new Error(`Patient care type (${patientType}) does not match branch type (${branchType}). ` + `${patientType === "ACADEMIC" ? "Academic patients can only be scheduled in Academic branches." : "Private patients can only be scheduled in Private branches."}`);
    err.statusCode = 422;
...
exports.getAvailability = async (req, res) => {
  try {
    ...
    if (branchCheck.error) {
      return res.status(branchCheck.status).json({
    ...
    if (chairId) {
      ...
      if (!chair) {
        return res.status(404).json({
          message: "Chair not found in this branch"
        });
      }
    }
    ...
    const [dentistAppointments, chairAppointments] = await Promise.all([dentistId
    // @per-org-compliant — Appointment is secureModel-wrapped via _getModels(req)
    ? Appointment.find({
      ...appointmentQuery,
      dentistId
    }) : Promise.resolve([]), chairId
    // @per-org-compliant — Appointment is secureModel-wrapped via _getModels(req)
    ? Appointment.find({
      ...appointmentQuery,
      branchId,
    ...
    for (let cursor = startMins; cursor + slotDuration <= endMins; cursor += slotDuration) {
      ...
      const dentistOccupied = dentistAppointments.some(a => a.startTime < slotEnd && a.endTime > slotStart);
      const chairOccupied = chairAppointments.some(a => a.startTime < slotEnd && a.endTime > slotStart);
      slots.push({
  ...
  try {
    ...
    try {
      ...
      return res.status(compatErr.statusCode || 422).json({
        success: false,
        error: {
          code: compatErr.code || "CARE_TYPE_BRANCH_MISMATCH",
    ...
    const startDate = startTime.includes("T") ? new Date(startTime) : new Date(`${date}T${startTime}:00`);
    ...
    if (req.body.procedureId) {
      try {
        ...
        } = require("../../modules/treatment-catalog/application/dto/procedure.dto");
...
exports.updateAppointmentStatus = async (req, res) => {
  try {
    ...
    const updated = await appointmentService.updateStatus({
      ...
      // skips expectedVersion guard for sync HTTP path
      req
    });

    // 🔔 Billing trigger — fire-and-forget on completion
    if (existing.status !== "completed" && transition.normalizedStatus === "completed") {
      createDiagnosticInvoice(req.params.id).catch(err => console.error("[BillingService] Error:", err.message));
    }
    res.json({
      success: true,
  ...
  try {
    ...
    if (branchId) {
      if (req.context?.allowedBranches) {
        const allowed = req.context.allowedBranches.some(id => id.toString() === branchId.toString());
        if (!allowed) {
  ...
  try {
    ...
    if (req.body.dentistId && req.body.dentistId.toString() !== appointment.dentistId.toString()) {
      ...
      await validatePractitioner({
        ...
        branchId: targetBranchForPractitioner
      }, req);
    }

    // If branch is changing, validate new branch
    if (req.body.branchId && req.body.branchId.toString() !== appointment.branchId.toString()) {
      const branchCheck = await validateBranchAccess(req.body.branchId, req.context.organizationId, req.context.allowedBranches);
      if (branchCheck.error) {
        return res.status(branchCheck.status).json({
          message: branchCheck.error
        });
    ...
    const needsOverlapCheck = req.body.startTime || req.body.endTime || req.body.duration || req.body.dentistId || req.body.chairId;
    if (needsOverlapCheck) {
      ...
      if (req.body.startTime || req.body.duration || req.body.date) {
        ...
        if (req.body.startTime) {
          newStartTime = req.body.startTime.includes("T") ? new Date(req.body.startTime) : new Date(`${date}T${req.body.startTime}:00`);
        }
...
exports.getCalendarDay = async (req, res) => {
  try {
    ...
    const branchData = branchList.map(branch => ({
      ...
      chairs: chairList.filter(c => c.branchId.toString() === branch._id.toString()).map(c => ({
        chairId: c._id,
        chairName: c.name
      }))
    }));
