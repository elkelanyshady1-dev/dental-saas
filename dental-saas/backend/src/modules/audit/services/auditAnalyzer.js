...
const THRESHOLDS = {
    ...
    BUSINESS_HOURS_START: 6,  // 6:00 AM
 ...
 * @returns {Object}
 */
function createAlert({ type, severity, message, details, actorId, organizationId }) {
    return {
        type,
        severity,
        message,
...
function detectExcessiveMutations(logs, organizationId) {
    ...
    const updateActions = logs.filter(l =>
        l.action?.endsWith("_UPDATED") || l.action?.endsWith("_CREATED")
    );
...
function detectCrossBranchAccess(logs, organizationId) {
    ...
    for (const log of logs) {
        ...
        const isCrossBranch = log.details?.crossBranch === true
            || log.details?.changes?.crossBranch === true
            || (log.action === "ORG_PERMISSION_DENIED" && log.details?.reason?.includes("branch"));
                ...
                alerts.push(createAlert({
                    ...
                    details: {
                        ...
                        topActions: Object.entries(
                            actions.reduce((acc, a) => { acc[a.action] = (acc[a.action] || 0) + 1; return acc; }, {})
...
function detectSensitiveAccess(logs, organizationId) {
    ...
    const sensitivePatterns = [
        "INVOICE_CREATED", "PAYMENT_CREATED", "PAYMENT_REFUNDED",
        "CLINICAL_NOTE_ADDED", "PRESCRIPTION_CREATED",
    ];
...
function analyzeAuditLogs(logs, organizationId) {
    ...
    try {
        const alerts = [
            ...detectExcessiveMutations(logs, organizationId),
            ...detectBulkDeletes(logs, organizationId),
            ...detectCrossBranchAccess(logs, organizationId),
            ...detectOffHoursActivity(logs, organizationId),
            ...detectRapidFire(logs, organizationId),
            ...detectDenialSpike(logs, organizationId),
            ...detectSensitiveAccess(logs, organizationId),
        ];
