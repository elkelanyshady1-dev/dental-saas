...
const {
  getRegionContext
} = require("../../../infrastructure/regionRouter");
...
async function getEntityTimeline(params) {
  ...
  if (category && category !== "all") {
    const categoryActions = Object.entries(ACTION_CATEGORIES).filter(([, cat]) => cat === category).map(([action]) => action);
  ...
  const [logs, total] = await Promise.all([AuditLog.find(query).sort({
    createdAt: -1
  }).skip(skip).limit(limit).lean(), AuditLog.countDocuments(query)]);
...
async function getOrgTimeline(params) {
  ...
  if (category && category !== "all") {
    const categoryActions = Object.entries(ACTION_CATEGORIES).filter(([, cat]) => cat === category).map(([act]) => act);
    if (categoryActions.length > 0) {
      query.action = {
        $in: categoryActions
      };
...
async function getAuditStats(params) {
  ...
  const [totalActions, byCategory, recentDenials] = await Promise.all([AuditLog.countDocuments({
    createdAt: {
      $gte: since
    }
  }),
  ...
  AuditLog.aggregate([{
    ...
    action: {
      $in: ["ORG_PERMISSION_DENIED", "CAPABILITY_DENIED", "ENTITLEMENT_DENIED"]
    }
  })]);
