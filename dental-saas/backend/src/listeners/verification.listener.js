...
function buildOtpSmsPayload(data) {
  return {
    ...
    code: data.rawToken,
    message: `Your OrthoNoe verification code: ${data.rawToken}. Valid for 10 minutes.`
  };
}
function buildOtpWhatsappPayload(data) {
  return {
    phone: data.identifier,
    body: `Your OrthoNoe verification code: ${data.rawToken}. Valid for 10 minutes. Do not share this code.`
  };
}
...
const DISPATCH_MAP = {
  PHONE_OTP: {
    sms: {
      type: "OTP",
      builder: buildOtpSmsPayload
    },
    whatsapp: {
      type: "OTP",
      builder: buildOtpWhatsappPayload
    },
    email: {
      type: "EMAIL_OTP",
      builder: buildOtpEmailPayload
    }
  },
  EMAIL_VERIFY: {
    email: {
      type: "EMAIL_VERIFY",
      builder: buildEmailVerifyPayload
    }
  },
  PASSWORD_RESET: {
    email: {
      type: "PASSWORD_RESET",
      builder: buildPasswordResetPayload
    }
  },
  MAGIC_LOGIN: {
    email: {
      type: "MAGIC_LINK",
      builder: buildMagicLoginPayload
    }
  }
};
...
async function handleVerificationCreated(data) {
  ...
  if (!purpose || !channelOrder.length) {
    logger.error({
      data
    }, "[verification.listener] Invalid event data — missing purpose or channelOrder");
    return;
  }
  ...
  if (!purposeMap) {
    logger.error({
      purpose
    }, "[verification.listener] No dispatch map for purpose");
    return;
  }
  let delivered = false;
  ...
  for (let i = 0; i < channelOrder.length; i++) {
    ...
    if (!dispatch) {
      logger.warn({
        purpose,
        channel
      }, "[verification.listener] No dispatch config for channel — skipping");
      continue;
    }
    try {
...
function registerVerificationListeners() {
  ...
  EventEmitter.prototype.on.call(eventBus, "verification.token.created", data => {
    handleVerificationCreated(data).catch(err => {
      logger.error({
        err: err.message
      }, "[verification.listener] Unhandled error in delivery handler");
    });
  });
