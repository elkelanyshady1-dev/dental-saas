...
exports.createFeature = async (req, res) => {
    try {
        const { key, name, description, category, defaultEnabled, isCore, allowedPlans, allowedRoles } = req.body;

        const exists = await FeatureDefinition.findOne({ key });
        if (exists) {
            return res.status(400).json({ message: `Feature key '${key}' already exists.` });
        }

        const feature = new FeatureDefinition({
            key, name, description, category, defaultEnabled, isCore, allowedPlans, allowedRoles
        });
...
exports.updateFeature = async (req, res) => {
    try {
        ...
        const { name, description, category, defaultEnabled, allowedPlans, allowedRoles } = req.body;
...
exports.overrideOrganizationFeature = async (req, res) => {
    try {
        ...
        const { enabled, resetOverride } = req.body;

        const org = await Organization.findById(orgId);
        if (!org) return res.status(404).json({ message: "Organization not found" });

        const featureDef = await FeatureDefinition.findOne({ key });
        if (!featureDef) return res.status(404).json({ message: "Feature definition not found" });

        if (featureDef.isCore && !enabled && !resetOverride) {
            return res.status(400).json({ message: "Cannot explicitly disable a core feature." });
        }

        // Initialize features map if it doesn't exist
        if (!org.features) org.features = new Map();

        if (resetOverride) {
            const current = org.features.get(key) || { enabled: featureDef.defaultEnabled, overridden: false };
            org.features.set(key, { ...current, overridden: false });
        } else {
            org.features.set(key, { enabled, overridden: true });
        }
        ...
        res.json({ message: "Feature override applied", features: org.features });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};
