...
exports.getOrganizationSettings = async (req, res) => {
    try {
        let settings = await OrganizationSettings.findOne({ organizationId: req.organization._id });
        if (!settings) {
            settings = await OrganizationSettings.create({ organizationId: req.organization._id });
        }
...
exports.updateOrganizationSettings = async (req, res) => {
    try {
        ...
        if (!settings) {
            settings = new OrganizationSettings({ organizationId: req.organization._id });
        }
