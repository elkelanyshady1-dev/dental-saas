/**
 * BranchAccessSelector.jsx — Multi-branch checkbox selector
 */
...
export default function BranchAccessSelector({ selected = [], onChange }) {
    ...
    if (loading) return <div className="text-xs text-gray-400 py-2">Loading branches...</div>;

    return (
        <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Branch Access</label>
            <div className="space-y-2 max-h-40 overflow-y-auto bg-gray-50 rounded-xl p-3 border border-gray-100">
                {branches.length === 0 ? (
                    ...
                    <label key={b._id} className="flex items-center gap-2.5 cursor-pointer hover:bg-white rounded-lg px-2 py-1 transition">
                        ...
                        {!b.isActive && <span className="text-[10px] text-gray-400 font-medium">(inactive)</span>}
                    </label>
                ))}
            </div>
        </div>
