...
interface SnapshotHistorySidebarProps {
  /** List of snapshots (no chartState — list-item shape from API) */
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {isLoading ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6">
            ...
            <p className="text-xs font-medium text-slate-400">Loading snapshots…</p>
          </div>
        ) : snapshots.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6">
            ...
            {snapshots.map((snap) => (
              ...
              >
                <div className="h-24 bg-slate-100 relative overflow-hidden">
                  <img
                    src={snap.thumbnail ?? "https://picsum.photos/seed/ortho-thumb/200/100"}
                    alt="Thumbnail"
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">
                        {new Date(snap.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}
                      </span>
                      <span className="text-xs font-bold text-slate-700 mt-0.5 truncate max-w-[160px]">
                        {snap.name ?? 'Untitled Snapshot'}
                      </span>
                      {snap.procedureCount > 0 && (
                        <span className="text-[10px] text-slate-400 mt-0.5">{snap.procedureCount} procedures</span>
                      )}
                    </div>
                    <ChevronRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all flex-shrink-0" />
                  </div>
                </div>
              </div>
            ))}
