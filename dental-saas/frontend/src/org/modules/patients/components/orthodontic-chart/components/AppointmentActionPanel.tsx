...
import { Clock, Mic, Paperclip, Save, Activity, RefreshCw } from 'lucide-react';
...
interface AppointmentActionPanelProps {
  ...
  onSaveSnapshot: () => void;
  isSaving: boolean;
  /** Controlled sidebar width in pixels */
  width?: number;
...
const AppointmentActionPanel: React.FC<AppointmentActionPanelProps> = ({
  ...
  return (
    <div
      className="relative bg-white border-r border-slate-200 flex flex-col h-full overflow-hidden flex-shrink-0"
      style={{ width }}
    >
      {/* Header */}
        ...
        {actions.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6">
            ...
            <p className="text-xs font-medium text-slate-400">
              No actions logged yet. Interactions with the chart will appear here.
            </p>
          </div>
        ) : (
      ...
      <div className="p-4 border-t border-slate-100 bg-slate-50/30 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={isSaving}
            className={`flex-[1.5] flex items-center justify-center gap-2 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-bold hover:bg-blue-700 transition-all shadow-md ${
      ...
      <div
        ...
        className="absolute top-0 right-0 w-[8px] h-full cursor-col-resize z-30 select-none group/handle flex items-center justify-center"
        title="Drag to resize panel"
      >
        {/* Permanent visible border strip */}
        <div className="absolute inset-y-0 right-0 w-[2px] bg-slate-200 group-hover/handle:bg-blue-400 transition-colors duration-150" />

        {/* Always-visible grip pill */}
        <div className="relative z-10 flex flex-col items-center gap-[5px] px-[2px] py-2 rounded-full bg-white border border-slate-200 shadow-sm group-hover/handle:border-blue-400 group-hover/handle:shadow-blue-100 transition-all duration-150">
          {[0, 1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="w-[3px] h-[3px] rounded-full bg-slate-300 group-hover/handle:bg-blue-400 transition-colors duration-150"
            />
          ))}
        </div>
      </div>
