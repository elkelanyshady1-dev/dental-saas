...
import { Clock, MessageSquare, Mic, Paperclip, Save, CheckCircle2, Activity, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
const AppointmentActionPanel: React.FC<AppointmentActionPanelProps> = ({ 
  ...
  return (
    <div className={`${isCollapsed ? 'w-16' : 'w-80'} bg-white border-r border-slate-200 flex flex-col h-full overflow-hidden transition-all duration-300 relative`}>
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute top-4 -right-4 z-50 w-8 h-8 bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center hover:bg-blue-700 transition-all"
      >
