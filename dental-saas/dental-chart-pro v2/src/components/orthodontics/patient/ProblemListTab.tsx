...
export const ProblemListTab: React.FC<ProblemListTabProps> = ({ data, onUpdate, onOpenPhotoViewer }) => {
  ...
  const deriveFromCephAnalysis = () => {
    ...
    // 4. Vertical Pattern (MMP)
    const mmp = parseFloat(analysis.mmp);
    if (!isNaN(mmp)) {
      if (mmp > 28) updatedProblemList.developmental.verticalProblems.skeletal = 'open-bite'; // High Angle
      else if (mmp < 22) updatedProblemList.developmental.verticalProblems.skeletal = 'deep-bite'; // Low Angle
      else updatedProblemList.developmental.verticalProblems.skeletal = null;
    }

    // 5. Upper Incisor Inclination (U1/PP)
    ...
    // 6. Lower Incisor Inclination (L1/Mandb)
  ...
  const handleDentalChartChange = (tooth: string, statuses: string[]) => {
    // We can store the dental chart snapshot in a specific field if needed
    // For now, let's assume we update the pathological notes based on the chart
    const updatedProblemList = { ...problemList };
    // In a real app, we might want to store the chart state separately
    // But for this UI, we'll just trigger the update
    onUpdate({ problemList: updatedProblemList });
  };
        ...
        <div className="lg:col-span-2 space-y-6">
          {activeCategory === 'pathological' ? (
            <div className="space-y-6">
              ...
              {/* Dental Chart Snapshot for Pathological */}
              <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
                ...
                <DentalNotationChart 
                  arch="both"
                  data={{}} // In a real app, this would be linked to problemList.pathological
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="group space-y-2">
                      <div className="flex items-center justify-between px-1">
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Frontal Smile</span>
                        <span className="text-[8px] font-medium text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">Smile Arc, Display</span>
                      </div>
                      <textarea 
                        value={problemList.developmental.esthetics.frontalSmile}
                        onChange={(e) => handleUpdateProblem('developmental', 'esthetics.frontalSmile', e.target.value)}
                        ...
                        placeholder="Describe smile esthetics..."
                      />
                    </div>
                    <div className="group space-y-2">
                      <div className="flex items-center justify-between px-1">
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Frontal Rest</span>
                        <span className="text-[8px] font-medium text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">Proportions, Lips</span>
                      </div>
                      <textarea 
                        value={problemList.developmental.esthetics.frontalRest}
                        onChange={(e) => handleUpdateProblem('developmental', 'esthetics.frontalRest', e.target.value)}
                        className="w-full px-4 py-4 rounded-[24px] border border-slate-200 text-[11px] font-medium text-slate-700 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all h-32 resize-none bg-slate-50/50 hover:bg-white leading-relaxed placeholder:text-slate-300"
                        placeholder="Describe resting features..."
                      />
                    </div>
                    <div className="group space-y-2">
                      <div className="flex items-center justify-between px-1">
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Profile</span>
                        <span className="text-[8px] font-medium text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">Nasolabial, Chin</span>
                      </div>
                      <textarea 
                        value={problemList.developmental.esthetics.profile}
                        onChange={(e) => handleUpdateProblem('developmental', 'esthetics.profile', e.target.value)}
                        className="w-full px-4 py-4 rounded-[24px] border border-slate-200 text-[11px] font-medium text-slate-700 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all h-32 resize-none bg-slate-50/50 hover:bg-white leading-relaxed placeholder:text-slate-300"
                        placeholder="Describe profile findings..."
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-6">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div 
                        onClick={() => handleUpdateProblem('developmental', 'transverse.skeletal', !problemList.developmental.transverse.skeletal)}
                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${problemList.developmental.transverse.skeletal ? 'bg-blue-600 border-blue-600' : 'border-slate-200 group-hover:border-blue-400'}`}
                      >
                        {problemList.developmental.transverse.skeletal && <Plus className="w-3.5 h-3.5 text-white rotate-45" />}
                      </div>
                      <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Skeletal</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div 
                        onClick={() => handleUpdateProblem('developmental', 'transverse.dental', !problemList.developmental.transverse.dental)}
                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${problemList.developmental.transverse.dental ? 'bg-blue-600 border-blue-600' : 'border-slate-200 group-hover:border-blue-400'}`}
                      >
                        {problemList.developmental.transverse.dental && <Plus className="w-3.5 h-3.5 text-white rotate-45" />}
                      </div>
                      <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Dental</span>
                    </label>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-400 uppercase">Molar Class</label>
                        <input 
                          type="text"
                          value={problemList.developmental.apProblems.dental.molarClass}
                          onChange={(e) => handleUpdateProblem('developmental', 'apProblems.dental.molarClass', e.target.value)}
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-400 uppercase">Canine Class</label>
                        <input 
                          type="text"
                          value={problemList.developmental.apProblems.dental.canineClass}
                          onChange={(e) => handleUpdateProblem('developmental', 'apProblems.dental.canineClass', e.target.value)}
                          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20"
                          placeholder="e.g. Class II"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-400 uppercase">Incisal Class</label>
                        <input 
                          type="text"
                          value={problemList.developmental.apProblems.dental.incisalClass}
                          onChange={(e) => handleUpdateProblem('developmental', 'apProblems.dental.incisalClass', e.target.value)}
                          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20"
                          placeholder="e.g. Class I"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-400 uppercase">Increased Overjet</label>
                        <input 
                          type="text"
                          value={problemList.developmental.apProblems.dental.increasedOverjet}
                          onChange={(e) => handleUpdateProblem('developmental', 'apProblems.dental.increasedOverjet', e.target.value)}
                          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20"
                          placeholder="e.g. 4mm"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-400 uppercase">Anterior Crossbite</label>
                        <input 
                          type="text"
                          value={problemList.developmental.apProblems.dental.anteriorCrossbite}
                          onChange={(e) => handleUpdateProblem('developmental', 'apProblems.dental.anteriorCrossbite', e.target.value)}
                          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20"
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                      <span className="text-[9px] font-bold text-slate-500 uppercase block">a. Skeletal</span>
                      <div className="flex gap-3">
                        {[
                          ...
                          <button
                            key={v.id}
                            onClick={() => handleUpdateProblem('developmental', 'verticalProblems.skeletal', problemList.developmental.verticalProblems.skeletal === v.id ? null : v.id)}
                            className={`px-4 py-2 rounded-xl text-[10px] font-bold transition-all border ${
                              problemList.developmental.verticalProblems.skeletal === v.id
                              ? 'bg-blue-600 border-blue-600 text-white shadow-md'
                              : 'bg-white border-slate-200 text-slate-600 hover:border-blue-300 hover:text-blue-600'
                            }`}
                          >
                    ...
                    <div className="space-y-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                      <span className="text-[9px] font-bold text-slate-500 uppercase block">b. Dental</span>
                      <div className="flex gap-3">
                        {[
                          { id: 'open-bite', label: 'Open-bite' },
                          { id: 'deep-bite', label: 'Deep-bite' }
                          ...
                          <button
                            key={v.id}
                            onClick={() => handleUpdateProblem('developmental', 'verticalProblems.dental', problemList.developmental.verticalProblems.dental === v.id ? null : v.id)}
                            className={`px-4 py-2 rounded-xl text-[10px] font-bold transition-all border ${
                              problemList.developmental.verticalProblems.dental === v.id
                              ? 'bg-blue-600 border-blue-600 text-white shadow-md'
                              : 'bg-white border-slate-200 text-slate-600 hover:border-blue-300 hover:text-blue-600'
                            }`}
                          >
